...
class SQLite3_result {
 public:
  ...
  SQLite3_result(sqlite3_stmt *stmt)
  {
    ...
    {
      add_column_definition(sqlite3_column_type(stmt, i),
  ...
  ~SQLite3_result()
  {
    for (std::vector<SQLite3_column *>::iterator it=
             column_definition.begin();
...
class SQLite3DB {
 ...
 public:
  ...
  bool execute_statement(const char *, char **, int *, int *,
                         SQLite3_result **);
  bool execute_statement_raw(const char *, char **, int *, int *,
                             sqlite3_stmt **);
