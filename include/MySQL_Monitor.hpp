...
#define MONITOR_SQLITE_TABLE_MYSQL_SERVER_CONNECT_LOG                        \
  "CREATE TABLE mysql_server_connect_log (hostname VARCHAR NOT NULL , port " \
  "INT NOT NULL DEFAULT 3306 , time_start_us INT NOT NULL DEFAULT 0 , "      \
  "connect_success_time_us INT DEFAULT 0 , connect_error VARCHAR , PRIMARY " \
  "KEY (hostname, port, time_start_us))"

#define MONITOR_SQLITE_TABLE_MYSQL_SERVER_PING_LOG                         \
  "CREATE TABLE mysql_server_ping_log ( hostname VARCHAR NOT NULL , port " \
  "INT NOT NULL DEFAULT 3306 , time_start_us INT NOT NULL DEFAULT 0 , "    \
  "ping_success_time_us INT DEFAULT 0 , ping_error VARCHAR , PRIMARY KEY " \
  "(hostname, port, time_start_us))"

#define MONITOR_SQLITE_TABLE_MYSQL_SERVER_READ_ONLY_LOG                      \
  "CREATE TABLE mysql_server_read_only_log ( hostname VARCHAR NOT NULL , "   \
  "port INT NOT NULL DEFAULT 3306 , time_start_us INT NOT NULL DEFAULT 0 , " \
  "success_time_us INT DEFAULT 0 , read_only INT DEFAULT 1 , error VARCHAR " \
  ", PRIMARY KEY (hostname, port, time_start_us))"

#define MONITOR_SQLITE_TABLE_MYSQL_SERVER_REPLICATION_LAG_LOG             \
  "CREATE TABLE mysql_server_replication_lag_log ( hostname VARCHAR NOT " \
  "NULL , port INT NOT NULL DEFAULT 3306 , time_start_us INT NOT NULL "   \
  "DEFAULT 0 , success_time_us INT DEFAULT 0 , repl_lag INT DEFAULT 0 , " \
  "error VARCHAR , PRIMARY KEY (hostname, port, time_start_us))"

#define MONITOR_SQLITE_TABLE_MYSQL_SERVER_GROUP_REPLICATION_LOG            \
  "CREATE TABLE mysql_server_group_replication_log (hostname VARCHAR NOT " \
  "NULL , port INT NOT NULL DEFAULT 3306 , time_start_us INT NOT NULL "    \
  "DEFAULT 0 , success_time_us INT DEFAULT 0 , viable_candidate VARCHAR "  \
  "NOT NULL DEFAULT 'NO' , read_only VARCHAR NOT NULL DEFAULT 'YES' , "    \
  "transactions_behind INT DEFAULT 0 , error VARCHAR , PRIMARY KEY "       \
  "(hostname, port, time_start_us))"
...
class MyGR_monitor_node {
 ...
 public:
  ...
  ~MyGR_monitor_node();
  bool add_entry(unsigned long long _st, unsigned long long _ct,
                 long long _tb, bool _pp, bool _ro,
                 char *_error);  // return true if status changed
};
...
class MySQL_Monitor {
 private:
  ...
  void insert_into_tables_defs(std::vector<table_def_t *> *tables_defs,
 ...
 public:
  pthread_mutex_t
      group_replication_mutex;  // for simplicity, a mutex instead of a rwlock
