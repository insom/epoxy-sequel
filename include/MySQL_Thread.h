...
#ifdef IDLE_THREADS
typedef struct _conn_exchange_t {
...
class ProxySQL_Poll {
 private:
  void shrink()
  {
    ...
    myds= (MySQL_Data_Stream **)realloc(
        myds, new_size * sizeof(MySQL_Data_Stream *));
    last_recv= (unsigned long long *)realloc(
        last_recv, new_size * sizeof(unsigned long long));
    last_sent= (unsigned long long *)realloc(
        last_sent, new_size * sizeof(unsigned long long));
    size= new_size;
  };
 ...
 public:
  ...
  ~ProxySQL_Poll()
  {
    ...
    for (i= 0; i < len; i++)
    {
      if (myds[i] &&  // fix bug #278 . This should be caused by not initialized datastreams used to ping the backend
...
class MySQL_Thread {
 ...
 public:
  void
      *gen_args;  // this is a generic pointer to create any sort of structure
  ...
  void listener_handle_new_connection(MySQL_Data_Stream *myds,
                                      unsigned int       n);
...
class MySQL_Threads_Handler {
 ...
 public:
  ...
  proxysql_mysql_thread_t *create_thread(unsigned int tn,
  ...
  void Get_Memory_Stats();
  void kill_connection_or_query(uint32_t _thread_session_id, bool query,
                                char *username);
};
