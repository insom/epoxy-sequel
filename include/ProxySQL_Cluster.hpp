...
#define CLUSTER_QUERY_MYSQL_SERVERS                                         \
  "SELECT hostgroup_id, hostname, port, status, weight, compression, "      \
  "max_connections, max_replication_lag, use_ssl, max_latency_ms, comment " \
  "FROM runtime_mysql_servers WHERE status<>'OFFLINE_HARD'"
#define CLUSTER_QUERY_MYSQL_REPLICATION_HOSTGROUPS                        \
  "SELECT writer_hostgroup, reader_hostgroup, COALESCE(comment,'') FROM " \
...
class ProxySQL_Node_Entry {
 ...
 public:
  uint64_t get_hash();
  ProxySQL_Node_Entry(char *_hostname, uint16_t _port, uint64_t _weight,
                      char *_comment);
  ~ProxySQL_Node_Entry();
  bool     get_active();
...
class ProxySQL_Cluster_Nodes {
 ...
 public:
  ...
  bool Update_Node_Metrics(char *_h, uint16_t _p, MYSQL_RES *_r,
