...
struct __QC_entry_t {
  ...
  KV_BtreeArray
      *       kv;    // pointer to the KV_BtreeArray where the entry is stored
  ...
  unsigned long long
      expire_ms;  // when the entry will expire, monotonic , millisecond granularity
  unsigned long long
           access_ms;  // when the entry was read last , monotonic , millisecond granularity
...
class Query_Cache {
 ...
 public:
  ...
  ~Query_Cache();
  void print_version();
  bool set(uint64_t, const unsigned char *, uint32_t, unsigned char *,
           uint32_t, unsigned long long, unsigned long long);
  unsigned char * get(uint64_t, const unsigned char *, const uint32_t,
