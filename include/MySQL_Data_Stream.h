...
class MySQL_Data_Stream {
 ...
 public:
  ...
  MySQL_Backend *
                   mybe;  // if this is a connection to a mysql server, this points to a backend structure
  SSL *            ssl;
  ...
  void init(enum MySQL_DS_type, MySQL_Session *,
            int);  // initialize with arguments
  void shut_soft();
  void shut_hard();
  int  read_from_net();
  int  write_to_net();
  ...
  void setDSS(enum mysql_data_stream_status dss) { DSS= dss; }
  ...
  int myds_connect(char *, int,
                   int *);  // the data stream MUST be initialized
