...
class MySQL_STMT_Global_info {
 ...
 public:
  ...
  MySQL_STMT_Global_info(uint64_t id, unsigned int h, char *u, char *s,
                         char *q, unsigned int ql, MYSQL_STMT *stmt,
                         uint64_t _h);
...
class MySQL_STMTs_meta {
 ...
 public:
  ...
  ~MySQL_STMTs_meta()
  {
    for (std::map<uint32_t, stmt_execute_metadata_t *>::iterator it=
             m.begin();
         it != m.end(); ++it)
    {
...
class MySQL_STMTs_local_v14 {
 ...
 public:
  ...
  unsigned int get_num_backend_stmts()
  {
    return backend_stmt_to_global_ids.size();
  }
  ...
  MYSQL_STMT *find_backend_stmt_by_global_id(uint32_t global_statement_id)
  {
    ...
    if (s != global_stmt_to_backend_stmt.end())
    {  // found
      return s->second;
    }
    return NULL;  // not found
...
class MySQL_STMT_Manager_v14 {
 private:
  ...
  pthread_rwlock_t rwlock_;
  std::map<uint64_t, MySQL_STMT_Global_info *>
      map_stmt_id_to_info;  // map using statement id
  std::map<uint64_t, MySQL_STMT_Global_info *>
                       map_stmt_hash_to_info;  // map using hashes
 ...
 public:
  ...
  MySQL_STMT_Global_info *find_prepared_statement_by_hash(uint64_t hash,
                                                          bool lock= true);
  MySQL_STMT_Global_info *find_prepared_statement_by_stmt_id(uint64_t id,
                                                             bool lock= true);
  ...
  MySQL_STMT_Global_info *add_prepared_statement(unsigned int h, char *u,
                                                 char *s, char *q,
                                                 unsigned int ql,
                                                 MYSQL_STMT * stmt,
                                                 int _cache_ttl, int _timeout,
                                                 int _delay, bool lock= true);
  void                    get_metrics(uint64_t *c_unique, uint64_t *c_total,
