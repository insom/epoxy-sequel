...
struct _Query_Processor_rule_t {
  ...
  uint64_t hits;
  struct _Query_Processor_rule_t
      *parent;  // pointer to parent, to speed up parent update
};
...
class Query_Processor_Output {
 public:
  ...
  void *       operator new(size_t size) { return l_alloc(size); }
...
class Query_Processor {
 ...
 public:
  ...
  bool insert(
      QP_rule_t *qr,
      bool       lock=
          true);  // insert a new rule. Uses a generic void pointer to a structure that may vary depending from the Query Processor
  QP_rule_t *new_query_rule(
      ...
      int retries, int delay, int next_query_flagIN, int mirror_hostgroup,
      int mirror_flagOUT, char *error_msg, char *OK_msg, int sticky_conn,
      int multiplex, int log, bool apply,
      char *
          comment);  // to use a generic query rule struct, this is generated by this function and returned as generic void pointer
  ...
  void update_query_digest(SQP_par_t *qp, int hid,
