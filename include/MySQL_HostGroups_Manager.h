...
#ifdef DEBUG
#define MYHGM_MYSQL_SERVERS                                                  \
  "CREATE TABLE mysql_servers ( hostgroup_id INT NOT NULL DEFAULT 0 , "      \
  "hostname VARCHAR NOT NULL , port INT NOT NULL DEFAULT 3306 , weight INT " \
  "CHECK (weight >= 0) NOT NULL DEFAULT 1 , status INT CHECK (status IN "    \
  "(0, 1, 2, 3, 4)) NOT NULL DEFAULT 0 , compression INT CHECK "             \
  "(compression >=0 AND compression <= 102400) NOT NULL DEFAULT 0 , "        \
  "max_connections INT CHECK (max_connections >=0) NOT NULL DEFAULT 1000 , " \
  "max_replication_lag INT CHECK (max_replication_lag >= 0 AND "             \
  "max_replication_lag <= 126144000) NOT NULL DEFAULT 0 , use_ssl INT "      \
  "CHECK (use_ssl IN(0,1)) NOT NULL DEFAULT 0 , max_latency_ms INT "         \
  "UNSIGNED CHECK (max_latency_ms>=0) NOT NULL DEFAULT 0 , comment VARCHAR " \
  "NOT NULL DEFAULT '' , mem_pointer INT NOT NULL DEFAULT 0 , PRIMARY KEY "  \
  "(hostgroup_id, hostname, port) )"
#define MYHGM_MYSQL_SERVERS_INCOMING                                         \
  "CREATE TABLE mysql_servers_incoming ( hostgroup_id INT NOT NULL DEFAULT " \
  "0 , hostname VARCHAR NOT NULL , port INT NOT NULL DEFAULT 3306 , weight " \
  "INT CHECK (weight >= 0) NOT NULL DEFAULT 1 , status INT CHECK (status "   \
  "IN (0, 1, 2, 3, 4)) NOT NULL DEFAULT 0 , compression INT CHECK "          \
  "(compression >=0 AND compression <= 102400) NOT NULL DEFAULT 0 , "        \
  "max_connections INT CHECK (max_connections >=0) NOT NULL DEFAULT 1000 , " \
  "max_replication_lag INT CHECK (max_replication_lag >= 0 AND "             \
  "max_replication_lag <= 126144000) NOT NULL DEFAULT 0 , use_ssl INT "      \
...
#define MYHGM_MYSQL_SERVERS                                                  \
  "CREATE TABLE mysql_servers ( hostgroup_id INT NOT NULL DEFAULT 0 , "      \
  "hostname VARCHAR NOT NULL , port INT NOT NULL DEFAULT 3306 , weight INT " \
  "NOT NULL DEFAULT 1 , status INT NOT NULL DEFAULT 0 , compression INT "    \
  "NOT NULL DEFAULT 0 , max_connections INT NOT NULL DEFAULT 1000 , "        \
  "max_replication_lag INT NOT NULL DEFAULT 0 , use_ssl INT NOT NULL "       \
  "DEFAULT 0 , max_latency_ms INT UNSIGNED NOT NULL DEFAULT 0 , comment "    \
  "VARCHAR NOT NULL DEFAULT '' , mem_pointer INT NOT NULL DEFAULT 0 , "      \
  "PRIMARY KEY (hostgroup_id, hostname, port) )"
#define MYHGM_MYSQL_SERVERS_INCOMING                                         \
  "CREATE TABLE mysql_servers_incoming ( hostgroup_id INT NOT NULL DEFAULT " \
  "0 , hostname VARCHAR NOT NULL , port INT NOT NULL DEFAULT 3306 , weight " \
  "INT NOT NULL DEFAULT 1 , status INT NOT NULL DEFAULT 0 , compression "    \
  "INT NOT NULL DEFAULT 0 , max_connections INT NOT NULL DEFAULT 1000 , "    \
  "max_replication_lag INT NOT NULL DEFAULT 0 , use_ssl INT NOT NULL "       \
  "DEFAULT 0 , max_latency_ms INT UNSIGNED NOT NULL DEFAULT 0 , comment "    \
  "VARCHAR NOT NULL DEFAULT '' , PRIMARY KEY (hostgroup_id, hostname, "      \
  "port))"
#endif /* DEBUG */
#define MYHGM_MYSQL_REPLICATION_HOSTGROUPS                                  \
...
#define MYHGM_MYSQL_GROUP_REPLICATION_HOSTGROUPS                             \
...
class MySrvC {  // MySQL Server Container
 public:
  ...
  bool
                 shunned_and_kill_all_connections;  // if a serious failure is detected, this will cause all connections to die even if the server is just shunned
  bool           use_ssl;
  char *         comment;
  ...
  MySrvC(char *, uint16_t, unsigned int, enum MySerStatus, unsigned int,
         ...
         unsigned int _use_ssl, unsigned int _max_latency_ms, char *_comment);
  ~MySrvC();
...
class Group_Replication_Info {
 public:
  ...
  bool  __active;
  bool
      need_converge;  // this is set to true on LOAD MYSQL SERVERS TO RUNTIME . This ensure that checks wil take an action
  int current_num_writers;
  int current_num_backup_writers;
  int current_num_readers;
  int current_num_offline;
  Group_Replication_Info(int w, int b, int r, int o, int mw, int mtb, bool _a,
                         bool _w, char *c);
  bool update(int b, int r, int o, int mw, int mtb, bool _a, bool _w,
              char *c);
...
class MySQL_HostGroups_Manager {
 ...
 public:
  ...
  void wrlock();
  void wrunlock();
  bool server_add(unsigned int hid, char *add, uint16_t p= 3306,
                  unsigned int     _weight= 1,
                  enum MySerStatus status= MYSQL_SERVER_STATUS_ONLINE,
                  unsigned int _comp= 0, unsigned int _max_connections= 100,
                  unsigned int _max_replication_lag= 0,
                  unsigned int _use_ssl= 0, unsigned int _max_latency_ms= 0,
                  char *comment= NULL);
  ...
  MySQL_Connection *get_MyConn_from_pool(unsigned int   hid,
                                         MySQL_Session *sess, bool ff= false);

  void            drop_all_idle_connections();
  int             get_multiple_idle_connections(int, unsigned long long,
                                                MySQL_Connection **, int);
  ...
  void         set_server_current_latency_us(char *hostname, int port,
                                             unsigned int _current_latency_us);
  unsigned long long Get_Memory_Stats();

  void update_group_replication_set_offline(char *_hostname, int _port,
                                            int   _writer_hostgroup,
                                            char *error);
  void update_group_replication_set_read_only(char *_hostname, int _port,
                                              int   _writer_hostgroup,
                                              char *error);
  void update_group_replication_set_writer(char *_hostname, int _port,
                                           int _writer_hostgroup);
