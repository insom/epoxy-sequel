...
SQLite3DB::~SQLite3DB()
{
  if (db)
  {
    ...
    if (rc != SQLITE_OK)
    {
      proxy_debug(PROXY_DEBUG_SQLITE, 1,
                  "SQLITE: Error on sqlite3_close_v2(): %s\n",
                  sqlite3_errmsg(db));
      if (assert_on_error)
      {
        assert(rc == 0);
...
int SQLite3DB::open(char *__url, int flags)
{
  ...
  {
    proxy_debug(PROXY_DEBUG_SQLITE, 1,
                "SQLITE: Error on sqlite3_open_v2(): %s\n",
                sqlite3_errmsg(db));
    if (assert_on_error)
    {
      assert(rc == 0);
...
bool SQLite3DB::execute(const char *str)
{
  ...
  do
  {
    ...
    if (rc == SQLITE_LOCKED)
    {  // the execution of sqlite3_exec() failed because locked
...
bool SQLite3DB::execute_statement(const char *str, char **error, int *cols,
                                  int *            affected_rows,
  ...
  {  // not a SELECT
    *resultset= NULL;
    do
    {
      ...
      if (rc == SQLITE_LOCKED)
      {  // the execution of the prepared statement failed because locked
        usleep(USLEEP_SQLITE_LOCKED);
      }
    } while (rc == SQLITE_LOCKED);
    if (rc == SQLITE_DONE)
...
bool SQLite3DB::execute_statement_raw(const char *str, char **error,
                                      int *cols, int *affected_rows,
...
uint64_t SQLite3_result::raw_checksum()
{
  ...
  for (std::vector<SQLite3_row *>::iterator it= rows.begin();
       it != rows.end(); ++it)
  {
    SQLite3_row *r= *it;
...
void SQLite3_result::dump_to_stderr()
{
  ...
  i= 0;
  for (std::vector<SQLite3_column *>::iterator it= column_definition.begin();
       it != column_definition.end(); ++it)
  {
    SQLite3_column *r= *it;
