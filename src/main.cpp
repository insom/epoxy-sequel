...
void *mysql_worker_thread_func(void *arg)
{
  ...
  if (!pthread_attr_init(&thread_attr))
  {
    ...
    {
      __sync_fetch_and_add(&GloVars.statuses.stack_memory_mysql_threads,
                           tmp_stack_size);
    }
  }
...
void ProxySQL_Main_process_global_variables(int argc, const char **argv)
{
  ...
  if (GloVars.confFile->OpenFile(GloVars.config_file) == true)
  {
    ...
    if (root.exists("restart_on_missing_heartbeats") == true)
    {
      ...
      bool rc;
      rc= root.lookupValue("restart_on_missing_heartbeats",
    ...
    if (root.exists("execute_on_exit_failure") == true)
    {
      ...
      bool   rc;
      rc= root.lookupValue("execute_on_exit_failure",
                           execute_on_exit_failure);
      if (rc == true)
      {
        GloVars.execute_on_exit_failure=
            strdup(execute_on_exit_failure.c_str());
      }
    }
  }
  else
  {
    ...
    {
      proxy_error(
          "Unable to open config file %s specified in the command line. "
          "Aborting!\n",
          GloVars.config_file);
  ...
  if (GloVars.__cmd_proxysql_datadir == NULL)
  {
    ...
    if (GloVars.configfile_open == true)
    {
      ...
      if (root.exists("restart_on_missing_heartbeats") == true)
      {
        ...
        if (rc == true)
        {
          GloVars.restart_on_missing_heartbeats=
              restart_on_missing_heartbeats;
        }
        else
        {
    ...
    else
    {
      ...
      GloVars.datadir= strdup(t);
      std::cerr << "[Warning]: Cannot open any default config file . Using "
                   "default datadir in current working directory "
                << GloVars.datadir << endl;
    }
  }
  else
  {
  ...
  GloVars.admindb= (char *)malloc(strlen(GloVars.datadir) +
                                  strlen((char *)"proxysql.db") + 2);
  sprintf(GloVars.admindb, "%s/%s", GloVars.datadir, (char *)"proxysql.db");

  GloVars.statsdb_disk= (char *)malloc(
      strlen(GloVars.datadir) + strlen((char *)"proxysql_stats.db") + 2);
  sprintf(GloVars.statsdb_disk, "%s/%s", GloVars.datadir,
          (char *)"proxysql_stats.db");

  GloVars.errorlog= (char *)malloc(strlen(GloVars.datadir) +
                                   strlen((char *)"proxysql.log") + 2);
  sprintf(GloVars.errorlog, "%s/%s", GloVars.datadir, (char *)"proxysql.log");

  GloVars.pid= (char *)malloc(strlen(GloVars.datadir) +
                              strlen((char *)"proxysql.pid") + 2);
  ...
  if (GloVars.__cmd_proxysql_initial == true)
  {
    ...
    rename(
        GloVars.admindb,
        newpath);  // FIXME: should we check return value, or ignore whatever it successed or not?
  }
...
void ProxySQL_Main_init_Query_Cache_module()
{
  ...
  GloQC->print_version();
  pthread_create(&GloQC->purge_thread_id, NULL,
                 mysql_shared_query_cache_funct, NULL);
}
...
void ProxySQL_Main_init()
{
...

  {
    ...
    {
      fprintf(stderr,
              "SQLITE: Error on sqlite3_config(SQLITE_CONFIG_URI,1)\n");
      assert(i == SQLITE_OK);
      exit(EXIT_FAILURE);
    }
  }
...
void ProxySQL_daemonize_phase1(char *argv0)
{
  ...
  if (rc)
  {
    daemon_log(LOG_ERR, "Could not chdir into datadir: %s . Error: %s",
               GloVars.datadir, strerror(errno));
    exit(EXIT_FAILURE);
  }
    ...
    while (glovars.shutdown == 0)
    {
      ...
      if (GloMTH)
      {
        ...
        unsigned long long poll_timeout=
            (unsigned int)GloMTH->variables.poll_timeout;
        ...
        if (GloVars.global.idle_threads)
        {
          if (GloMTH->mysql_threads)
          {
            for (i= 0; i < GloMTH->num_threads; i++)
            {
              ...
              {
                atomic_curtime=
                    GloMTH->mysql_threads_idles[i].worker->atomic_curtime;
                if (curtime > atomic_curtime + poll_timeout)
                {
                  threads_missing_heartbeat++;
                }
        ...
        if (threads_missing_heartbeat)
        {
          ...
          if (missed_heartbeats >=
              (unsigned int)GloVars.restart_on_missing_heartbeats)
          {
