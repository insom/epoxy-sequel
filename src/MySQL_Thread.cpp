...
CHARSET_INFO *proxysql_find_charset_collate_names(const char *csname,
...
MySQL_Listeners_Manager::MySQL_Listeners_Manager() { ifaces= new PtrArray(); }
...
int MySQL_Listeners_Manager::add(const char *iface, unsigned int num_threads,
                                 int **perthrsocks)
{
  ...
  {
    s= (atoi(port) ? listen_on_port(address, atoi(port), PROXYSQL_LISTEN_LEN)
                   : listen_on_unix(address, PROXYSQL_LISTEN_LEN));
  }
  else
  {
    if (atoi(port) == 0)
    ...
    else
    {
      ...
      for (i= 0; i < num_threads; i++)
      {
        ...
        ioctl_FIONBIO(s, 1);
        iface_info *ifi=
            new iface_info((char *)iface, address, atoi(port), s);
        ifaces->add(ifi);
        l_perthrsocks[i]= s;
      }
...
MySQL_Threads_Handler::MySQL_Threads_Handler()
{
  ...
  variables.default_max_latency_ms=
      1 *
      1000;  // by default, the maximum allowed latency for a host is 1000ms
  ...
  variables.eventslog_filename=
      strdup((char *)"");  // proxysql-mysql-eventslog is recommended
  variables.eventslog_filesize= 100 * 1024 * 1024;
  variables.server_capabilities=
      CLIENT_FOUND_ROWS | CLIENT_PROTOCOL_41 | CLIENT_IGNORE_SIGPIPE |
  ...
  if (rc > -1)
  {
    ...
    if (perthrsocks == NULL)
    {
      for (i= 0; i < num_threads; i++)
      {
        ...
        while (!__sync_bool_compare_and_swap(
            &thr->mypolls.pending_listener_add, 0, rc))
        {
    ...
    else
    {
      for (i= 0; i < num_threads; i++)
      {
        MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
        while (!__sync_bool_compare_and_swap(
            &thr->mypolls.pending_listener_add, 0, perthrsocks[i]))
        {
          usleep(10);  // pause a bit
        }
      }
...
int MySQL_Threads_Handler::listener_del(const char *iface)
{
  ...
  while ((idx= MLM->find_idx(iface)) >= 0)
  {
    ...
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      while (!__sync_bool_compare_and_swap(&thr->mypolls.pending_listener_del,
                                           0, fd))
        ;
    }
    for (i= 0; i < num_threads; i++)
...
void MySQL_Threads_Handler::wrlock() { pthread_rwlock_wrlock(&rwlock); }

void MySQL_Threads_Handler::wrunlock() { pthread_rwlock_unlock(&rwlock); }

void MySQL_Threads_Handler::commit()
{
  __sync_add_and_fetch(&__global_MySQL_Thread_Variables_version, 1);
  proxy_debug(PROXY_DEBUG_MYSQL_SERVER, 1,
              "Increasing version number to %d - all threads will notice "
              "this and refresh their variables\n",
...
char *MySQL_Threads_Handler::get_variable_string(char *name)
{
  if (!strncasecmp(name, "monitor_", 8))
  {
    if (!strcasecmp(name, "monitor_username"))
      return strdup(variables.monitor_username);
    if (!strcasecmp(name, "monitor_password"))
      return strdup(variables.monitor_password);
  ...
  if (!strncasecmp(name, "ssl_", 4))
  {
    ...
    {
      if (variables.ssl_p2s_cert == NULL ||
          strlen(variables.ssl_p2s_cert) == 0)
      {
        return NULL;
      }
      else
    ...
    {
      if (variables.ssl_p2s_cipher == NULL ||
          strlen(variables.ssl_p2s_cipher) == 0)
      {
        return NULL;
      }
      else
  ...
  if (!strcasecmp(name, "server_version"))
    return strdup(variables.server_version);
  if (!strcasecmp(name, "eventslog_filename"))
    return strdup(variables.eventslog_filename);
  if (!strcasecmp(name, "default_schema"))
    return strdup(variables.default_schema);
  if (!strcasecmp(name, "interfaces"))
    return strdup(variables.interfaces);
...
uint16_t MySQL_Threads_Handler::get_variable_uint16(char *name)
{
  if (!strcasecmp(name, "server_capabilities"))
    return variables.server_capabilities;
  proxy_error("Not existing variable: %s\n", name);
  assert(0);
  return 0;
}
...
uint8_t MySQL_Threads_Handler::get_variable_uint8(char *name)
{
  if (!strcasecmp(name, "default_charset"))
    return variables.default_charset;
  proxy_error("Not existing variable: %s\n", name);
  assert(0);
  return 0;
}
...
int MySQL_Threads_Handler::get_variable_int(char *name)
{
  ...
  if (!strcasecmp(name, "session_debug"))
    return (int)variables.session_debug;
#endif /* DEBUG */
  if (!strncasecmp(name, "monitor_", 8))
  {
    if (!strcasecmp(name, "monitor_enabled"))
      return (int)variables.monitor_enabled;
    if (!strcasecmp(name, "monitor_history"))
      return (int)variables.monitor_history;
    if (!strcasecmp(name, "monitor_connect_interval"))
      return (int)variables.monitor_connect_interval;
    if (!strcasecmp(name, "monitor_connect_timeout"))
      return (int)variables.monitor_connect_timeout;
    if (!strcasecmp(name, "monitor_ping_interval"))
      return (int)variables.monitor_ping_interval;
    if (!strcasecmp(name, "monitor_ping_max_failures"))
      return (int)variables.monitor_ping_max_failures;
    if (!strcasecmp(name, "monitor_ping_timeout"))
      return (int)variables.monitor_ping_timeout;
    if (!strcasecmp(name, "monitor_read_only_interval"))
      return (int)variables.monitor_read_only_interval;
    if (!strcasecmp(name, "monitor_read_only_timeout"))
      return (int)variables.monitor_read_only_timeout;
    if (!strcasecmp(name, "monitor_read_only_max_timeout_count"))
      return (int)variables.monitor_read_only_max_timeout_count;
    if (!strcasecmp(name, "monitor_replication_lag_interval"))
      return (int)variables.monitor_replication_lag_interval;
    if (!strcasecmp(name, "monitor_replication_lag_timeout"))
      return (int)variables.monitor_replication_lag_timeout;
    if (!strcasecmp(name, "monitor_groupreplication_healthcheck_interval"))
      return (int)variables.monitor_groupreplication_healthcheck_interval;
    if (!strcasecmp(name, "monitor_groupreplication_healthcheck_timeout"))
      return (int)variables.monitor_groupreplication_healthcheck_timeout;
    if (!strcasecmp(name, "monitor_query_interval"))
      return (int)variables.monitor_query_interval;
    if (!strcasecmp(name, "monitor_query_timeout"))
      return (int)variables.monitor_query_timeout;
    if (!strcasecmp(name, "monitor_slave_lag_when_null"))
      return (int)variables.monitor_slave_lag_when_null;
    if (!strcasecmp(name, "monitor_wait_timeout"))
      return (int)variables.monitor_wait_timeout;
    if (!strcasecmp(name, "monitor_writer_is_also_reader"))
      return (int)variables.monitor_writer_is_also_reader;
  }
  if (!strcasecmp(name, "shun_on_failures"))
    return (int)variables.shun_on_failures;
  if (!strcasecmp(name, "shun_recovery_time_sec"))
    return (int)variables.shun_recovery_time_sec;
  if (!strcasecmp(name, "query_retries_on_failure"))
    return (int)variables.query_retries_on_failure;
  if (!strcasecmp(name, "connect_retries_on_failure"))
    return (int)variables.connect_retries_on_failure;
  if (!strcasecmp(name, "connection_delay_multiplex_ms"))
    return (int)variables.connection_delay_multiplex_ms;
  if (!strcasecmp(name, "connection_max_age_ms"))
    return (int)variables.connection_max_age_ms;
  if (!strcasecmp(name, "connect_timeout_server"))
    return (int)variables.connect_timeout_server;
  if (!strcasecmp(name, "connect_timeout_server_max"))
    return (int)variables.connect_timeout_server_max;
  if (!strcasecmp(name, "connect_retries_delay"))
    return (int)variables.connect_retries_delay;
  if (!strcasecmp(name, "eventslog_filesize"))
    return (int)variables.eventslog_filesize;
  if (!strcasecmp(name, "max_allowed_packet"))
    return (int)variables.max_allowed_packet;
  if (!strcasecmp(name, "throttle_connections_per_sec_to_hostgroup"))
    return (int)variables.throttle_connections_per_sec_to_hostgroup;
  if (!strcasecmp(name, "max_transaction_time"))
    return (int)variables.max_transaction_time;
  if (!strcasecmp(name, "hostgroup_manager_verbose"))
    return (int)variables.hostgroup_manager_verbose;
  if (!strcasecmp(name, "threshold_query_length"))
    return (int)variables.threshold_query_length;
  if (!strcasecmp(name, "threshold_resultset_size"))
    return (int)variables.threshold_resultset_size;
  if (!strcasecmp(name, "query_digests_max_digest_length"))
    return (int)variables.query_digests_max_digest_length;
  if (!strcasecmp(name, "query_digests_max_query_length"))
    return (int)variables.query_digests_max_query_length;
  if (!strcasecmp(name, "wait_timeout"))
    return (int)variables.wait_timeout;
  if (!strcasecmp(name, "throttle_max_bytes_per_second_to_client"))
    return (int)variables.throttle_max_bytes_per_second_to_client;
  if (!strcasecmp(name, "throttle_ratio_server_to_client"))
    return (int)variables.throttle_ratio_server_to_client;
  if (!strcasecmp(name, "max_connections"))
    return (int)variables.max_connections;
  if (!strcasecmp(name, "max_stmts_per_connection"))
    return (int)variables.max_stmts_per_connection;
  if (!strcasecmp(name, "max_stmts_cache"))
    return (int)variables.max_stmts_cache;
  if (!strcasecmp(name, "mirror_max_concurrency"))
    return (int)variables.mirror_max_concurrency;
  if (!strcasecmp(name, "mirror_max_queue_length"))
    return (int)variables.mirror_max_queue_length;
  if (!strcasecmp(name, "default_query_delay"))
    return (int)variables.default_query_delay;
  if (!strcasecmp(name, "default_query_timeout"))
    return (int)variables.default_query_timeout;
  if (!strcasecmp(name, "query_processor_iterations"))
    return (int)variables.query_processor_iterations;
  if (!strcasecmp(name, "query_processor_regex"))
    return (int)variables.query_processor_regex;
  if (!strcasecmp(name, "auto_increment_delay_multiplex"))
    return (int)variables.auto_increment_delay_multiplex;
  if (!strcasecmp(name, "default_max_latency_ms"))
    return (int)variables.default_max_latency_ms;
  if (!strcasecmp(name, "long_query_time"))
    return (int)variables.long_query_time;
  if (!strcasecmp(name, "query_cache_size_MB"))
    return (int)variables.query_cache_size_MB;
  if (!strcasecmp(name, "free_connections_pct"))
    return (int)variables.free_connections_pct;
#ifdef IDLE_THREADS
  if (!strcasecmp(name, "session_idle_ms"))
    return (int)variables.session_idle_ms;
#endif  // IDLE_THREADS
  ...
  if (!strcasecmp(name, "ping_timeout_server"))
    return (int)variables.ping_timeout_server;
  if (!strcasecmp(name, "have_compress"))
    return (int)variables.have_compress;
  if (!strcasecmp(name, "client_found_rows"))
    return (int)variables.client_found_rows;
  if (!strcasecmp(name, "multiplexing"))
    return (int)variables.multiplexing;
  if (!strcasecmp(name, "hostgroup_manager_verbose"))
    return (int)variables.hostgroup_manager_verbose;
  if (!strcasecmp(name, "forward_autocommit"))
    return (int)variables.forward_autocommit;
  ...
  if (!strcasecmp(name, "autocommit_false_not_reusable"))
    return (int)variables.autocommit_false_not_reusable;
  if (!strcasecmp(name, "autocommit_false_is_transaction"))
    return (int)variables.autocommit_false_is_transaction;
  if (!strcasecmp(name, "verbose_query_error"))
    return (int)variables.verbose_query_error;
  if (!strcasecmp(name, "commands_stats"))
    return (int)variables.commands_stats;
  if (!strcasecmp(name, "query_digests"))
    return (int)variables.query_digests;
  if (!strcasecmp(name, "query_digests_lowercase"))
    return (int)variables.query_digests_lowercase;
  if (!strcasecmp(name, "connpoll_reset_queue_length"))
    return (int)variables.connpoll_reset_queue_length;
  ...
  if (!strcasecmp(name, "stats_time_query_processor"))
    return (int)variables.stats_time_query_processor;
  if (!strcasecmp(name, "kill_backend_connection_when_disconnect"))
    return (int)variables.kill_backend_connection_when_disconnect;
  if (!strcasecmp(name, "sessions_sort"))
    return (int)variables.sessions_sort;
#ifdef IDLE_THREADS
  ...
  if (!strcasecmp(name, "servers_stats"))
    return (int)variables.servers_stats;
  if (!strcasecmp(name, "default_reconnect"))
    return (int)variables.default_reconnect;
  if (!strcasecmp(name, "poll_timeout"))
    return variables.poll_timeout;
  if (!strcasecmp(name, "poll_timeout_on_failure"))
    return variables.poll_timeout_on_failure;
  if (!strcasecmp(name, "stacksize"))
    return (stacksize ? stacksize : DEFAULT_STACK_SIZE);
  if (!strcasecmp(name, "client_multi_statements"))
    return (int)variables.client_multi_statements;
  proxy_error("Not existing variable: %s\n", name);
  assert(0);
  return 0;
}

char *MySQL_Threads_Handler::get_variable(char *name)
...
#define INTBUFSIZE 4096
  ...
  if (!strncasecmp(name, "monitor_", 8))
  {
    ...
    {
      sprintf(intbuf, "%d",
              variables.monitor_groupreplication_healthcheck_interval);
      return strdup(intbuf);
    }
    if (!strcasecmp(name, "monitor_groupreplication_healthcheck_timeout"))
    {
      sprintf(intbuf, "%d",
              variables.monitor_groupreplication_healthcheck_timeout);
      return strdup(intbuf);
    }
    if (!strcasecmp(name, "monitor_query_interval"))
    {
    ...
    {
      return strdup(
          (variables.monitor_writer_is_also_reader ? "true" : "false"));
    }
    if (!strcasecmp(name, "monitor_wait_timeout"))
    {
  ...
  if (!strcasecmp(name, "default_charset"))
  {
    const CHARSET_INFO *c=
        proxysql_find_charset_nr(variables.default_charset);
    if (!c)
    {
  ...
  {
    sprintf(intbuf, "%d",
            variables.throttle_connections_per_sec_to_hostgroup);
    return strdup(intbuf);
  }
  if (!strcasecmp(name, "max_transaction_time"))
  {
  ...
  {
    return strdup(
        (variables.autocommit_false_not_reusable ? "true" : "false"));
  }
  if (!strcasecmp(name, "autocommit_false_is_transaction"))
  {
    return strdup(
        (variables.autocommit_false_is_transaction ? "true" : "false"));
  }
  if (!strcasecmp(name, "verbose_query_error"))
  {
  ...
  if (!strcasecmp(name, "kill_backend_connection_when_disconnect"))
  {
    return strdup((variables.kill_backend_connection_when_disconnect
                       ? "true"
                       : "false"));
  }
  if (!strcasecmp(name, "sessions_sort"))
  ...
  {
    return strdup(
        (variables.session_idle_show_processlist ? "true" : "false"));
  }
#endif  // IDLE_THREADS
  if (!strcasecmp(name, "servers_stats"))
  {
...
bool MySQL_Threads_Handler::set_variable(char *name, char *value)
{  // this is the public function, accessible from admin
  // IN:
  // name: variable name
  ...
  if (!strcasecmp(name, "server_version"))
  {
    if (vallen)
    {
      ...
      if (strcmp(value, (const char *)"5.1.30") == 0)
      {  // per issue #632 , the default 5.1.30 is replaced with 5.5.30
        variables.server_version= strdup((char *)"5.5.30");
      }
      else
      {
  ...
  if (!strcasecmp(name, "init_connect"))
  {
    if (variables.init_connect)
      free(variables.init_connect);
    variables.init_connect= NULL;
    if (vallen)
    {
  ...
  if (!strcasecmp(name, "default_sql_mode"))
  {
    if (variables.default_sql_mode)
      free(variables.default_sql_mode);
    ...
    if (variables.default_sql_mode == NULL)
    {
      variables.default_sql_mode=
          strdup((char *)MYSQL_DEFAULT_SQL_MODE);  // default
    }
    return true;
  }
  ...
  if (!strcasecmp(name, "default_time_zone"))
  {
    if (variables.default_time_zone)
      free(variables.default_time_zone);
    ...
    if (variables.default_time_zone == NULL)
    {
      variables.default_time_zone=
          strdup((char *)MYSQL_DEFAULT_TIME_ZONE);  // default
    }
    return true;
  }
  ...
  if (!strcasecmp(name, "eventslog_filename"))
  {
    ...
    {
      proxy_error(
          "%s is an invalid value for eventslog_filename, please specify a "
    ...
    else if (value[0] == '/')
    {
      ...
      {
        proxy_error(
            "%s is an invalid value for eventslog_filename path, the "
  ...
  if (!strcasecmp(name, "threads"))
  {
    ...
    if ((num_threads == 0 || num_threads == intv || mysql_threads == NULL) &&
        intv > 0 && intv < 256)
    {
      num_threads= intv;
      return true;
  ...
  if (!strcasecmp(name, "query_digests"))
  {
    ...
    if (strcasecmp(value, "false") == 0 || strcasecmp(value, "0") == 0)
    {
      ...
      proxy_warning(
          "Setting mysql-query_digests=false . This is risky and not "
          "recommended\n");
      return true;
    }
    return false;
...
bool MySQL_Threads_Handler::has_variable(const char *name)
{
  ...
  for (unsigned int i= 0; i < no_vars - 1; ++i)
  {
    ...
    if (strlen(name) == var_len &&
        !strncmp(name, mysql_thread_variables_names[i], var_len))
    {
      return true;
    }
  }
...
void MySQL_Threads_Handler::init(unsigned int num, size_t stack)
{
  ...
  assert(rc == 0);
  mysql_threads= (proxysql_mysql_thread_t *)calloc(
      num_threads, sizeof(proxysql_mysql_thread_t));
#ifdef IDLE_THREADS
  mysql_threads_idles= (proxysql_mysql_thread_t *)calloc(
      num_threads, sizeof(proxysql_mysql_thread_t));
#endif  // IDLE_THREADS
}

proxysql_mysql_thread_t *MySQL_Threads_Handler::create_thread(
    unsigned int tn, void *(*start_routine)(void *), bool idles)
{
  if (idles == false)
  {
    if (pthread_create(&mysql_threads[tn].thread_id, &attr, start_routine,
                       &mysql_threads[tn]) != 0)
    {
  ...
  {
    if (pthread_create(&mysql_threads_idles[tn].thread_id, &attr,
                       start_routine, &mysql_threads_idles[tn]) != 0)
    {
      proxy_error("Thread creation\n");
      assert(0);
    }
...
void MySQL_Threads_Handler::start_listeners()
{
  ...
  if (strlen(_tmp) == 0)
  {
    ...
    GloMTH->set_variable((char *)"interfaces",
                         (char *)"0.0.0.0:6033");  // changed. See isseu #1104
  }
  free(_tmp);
  tokenizer_t tok;
...
MySQL_Threads_Handler::~MySQL_Threads_Handler()
{
  if (variables.monitor_username)
  {
    free(variables.monitor_username);
    variables.monitor_username= NULL;
  }
  if (variables.monitor_password)
  {
    free(variables.monitor_password);
    variables.monitor_password= NULL;
  }
  ...
  if (variables.default_schema)
    free(variables.default_schema);
  if (variables.interfaces)
    free(variables.interfaces);
  if (variables.server_version)
    free(variables.server_version);
  if (variables.init_connect)
    free(variables.init_connect);
  if (variables.default_sql_mode)
  ...
  if (variables.default_time_zone)
    free(variables.default_time_zone);
  if (variables.eventslog_filename)
    free(variables.eventslog_filename);
  if (variables.ssl_p2s_ca)
    free(variables.ssl_p2s_ca);
  if (variables.ssl_p2s_cert)
...
MySQL_Thread::~MySQL_Thread()
{
  if (mysql_sessions)
  {
    ...
    {
      MySQL_Session *sess=
          (MySQL_Session *)mysql_sessions->remove_index_fast(0);
      delete sess;
    }
  ...
  if (mirror_queue_mysql_sessions)
  {
    ...
    {
      MySQL_Session *sess=
          (MySQL_Session *)mirror_queue_mysql_sessions->remove_index_fast(0);
      delete sess;
    }
  ...
  if (mirror_queue_mysql_sessions_cache)
  {
    while (mirror_queue_mysql_sessions_cache->len)
    {
      MySQL_Session *sess=
          (MySQL_Session *)
              mirror_queue_mysql_sessions_cache->remove_index_fast(0);
      delete sess;
    }
  ...
  if (GloVars.global.idle_threads)
  {
    if (idle_mysql_sessions)
    {
      ...
      {
        MySQL_Session *sess=
            (MySQL_Session *)idle_mysql_sessions->remove_index_fast(0);
        delete sess;
      }
    ...
    if (resume_mysql_sessions)
    {
      ...
      {
        MySQL_Session *sess=
            (MySQL_Session *)resume_mysql_sessions->remove_index_fast(0);
        delete sess;
      }
    ...
    if (myexchange.idle_mysql_sessions)
    {
      while (myexchange.idle_mysql_sessions->len)
      {
        MySQL_Session *sess=
            (MySQL_Session *)
                myexchange.idle_mysql_sessions->remove_index_fast(0);
        delete sess;
      }
    ...
    if (myexchange.resume_mysql_sessions)
    {
      while (myexchange.resume_mysql_sessions->len)
      {
        MySQL_Session *sess=
            (MySQL_Session *)
                myexchange.resume_mysql_sessions->remove_index_fast(0);
        delete sess;
      }
  ...
  if (cached_connections)
  {
    ...
    {
      MySQL_Connection *c=
          (MySQL_Connection *)cached_connections->remove_index_fast(0);
      delete c;
    }
  ...
  {
    if (mypolls.myds
            [i] &&  // fix bug #278 . This should be caused by not initialized datastreams used to ping the backend
  ...
  if (mysql_thread___monitor_username)
  {
    free(mysql_thread___monitor_username);
    mysql_thread___monitor_username= NULL;
  }
  if (mysql_thread___monitor_password)
  {
    free(mysql_thread___monitor_password);
    mysql_thread___monitor_password= NULL;
  }
  ...
  {
    free(mysql_thread___default_schema);
    mysql_thread___default_schema= NULL;
  }
  if (mysql_thread___server_version)
  {
    free(mysql_thread___server_version);
    mysql_thread___server_version= NULL;
  }
  if (mysql_thread___init_connect)
  {
    free(mysql_thread___init_connect);
    mysql_thread___init_connect= NULL;
  }
  if (mysql_thread___default_sql_mode)
  {
    free(mysql_thread___default_sql_mode);
    mysql_thread___default_sql_mode= NULL;
  }
  if (mysql_thread___default_time_zone)
  {
    free(mysql_thread___default_time_zone);
    mysql_thread___default_time_zone= NULL;
  }
  if (mysql_thread___eventslog_filename)
  {
    free(mysql_thread___eventslog_filename);
    mysql_thread___eventslog_filename= NULL;
  }
  if (mysql_thread___ssl_p2s_ca)
  {
    free(mysql_thread___ssl_p2s_ca);
    mysql_thread___ssl_p2s_ca= NULL;
  }
  if (mysql_thread___ssl_p2s_cert)
  {
    free(mysql_thread___ssl_p2s_cert);
    mysql_thread___ssl_p2s_cert= NULL;
  }
  if (mysql_thread___ssl_p2s_key)
  {
    free(mysql_thread___ssl_p2s_key);
    mysql_thread___ssl_p2s_key= NULL;
  }
  if (mysql_thread___ssl_p2s_cipher)
  {
    free(mysql_thread___ssl_p2s_cipher);
...
MySQL_Session *MySQL_Thread::create_new_session_and_client_data_stream(
    int _fd)
{
  int            arg_on= 1;
  ...
  setsockopt(sess->client_myds->fd, IPPROTO_TCP, TCP_NODELAY, (char *)&arg_on,
             sizeof(int));
#ifdef __APPLE__
  setsockopt(sess->client_myds->fd, SOL_SOCKET, SO_NOSIGPIPE, (char *)&arg_on,
             sizeof(int));
#endif
  sess->client_myds->init(MYDS_FRONTEND, sess, sess->client_myds->fd);
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p, DataStream=%p -- Created new client "
              "Data Stream\n",
  ...
  sess->client_myds->myprot.init(&sess->client_myds,
                                 sess->client_myds->myconn->userinfo, sess);

  // fix bug 1253 : initialize time_zone
  uint32_t time_zone_int=
      SpookyHash::Hash32(mysql_thread___default_time_zone,
                         strlen(mysql_thread___default_time_zone), 10);
  ...
  sess->client_myds->myconn->options.time_zone=
      strdup(mysql_thread___default_time_zone);
  return sess;
}
...
bool MySQL_Thread::init()
{
  ...
  my_idle_conns= (MySQL_Connection **)malloc(
      sizeof(MySQL_Connection *) * SESSIONS_FOR_CONNECTIONS_HANDLER);
  memset(my_idle_conns, 0,
         sizeof(MySQL_Connection *) * SESSIONS_FOR_CONNECTIONS_HANDLER);
  GloQPro->init_thread();
  refresh_variables();
  i= pipe(pipefd);
  ...
  match_regexes[0]= new Session_Regex(
      (char *)"^SET (|SESSION |@@|@@session.)SQL_LOG_BIN( *)(:|)=( *)");
  match_regexes[1]= new Session_Regex(
      (char *)"^SET (|SESSION |@@|@@session.)SQL_MODE( *)(:|)=( *)");
...
void MySQL_Thread::register_session(MySQL_Session *_sess, bool up_start)
{
  ...
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p -- Registered new session\n",
              _sess->thread, _sess);
}
...
void MySQL_Thread::unregister_session(int idx)
{
  if (mysql_sessions == NULL)
    return;
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p -- Unregistered session\n", this,
              mysql_sessions->index(idx));
...
void MySQL_Thread::run()
{
  ...
  while (shutdown == 0)
  {
    ...
    int num_idles;
    if (processing_idles == true &&
        (last_processing_idles <
         curtime - mysql_thread___ping_timeout_server * 1000))
    {
      processing_idles= false;
    }
    if (processing_idles == false &&
        (last_processing_idles <
         curtime - mysql_thread___ping_interval_server_msec * 1000))
    {
      int i;
      num_idles= MyHGM->get_multiple_idle_connections(
          -1, curtime - mysql_thread___ping_interval_server_msec * 1000,
          my_idle_conns, SESSIONS_FOR_CONNECTIONS_HANDLER);
      for (i= 0; i < num_idles; i++)
      {
        ...
        sess->to_process= 1;
        myds->wait_until= curtime + mysql_thread___ping_timeout_server *
                                        1000;  // max_timeout
  ...
  __run_skip_1:
    ...
    while (mirror_queue_mysql_sessions->len)
    {
      if (__sync_add_and_fetch(
              &GloMTH->status_variables.mirror_sessions_current, 1) >
          (unsigned int)mysql_thread___mirror_max_concurrency)
      {
      ...
      else
      {
        ...
        MySQL_Session *newsess=
            (MySQL_Session *)mirror_queue_mysql_sessions->remove_index_fast(
                idx);
        register_session(newsess);
        newsess->handler();  // execute immediately
        if (newsess->status == WAITING_CLIENT_DATA)
        {  // the mirror session has completed
          unregister_session(mysql_sessions->len - 1);
          unsigned int l= (unsigned int)mysql_thread___mirror_max_concurrency;
          if (mirror_queue_mysql_sessions->len * 0.3 > l)
            l= mirror_queue_mysql_sessions->len * 0.3;
    ...
    for (n= 0; n < mypolls.len; n++)
    {
      ...
      if (myds)
      {
        ...
        if (GloVars.global.idle_threads)
        {
          ...
          {
            if (myds->DSS == STATE_SLEEP &&
                myds->sess->status == WAITING_CLIENT_DATA)
            {
              unsigned long long _tmp_idle=
                  mypolls.last_recv[n] > mypolls.last_sent[n]
                      ? mypolls.last_recv[n]
                      : mypolls.last_sent[n];
              if (_tmp_idle <
                  ((curtime >
                    (unsigned int)mysql_thread___session_idle_ms * 1000)
                       ? (curtime - mysql_thread___session_idle_ms * 1000)
                       : 0))
              {
                if (myds->sess->client_myds == myds &&
                    myds->PSarrayOUT->len == 0 &&
                    (myds->queueOUT.head - myds->queueOUT.tail) == 0)
                {  // extra check
                  unsigned int j;
                  int          conns= 0;
                  for (j= 0; j < myds->sess->mybes->len; j++)
                  {
                    MySQL_Backend *tmp_mybe=
                        (MySQL_Backend *)myds->sess->mybes->index(j);
                  ...
                  if (conns == 0)
                  {
                    ...
                    {
                      MySQL_Session *mysess=
                          (MySQL_Session *)mysql_sessions->index(i);
                      if (mysess == myds->sess)
                      {
                        mysess->thread= NULL;
        ...
        if (myds->wait_until)
        {
          if (myds->wait_until > curtime)
          {
            if (mypolls.poll_timeout == 0 ||
                (myds->wait_until - curtime < mypolls.poll_timeout))
            {
        ...
        if (myds->sess)
        {
          ...
          {
            if (mypolls.poll_timeout == 0 ||
                (myds->sess->pause_until - curtime < mypolls.poll_timeout))
            {
      ...
      {
        if (myds && myds->myds_type == MYDS_FRONTEND &&
            myds->DSS == STATE_SLEEP && myds->sess &&
            myds->sess->status == WAITING_CLIENT_DATA)
        {
        ...
        {
          if (mypolls.myds[n]->DSS > STATE_MARIADB_BEGIN &&
              mypolls.myds[n]->DSS < STATE_MARIADB_END)
          {
        ...
        if (myds && myds->myds_type == MYDS_BACKEND)
        {
              ...
              myds->sess->mirror == false)
          {
            ...
            buffered_data=
                myds->sess->client_myds->PSarrayOUT->len * RESULTSET_BUFLEN;
            buffered_data+=
                myds->sess->client_myds->resultset->len * RESULTSET_BUFLEN;
            ...
            if (buffered_data >
                (unsigned int)mysql_thread___threshold_resultset_size * 4)
            {
              mypolls.fds[n].events= 0;
            }
          }
      ...
      proxy_debug(
          PROXY_DEBUG_NET, 1,
          "Poll for DataStream=%p will be called with FD=%d and events=%d\n",
  ...
  __run_skip_1a:
    ...
    while ((n= __sync_add_and_fetch(&mypolls.pending_listener_add, 0)))
    {  // spin here
      poll_listener_add(n);
      assert(
          __sync_bool_compare_and_swap(&mypolls.pending_listener_add, n, 0));
    }
    ...
    if (GloVars.global.idle_threads && idle_maintenance_thread)
    {
      memset(
          ...
          sizeof(struct epoll_event) *
              MY_EPOLL_THREAD_MAXEVENTS);  // let's make valgrind happy. It also seems that needs to be zeroed anyway
      // we call epoll()
    ...
    while ((n= __sync_add_and_fetch(&mypolls.pending_listener_del, 0)))
    {  // spin here
      poll_listener_del(n);
      assert(
          __sync_bool_compare_and_swap(&mypolls.pending_listener_del, n, 0));
    }

    pthread_mutex_lock(&thread_mutex);
    mypolls.poll_timeout=
        0;  // always reset this to 0 . If a session needs a specific timeout, it will set this one
    ...
    if (maintenance_loop)
    {
      ...
      {
        while (mirror_queue_mysql_sessions_cache->len >
                   mirror_queue_mysql_sessions->len &&
               mirror_queue_mysql_sessions_cache->len > l)
        {
          MySQL_Session *newsess=
              (MySQL_Session *)
                  mirror_queue_mysql_sessions_cache->remove_index_fast(0);
        ...
        for (i= 0; i < rc; i++)
        {
          if (events[i].data.u32)
          {
            ...
            if (events[i].events)
            {
              ...
              MySQL_Session *mysess=
                  (MySQL_Session *)mysql_sessions->index(sess_pos);
              ...
              if (mysql_sessions->len > 1)
              {
                ...
                MySQL_Session *mysess_last=
                    (MySQL_Session *)mysql_sessions->index(
                        mysql_sessions->len - 1);
                if (mysess->thread_session_id !=
                    mysess_last->thread_session_id)
...
#define SESS_TO_SCAN 128
        ...
        for (i= 0; i < SESS_TO_SCAN && mysess_idx < mysql_sessions->len; i++)
        {
          ...
          if (mysess->idle_since < min_idle)
          {
            ...
            if (mysql_sessions->len > 1)
            {
              // take the last element and adjust the map
              MySQL_Session *mysess_last=
                  (MySQL_Session *)mysql_sessions->index(mysql_sessions->len -
                                                         1);
    ...
    for (n= 0; n < mypolls.len; n++)
    {
      ...
      if (myds == NULL)
      {
        ...
        {
          unsigned char c;
          if (read(mypolls.fds[n].fd, &c, 1) == -1)
          {  // read just one byte
            proxy_error("Error during read from signal_all_threads()\n");
          }
          proxy_debug(PROXY_DEBUG_GENERIC, 3,
                      "Got signal from admin , done nothing\n");
                                               ...
                                               unsigned int       n)
{
  ...
  if (mypolls.fds[n].revents)
  {
    ...
    if ((mypolls.fds[n].events & POLLOUT) &&
        ((mypolls.fds[n].revents & POLLERR) ||
         (mypolls.fds[n].revents & POLLHUP)))
    {
  ...
  if (myds->active == 0)
  {
    ...
    {
      proxy_debug(PROXY_DEBUG_NET, 1,
                  "Session=%p, DataStream=%p -- Deleting FD %d\n", myds->sess,
                  myds, myds->fd);
      myds->sess->set_unhealthy();
    }
    else
...
void MySQL_Thread::process_all_sessions()
{
    ...
    for (n= 0; n < mysql_sessions->len; n++)
    {
      ...
      if (sess->mybe && sess->mybe->server_myds)
      {
        if (sess->mybe->server_myds->max_connect_time)
        {
          ...
          if (sess2->mybe && sess2->mybe->server_myds &&
              sess2->mybe->server_myds->max_connect_time &&
              sess2->mybe->server_myds->max_connect_time <=
  ...
  for (n= 0; n < mysql_sessions->len; n++)
  {
...
#endif
    if (sess->mirror == true)
    {  // this is a mirror session
      if (sess->status == WAITING_CLIENT_DATA)
      {  // the mirror session has completed
    ...
    if (maintenance_loop)
    {
...
#endif  // IDLE_THREADS
      {
        sess->to_process= 1;
        if ((sess_time / 1000 >
             (unsigned long long)mysql_thread___max_transaction_time) ||
            (sess_time / 1000 >
             (unsigned long long)mysql_thread___wait_timeout))
        {
        ...
        if (servers_table_version_current != servers_table_version_previous)
        {  // bug fix for #1085
  ...
  total_active_transactions_tmp=
      __sync_add_and_fetch(&status_variables.active_transactions, 0);
  __sync_bool_compare_and_swap(&status_variables.active_transactions,
                               total_active_transactions_tmp,
                               total_active_transactions_);
}
...
void MySQL_Thread::refresh_variables()
{
  ...
  mysql_thread___max_allowed_packet=
      GloMTH->get_variable_int((char *)"max_allowed_packet");
  mysql_thread___throttle_connections_per_sec_to_hostgroup=
      GloMTH->get_variable_int(
          (char *)"throttle_connections_per_sec_to_hostgroup");
  mysql_thread___max_transaction_time=
      GloMTH->get_variable_int((char *)"max_transaction_time");
  mysql_thread___threshold_query_length=
      GloMTH->get_variable_int((char *)"threshold_query_length");
  ...
  mysql_thread___query_digests_max_digest_length=
      GloMTH->get_variable_int((char *)"query_digests_max_digest_length");
  mysql_thread___query_digests_max_query_length=
      GloMTH->get_variable_int((char *)"query_digests_max_query_length");
  mysql_thread___wait_timeout=
      GloMTH->get_variable_int((char *)"wait_timeout");
  mysql_thread___throttle_max_bytes_per_second_to_client=
      GloMTH->get_variable_int(
          (char *)"throttle_max_bytes_per_second_to_client");
  mysql_thread___throttle_ratio_server_to_client=
      GloMTH->get_variable_int((char *)"throttle_ratio_server_to_client");
  mysql_thread___max_connections=
      GloMTH->get_variable_int((char *)"max_connections");
  mysql_thread___max_stmts_per_connection=
      GloMTH->get_variable_int((char *)"max_stmts_per_connection");
  mysql_thread___max_stmts_cache=
      GloMTH->get_variable_int((char *)"max_stmts_cache");
  mysql_thread___mirror_max_concurrency=
      GloMTH->get_variable_int((char *)"mirror_max_concurrency");
  mysql_thread___mirror_max_queue_length=
      GloMTH->get_variable_int((char *)"mirror_max_queue_length");
  mysql_thread___default_query_delay=
      GloMTH->get_variable_int((char *)"default_query_delay");
  mysql_thread___default_query_timeout=
      GloMTH->get_variable_int((char *)"default_query_timeout");
  mysql_thread___query_processor_iterations=
      GloMTH->get_variable_int((char *)"query_processor_iterations");
  mysql_thread___query_processor_regex=
      GloMTH->get_variable_int((char *)"query_processor_regex");
  mysql_thread___auto_increment_delay_multiplex=
      GloMTH->get_variable_int((char *)"auto_increment_delay_multiplex");
  mysql_thread___default_max_latency_ms=
      GloMTH->get_variable_int((char *)"default_max_latency_ms");
  mysql_thread___long_query_time=
      GloMTH->get_variable_int((char *)"long_query_time");
  mysql_thread___query_cache_size_MB=
      GloMTH->get_variable_int((char *)"query_cache_size_MB");
  mysql_thread___ping_interval_server_msec=
      GloMTH->get_variable_int((char *)"ping_interval_server_msec");
  mysql_thread___ping_timeout_server=
      GloMTH->get_variable_int((char *)"ping_timeout_server");
  mysql_thread___shun_on_failures=
      GloMTH->get_variable_int((char *)"shun_on_failures");
  mysql_thread___shun_recovery_time_sec=
      GloMTH->get_variable_int((char *)"shun_recovery_time_sec");
  mysql_thread___query_retries_on_failure=
      GloMTH->get_variable_int((char *)"query_retries_on_failure");
  mysql_thread___connect_retries_on_failure=
      GloMTH->get_variable_int((char *)"connect_retries_on_failure");
  mysql_thread___client_multi_statements=
      (bool)GloMTH->get_variable_int((char *)"client_multi_statements");
  mysql_thread___connection_delay_multiplex_ms=
      GloMTH->get_variable_int((char *)"connection_delay_multiplex_ms");
  mysql_thread___connection_max_age_ms=
      GloMTH->get_variable_int((char *)"connection_max_age_ms");
  mysql_thread___connect_timeout_server=
      GloMTH->get_variable_int((char *)"connect_timeout_server");
  mysql_thread___connect_timeout_server_max=
      GloMTH->get_variable_int((char *)"connect_timeout_server_max");
  mysql_thread___free_connections_pct=
      GloMTH->get_variable_int((char *)"free_connections_pct");
#ifdef IDLE_THREADS
  mysql_thread___session_idle_ms=
      GloMTH->get_variable_int((char *)"session_idle_ms");
#endif  // IDLE_THREADS
  mysql_thread___connect_retries_delay=
      GloMTH->get_variable_int((char *)"connect_retries_delay");

  if (mysql_thread___monitor_username)
    free(mysql_thread___monitor_username);
  mysql_thread___monitor_username=
      GloMTH->get_variable_string((char *)"monitor_username");
  if (mysql_thread___monitor_password)
    free(mysql_thread___monitor_password);
  mysql_thread___monitor_password=
      GloMTH->get_variable_string((char *)"monitor_password");
  if (mysql_thread___monitor_replication_lag_use_percona_heartbeat)
    free(mysql_thread___monitor_replication_lag_use_percona_heartbeat);
  mysql_thread___monitor_replication_lag_use_percona_heartbeat=
      GloMTH->get_variable_string(
          (char *)"monitor_replication_lag_use_percona_heartbeat");
  ...
  mysql_thread___ssl_p2s_ca=
      GloMTH->get_variable_string((char *)"ssl_p2s_ca");
  if (mysql_thread___ssl_p2s_cert)
    free(mysql_thread___ssl_p2s_cert);
  mysql_thread___ssl_p2s_cert=
      GloMTH->get_variable_string((char *)"ssl_p2s_cert");
  if (mysql_thread___ssl_p2s_key)
    free(mysql_thread___ssl_p2s_key);
  mysql_thread___ssl_p2s_key=
      GloMTH->get_variable_string((char *)"ssl_p2s_key");
  if (mysql_thread___ssl_p2s_cipher)
    free(mysql_thread___ssl_p2s_cipher);
  mysql_thread___ssl_p2s_cipher=
      GloMTH->get_variable_string((char *)"ssl_p2s_cipher");

  mysql_thread___monitor_wait_timeout=
      (bool)GloMTH->get_variable_int((char *)"monitor_wait_timeout");
  mysql_thread___monitor_writer_is_also_reader=
      (bool)GloMTH->get_variable_int((char *)"monitor_writer_is_also_reader");
  mysql_thread___monitor_enabled=
      (bool)GloMTH->get_variable_int((char *)"monitor_enabled");
  mysql_thread___monitor_history=
      GloMTH->get_variable_int((char *)"monitor_history");
  mysql_thread___monitor_connect_interval=
      GloMTH->get_variable_int((char *)"monitor_connect_interval");
  mysql_thread___monitor_connect_timeout=
      GloMTH->get_variable_int((char *)"monitor_connect_timeout");
  mysql_thread___monitor_ping_interval=
      GloMTH->get_variable_int((char *)"monitor_ping_interval");
  mysql_thread___monitor_ping_max_failures=
      GloMTH->get_variable_int((char *)"monitor_ping_max_failures");
  mysql_thread___monitor_ping_timeout=
      GloMTH->get_variable_int((char *)"monitor_ping_timeout");
  ...
  mysql_thread___monitor_read_only_timeout=
      GloMTH->get_variable_int((char *)"monitor_read_only_timeout");
  mysql_thread___monitor_read_only_max_timeout_count=
      GloMTH->get_variable_int((char *)"monitor_read_only_max_timeout_count");
  mysql_thread___monitor_replication_lag_interval=
      GloMTH->get_variable_int((char *)"monitor_replication_lag_interval");
  mysql_thread___monitor_replication_lag_timeout=
      GloMTH->get_variable_int((char *)"monitor_replication_lag_timeout");
  mysql_thread___monitor_groupreplication_healthcheck_interval=
      GloMTH->get_variable_int(
          (char *)"monitor_groupreplication_healthcheck_interval");
  ...
  mysql_thread___monitor_query_timeout=
      GloMTH->get_variable_int((char *)"monitor_query_timeout");
  mysql_thread___monitor_slave_lag_when_null=
      GloMTH->get_variable_int((char *)"monitor_slave_lag_when_null");

  if (mysql_thread___init_connect)
    free(mysql_thread___init_connect);
  mysql_thread___init_connect=
      GloMTH->get_variable_string((char *)"init_connect");
  if (mysql_thread___default_sql_mode)
    free(mysql_thread___default_sql_mode);
  mysql_thread___default_sql_mode=
      GloMTH->get_variable_string((char *)"default_sql_mode");
  if (mysql_thread___default_time_zone)
    free(mysql_thread___default_time_zone);
  ...
  mysql_thread___server_version=
      GloMTH->get_variable_string((char *)"server_version");
  if (mysql_thread___eventslog_filename)
    free(mysql_thread___eventslog_filename);
  mysql_thread___eventslog_filesize=
      GloMTH->get_variable_int((char *)"eventslog_filesize");
  mysql_thread___eventslog_filename=
      GloMTH->get_variable_string((char *)"eventslog_filename");
  GloMyLogger
      ->set_base_filename();  // both filename and filesize are set here
  if (mysql_thread___default_schema)
    free(mysql_thread___default_schema);
  mysql_thread___default_schema=
      GloMTH->get_variable_string((char *)"default_schema");
  mysql_thread___server_capabilities=
      GloMTH->get_variable_uint16((char *)"server_capabilities");
  mysql_thread___default_charset=
      GloMTH->get_variable_uint8((char *)"default_charset");
  mysql_thread___poll_timeout=
      GloMTH->get_variable_int((char *)"poll_timeout");
  mysql_thread___poll_timeout_on_failure=
      GloMTH->get_variable_int((char *)"poll_timeout_on_failure");
  mysql_thread___have_compress=
      (bool)GloMTH->get_variable_int((char *)"have_compress");
  mysql_thread___client_found_rows=
      (bool)GloMTH->get_variable_int((char *)"client_found_rows");
  mysql_thread___multiplexing=
      (bool)GloMTH->get_variable_int((char *)"multiplexing");
  mysql_thread___forward_autocommit=
      (bool)GloMTH->get_variable_int((char *)"forward_autocommit");
  mysql_thread___enforce_autocommit_on_reads=
      (bool)GloMTH->get_variable_int((char *)"enforce_autocommit_on_reads");
  mysql_thread___autocommit_false_not_reusable=
      (bool)GloMTH->get_variable_int((char *)"autocommit_false_not_reusable");
  mysql_thread___autocommit_false_is_transaction=
      (bool)GloMTH->get_variable_int(
          (char *)"autocommit_false_is_transaction");
  mysql_thread___verbose_query_error=
      (bool)GloMTH->get_variable_int((char *)"verbose_query_error");
  mysql_thread___commands_stats=
      (bool)GloMTH->get_variable_int((char *)"commands_stats");
  mysql_thread___query_digests=
      (bool)GloMTH->get_variable_int((char *)"query_digests");
  mysql_thread___query_digests_lowercase=
      (bool)GloMTH->get_variable_int((char *)"query_digests_lowercase");
  variables.stats_time_backend_query=
      (bool)GloMTH->get_variable_int((char *)"stats_time_backend_query");
  variables.stats_time_query_processor=
      (bool)GloMTH->get_variable_int((char *)"stats_time_query_processor");
  mysql_thread___hostgroup_manager_verbose=
      GloMTH->get_variable_int((char *)"hostgroup_manager_verbose");
  mysql_thread___kill_backend_connection_when_disconnect=
      (bool)GloMTH->get_variable_int(
          (char *)"kill_backend_connection_when_disconnect");
  mysql_thread___sessions_sort=
      (bool)GloMTH->get_variable_int((char *)"sessions_sort");
#ifdef IDLE_THREADS
  mysql_thread___session_idle_show_processlist=
      (bool)GloMTH->get_variable_int((char *)"session_idle_show_processlist");
#endif  // IDLE_THREADS
  mysql_thread___servers_stats=
      (bool)GloMTH->get_variable_int((char *)"servers_stats");
  mysql_thread___default_reconnect=
      (bool)GloMTH->get_variable_int((char *)"default_reconnect");
#ifdef DEBUG
  mysql_thread___session_debug=
      (bool)GloMTH->get_variable_int((char *)"session_debug");
#endif /* DEBUG */
  GloMTH->wrunlock();
}
...
void MySQL_Thread::listener_handle_new_connection(MySQL_Data_Stream *myds,
                                                  unsigned int       n)
{
  int c;
  union {
...
#ifdef SO_REUSEPORT
    if (GloVars.global.reuseport == false)
    {  // only if reuseport is not enabled
       //usleep(10+rand()%50);
    }
#else
    //usleep(10+rand()%50);
  ...
  {  // accept() succeeded
    ...
    if (__sync_add_and_fetch(&MyHGM->status.client_connections, 1) >
        mysql_thread___max_connections)
    {
    ...
    switch (sess->client_myds->client_addr->sa_family)
    {
      case AF_INET:
      {
        struct sockaddr_in *ipv4=
            (struct sockaddr_in *)sess->client_myds->client_addr;
        char buf[INET_ADDRSTRLEN];
        inet_ntop(sess->client_myds->client_addr->sa_family, &ipv4->sin_addr,
                  buf, INET_ADDRSTRLEN);
      ...
      case AF_INET6:
      {
        struct sockaddr_in6 *ipv6=
            (struct sockaddr_in6 *)sess->client_myds->client_addr;
        char buf[INET6_ADDRSTRLEN];
        inet_ntop(sess->client_myds->client_addr->sa_family, &ipv6->sin6_addr,
                  buf, INET6_ADDRSTRLEN);
        sess->client_myds->addr.addr= strdup(buf);
        sess->client_myds->addr.port= htons(ipv6->sin6_port);
        break;
      }
      default: sess->client_myds->addr.addr= strdup("localhost"); break;
    }

    iface_info *ifi= NULL;
    ifi= GloMTH->MLM_find_iface_from_fd(
        myds->fd);  // here we try to get the info about the proxy bind address
    if (ifi)
    {
    ...
    mypolls.add(POLLIN | POLLOUT, sess->client_myds->fd, sess->client_myds,
                curtime);
...
SQLite3_result *MySQL_Threads_Handler::SQL3_GlobalStatus(bool _memory)
{
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 4,
              "Dumping MySQL Global Status\n");
  ...
  if (GloMyMon)
  {
    {  // MySQL Monitor workers
      ...
      sprintf(buf, "%d",
              (variables.monitor_enabled ? GloMyMon->num_threads : 0));
      pta[1]= buf;
      result->add_row(pta);
    }
  }
    ...
    for (j= 0; j < thr->mysql_sessions->len; j++)
    {
      ...
      if (sess->client_myds)
      {
        ...
        if (sess->mirror == false)
        {
          switch (sess->client_myds->client_addr->sa_family)
          {
            case AF_INET:
            {
              struct sockaddr_in *ipv4=
                  (struct sockaddr_in *)sess->client_myds->client_addr;
              inet_ntop(sess->client_myds->client_addr->sa_family,
                        &ipv4->sin_addr, buf, INET_ADDRSTRLEN);
              pta[4]= strdup(buf);
            ...
            case AF_INET6:
            {
              struct sockaddr_in6 *ipv6=
                  (struct sockaddr_in6 *)sess->client_myds->client_addr;
              inet_ntop(sess->client_myds->client_addr->sa_family,
                        &ipv6->sin6_addr, buf, INET6_ADDRSTRLEN);
              pta[4]= strdup(buf);
            ...
            sess->mybe->server_myds->myconn)
        {
          ...
          if (rc == 0)
          {
            switch (addr.sa_family)
            {
              case AF_INET:
              {
                ...
                inet_ntop(addr.sa_family, &ipv4->sin_addr, buf,
                          INET_ADDRSTRLEN);
                pta[7]= strdup(buf);
                sprintf(port, "%d", ntohs(ipv4->sin_port));
                pta[8]= strdup(port);
              ...
              case AF_INET6:
              {
                ...
                inet_ntop(addr.sa_family, &ipv6->sin6_addr, buf,
                          INET6_ADDRSTRLEN);
                pta[7]= strdup(buf);
                sprintf(port, "%d", ntohs(ipv6->sin6_port));
                pta[8]= strdup(port);
          ...
          pta[10]= strdup(buf);
          if (sess->CurrentQuery.stmt_info == NULL)
          {  // text protocol
            if (mc->query.length)
            {
        ...
        switch (sess->status)
        {
          case CONNECTING_SERVER: pta[11]= strdup("Connect"); break;
          case PROCESSING_QUERY:
            ...
            break;
          case WAITING_CLIENT_DATA: pta[11]= strdup("Sleep"); break;
          case CHANGING_USER_SERVER: pta[11]= strdup("Change user"); break;
          case CHANGING_USER_CLIENT:
            pta[11]= strdup("Change user client");
            break;
          case CHANGING_SCHEMA: pta[11]= strdup("InitDB"); break;
          case PROCESSING_STMT_EXECUTE: pta[11]= strdup("Execute"); break;
          case PROCESSING_STMT_PREPARE: pta[11]= strdup("Prepare"); break;
          case CONNECTING_CLIENT: pta[11]= strdup("Connecting client"); break;
          case PINGING_SERVER: pta[11]= strdup("Pinging server"); break;
          case WAITING_SERVER_DATA:
            pta[11]= strdup("Waiting server data");
            break;
          case CHANGING_CHARSET: pta[11]= strdup("Changing charset"); break;
          ...
          case SETTING_INIT_CONNECT:
            ...
            break;
          case SETTING_SQL_LOG_BIN: pta[11]= strdup("Set log bin"); break;
          case SETTING_SQL_MODE: pta[11]= strdup("Set SQL mode"); break;
          case SETTING_TIME_ZONE: pta[11]= strdup("Set TZ"); break;
          case FAST_FORWARD: pta[11]= strdup("Fast forward"); break;
          case NONE: pta[11]= strdup("None"); break;
          default:
        ...
        else
        {
          ...
          sprintf(buf, "%llu",
                  (sess->thread->curtime - sess->start_time) / 1000);
        }
        pta[12]= strdup(buf);
...
void MySQL_Threads_Handler::kill_connection_or_query(
    uint32_t _thread_session_id, bool query, char *username)
{
  unsigned int i;
  for (i= 0; i < num_threads; i++)
  {
...
unsigned long long MySQL_Threads_Handler::get_total_backend_stmt_prepare()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.backend_stmt_prepare,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_total_backend_stmt_execute()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.backend_stmt_execute,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_total_backend_stmt_close()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.backend_stmt_close,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_total_frontend_stmt_prepare()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.frontend_stmt_prepare,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_total_frontend_stmt_execute()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.frontend_stmt_execute,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_total_frontend_stmt_close()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.frontend_stmt_close,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_queries_backends_bytes_recv()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.queries_backends_bytes_recv, 0);
    }
  }
  return q;
}
...
unsigned long long MySQL_Threads_Handler::get_queries_backends_bytes_sent()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.queries_backends_bytes_sent, 0);
    }
  }
  return q;
}
...
unsigned long long MySQL_Threads_Handler::get_queries_frontends_bytes_recv()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.queries_frontends_bytes_recv, 0);
    }
  }
  return q;
}
...
unsigned long long MySQL_Threads_Handler::get_queries_frontends_bytes_sent()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.queries_frontends_bytes_sent, 0);
    }
  }
  return q;
}
...
unsigned int MySQL_Threads_Handler::get_active_transations()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.active_transactions,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_query_processor_time()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.query_processor_time,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_backend_query_time()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      MySQL_Thread *thr= (MySQL_Thread *)mysql_threads[i].worker;
      if (thr)
        q+= __sync_fetch_and_add(&thr->status_variables.backend_query_time,
                                 0);
    }
  }
  return q;
...
unsigned long long MySQL_Threads_Handler::get_mysql_backend_buffers_bytes()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.mysql_backend_buffers_bytes, 0);
    }
  }
  return q;
}
...
unsigned long long MySQL_Threads_Handler::get_mysql_frontend_buffers_bytes()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.mysql_frontend_buffers_bytes, 0);
    }
  }
#ifdef IDLE_THREADS
  if (GloVars.global.idle_threads)
...
unsigned long long MySQL_Threads_Handler::get_mysql_session_internal_bytes()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.mysql_session_internal_bytes, 0);
    }
#ifdef IDLE_THREADS
    if (GloVars.global.idle_threads)
      if (mysql_threads_idles)
...
void MySQL_Thread::Get_Memory_Stats()
{
  ...
  if (mysql_sessions)
  {
    status_variables.mysql_session_internal_bytes+=
        (mysql_sessions->size) * sizeof(MySQL_Session *);
    ...
    {
      status_variables.mysql_frontend_buffers_bytes+=
          (mysql_sessions->len * QUEUE_T_DEFAULT_SIZE * 2);
      status_variables.mysql_session_internal_bytes+=
          (mysql_sessions->len * sizeof(MySQL_Connection));
#if !defined(__FreeBSD__) && !defined(__APPLE__)
      status_variables.mysql_session_internal_bytes+=
          ((sizeof(int) + sizeof(int) + sizeof(std::_Rb_tree_node_base)) *
           mysql_sessions->len);
#else
      status_variables.mysql_session_internal_bytes+=
          ((sizeof(int) + sizeof(int) + 32) * mysql_sessions->len);
#endif
    }
  }
}
                                                 ...
                                                 MySQL_Session *sess)
{
  ...
  for (i= 0; i < cached_connections->len; i++)
  {
    ...
    if (c->parent->myhgc->hid == _hid)
    {
      ...
      npc++;
      if (sess && sess->client_myds && sess->client_myds->myconn &&
  ...
  if (pcf)
  {  // there was a possible connection, but we skipped trying to find a better one
    if (npc > 5)
    {  // more candidates were evaluated
...
void MySQL_Thread::return_local_connections()
{
  ...
  MySQL_Connection **ca= (MySQL_Connection **)malloc(
      sizeof(MySQL_Connection *) * (cached_connections->len + 1));
  unsigned int i= 0;
  while (cached_connections->len)
  {
...
unsigned long long MySQL_Threads_Handler::get_ConnPool_get_conn_immediate()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.ConnPool_get_conn_immediate, 0);
    }
  }
  return q;
}
...
unsigned long long MySQL_Threads_Handler::get_ConnPool_get_conn_success()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.ConnPool_get_conn_success, 0);
    }
  }
  return q;
}
...
unsigned long long MySQL_Threads_Handler::get_ConnPool_get_conn_failure()
{
  ...
  for (i= 0; i < num_threads; i++)
  {
    if (mysql_threads)
    {
      ...
      if (thr)
        q+= __sync_fetch_and_add(
            &thr->status_variables.ConnPool_get_conn_failure, 0);
    }
  }
  return q;
}
...
#endif
  for (std::vector<thr_id_usr *>::iterator it= kq.conn_ids.begin();
       it != kq.conn_ids.end(); ++it)
  {
    thr_id_usr *t= *it;
    free(t->username);
    free(t);
  }
  for (std::vector<thr_id_usr *>::iterator it= kq.query_ids.begin();
       it != kq.query_ids.end(); ++it)
  {
    thr_id_usr *t= *it;
    free(t->username);
    free(t);
...
void MySQL_Thread::Scan_Sessions_to_Kill(PtrArray *mysess)
{
  for (unsigned int n= 0;
       n < mysess->len && (kq.conn_ids.size() + kq.query_ids.size()); n++)
  {
    MySQL_Session *_sess= (MySQL_Session *)mysess->index(n);
    bool           cont= true;
    for (std::vector<thr_id_usr *>::iterator it= kq.conn_ids.begin();
         cont && it != kq.conn_ids.end(); ++it)
    {
      thr_id_usr *t= *it;
      if (t->id == _sess->thread_session_id)
      {
        if (_sess->client_myds)
        {
          if (strcmp(t->username,
                     _sess->client_myds->myconn->userinfo->username) == 0)
          {
            _sess->killed= true;
          }
        }
