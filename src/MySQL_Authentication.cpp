...
bool MySQL_Authentication::add(char *username, char *password,
                               enum cred_username_type usertype, bool use_ssl,
                               int default_hostgroup, char *default_schema,
                               bool schema_locked,
...
unsigned int MySQL_Authentication::memory_usage()
{
  ...
  for (i= 0; i < creds_frontends.cred_array->len; i++)
  {
    ...
    if (ado->username)
      ret+= strlen(ado->username) + 1;
    if (ado->password)
      ret+= strlen(ado->password) + 1;
    if (ado->sha1_pass)
      ret+= SHA_DIGEST_LENGTH;
    if (ado->default_schema)
      ret+= strlen(ado->default_schema) + 1;
  }
...
bool MySQL_Authentication::del(char *                  username,
...
bool MySQL_Authentication::set_SHA1(char *                  username,
                                    enum cred_username_type usertype,
                                    void *                  sha_pass)
{
  bool        ret= false;
...
char *MySQL_Authentication::lookup(char *                  username,
                                   enum cred_username_type usertype,
...
uint64_t MySQL_Authentication::_get_runtime_checksum(
    enum cred_username_type usertype)
{
  creds_group_t &cg=
      (usertype == USERNAME_BACKEND ? creds_backends : creds_frontends);
  ...
  for (it= cg.bt_map.begin(); it != cg.bt_map.end();)
  {
    ...
    if (ad->default_hostgroup >= 0)
    {
      ...
      myhash.Update(&ad->transaction_persistent,
