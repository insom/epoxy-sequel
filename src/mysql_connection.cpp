...
static int mysql_status(short event, short cont)
{
  int status= 0;
  if (event & POLLIN)
...
static char *session_vars[]= {
    ...
    (char *)"SQL_SAFE_UPDATES",      (char *)"SQL_SELECT_LIMIT",
    (char *)"MAX_JOIN_SIZE",         (char *)"FOREIGN_KEY_CHECKS",
    (char *)"UNIQUE_CHECKS",         (char *)"AUTO_INCREMENT_INCREMENT",
    (char *)"AUTO_INCREMENT_OFFSET", (char *)"GROUP_CONCAT_MAX_LEN"};
...
void MySQL_Connection::compute_unknown_transaction_status()
{
  if (mysql)
  {
    ...
    if (_myerrno >= 2000 && _myerrno < 3000)
    {  // client error
      // do not change it
      return;
    }
    if (_myerrno >= 1000 && _myerrno < 2000)
    {  // server error
      unknown_transaction_status= true;
      return;
    }
    if (_myerrno >= 3000 && _myerrno < 4000)
    {  // server error
      unknown_transaction_status= true;
      return;
    }
...
MySQL_Connection::MySQL_Connection()
{
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4,
              "Creating new MySQL_Connection %p\n", this);
  local_stmts=
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4,
              "Destroying MySQL_Connection %p\n", this);
  if (options.server_version)
    free(options.server_version);
...
void MySQL_Connection::connect_start()
{
  ...
  const CHARSET_INFO *c=
      proxysql_find_charset_nr(mysql_thread___default_charset);
  if (!c)
  {
  ...
  if (parent->port)
  {
    async_exit_status= mysql_real_connect_start(
        &ret_mysql, mysql, parent->address, userinfo->username, auth_password,
        userinfo->schemaname, parent->port, NULL, client_flags);
  }
  else
  {
    async_exit_status= mysql_real_connect_start(
        &ret_mysql, mysql, "localhost", userinfo->username, auth_password,
        userinfo->schemaname, parent->port, parent->address, client_flags);
  }
...
void MySQL_Connection::connect_cont(short event)
{
  ...
  async_exit_status=
      mysql_real_connect_cont(&ret_mysql, mysql, mysql_status(event, true));
}
...
void MySQL_Connection::change_user_start()
{
  ...
  async_exit_status= mysql_change_user_start(&ret_bool, mysql, _ui->username,
                                             auth_password, _ui->schemaname);
}
...
void MySQL_Connection::change_user_cont(short event)
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status=
      mysql_change_user_cont(&ret_bool, mysql, mysql_status(event, true));
}
...
void MySQL_Connection::ping_cont(short event)
{
  ...
  async_exit_status=
      mysql_ping_cont(&interr, mysql, mysql_status(event, true));
}

void MySQL_Connection::initdb_start()
{
  PROXY_TRACE();
  MySQL_Connection_userinfo *client_ui=
      myds->sess->client_myds->myconn->userinfo;
  async_exit_status=
      mysql_select_db_start(&interr, mysql, client_ui->schemaname);
}
...
void MySQL_Connection::initdb_cont(short event)
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status=
      mysql_select_db_cont(&interr, mysql, mysql_status(event, true));
}
...
void MySQL_Connection::set_autocommit_start()
{
  PROXY_TRACE();
  async_exit_status=
      mysql_autocommit_start(&ret_bool, mysql, options.autocommit);
}
...
void MySQL_Connection::set_autocommit_cont(short event)
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status=
      mysql_autocommit_cont(&ret_bool, mysql, mysql_status(event, true));
}
...
void MySQL_Connection::set_names_cont(short event)
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status=
      mysql_set_character_set_cont(&interr, mysql, mysql_status(event, true));
}
...
void MySQL_Connection::real_query_start()
{
  PROXY_TRACE();
  async_exit_status=
      mysql_real_query_start(&interr, mysql, query.ptr, query.length);
}
...
void MySQL_Connection::real_query_cont(short event)
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status=
      mysql_real_query_cont(&interr, mysql, mysql_status(event, true));
}
...
void MySQL_Connection::stmt_prepare_start()
{
  ...
  async_exit_status=
      mysql_stmt_prepare_start(&interr, query.stmt, query.ptr, query.length);
}
...
void MySQL_Connection::stmt_prepare_cont(short event)
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status=
      mysql_stmt_prepare_cont(&interr, query.stmt, mysql_status(event, true));
}
...
void MySQL_Connection::stmt_execute_start()
{
  ...
  _rc= mysql_stmt_bind_param(
      query.stmt, query.stmt_meta->binds);  // FIXME : add error handling
  if (_rc)
  {
    proxy_error("mysql_stmt_bind_param() failed: %s",
...
void MySQL_Connection::stmt_execute_cont(short event)
{
  ...
  async_exit_status=
      mysql_stmt_execute_cont(&interr, query.stmt, mysql_status(event, true));
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status= mysql_stmt_store_result_cont(&interr, query.stmt,
                                                  mysql_status(event, true));
}
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status= mysql_store_result_cont(&mysql_result, mysql,
                                             mysql_status(event, true));
}
...
MDB_ASYNC_ST MySQL_Connection::handler(short event)
{
  unsigned long long processed_bytes=
      0;  // issue #527 : this variable will store the amount of bytes processed during this event
  if (mysql == NULL)
  {
    // it is the first time handler() is being called
    async_state_machine= ASYNC_CONNECT_START;
    myds->wait_until= myds->sess->thread->curtime +
                      mysql_thread___connect_timeout_server * 1000;
  ...
  switch (async_state_machine)
  {
    ...
    case ASYNC_CONNECT_END:
      if (!ret_mysql)
      {
        ...
        proxy_error(
            "Failed to mysql_real_connect() on %s:%d , FD (Conn:%d , "
            "MyDS:%d) , %d: %s.\n",
            parent->address, parent->port, mysql->net.fd, myds->fd,
    ...
    case ASYNC_CHANGE_USER_SUCCESSFUL:
      mysql->server_status=
          SERVER_STATUS_AUTOCOMMIT;  // we reset this due to bug https://jira.mariadb.org/browse/CONC-332
      break;
    case ASYNC_CHANGE_USER_FAILED: break;
    ...
    case ASYNC_PING_END:
      ...
      break;
    case ASYNC_PING_SUCCESSFUL: break;
    case ASYNC_PING_FAILED: break;
    case ASYNC_PING_TIMEOUT: break;
    case ASYNC_QUERY_START:
      real_query_start();
      __sync_fetch_and_add(&parent->queries_sent, 1);
      __sync_fetch_and_add(&parent->bytes_sent, query.length);
      myds->sess->thread->status_variables.queries_backends_bytes_sent+=
          query.length;
      if (async_exit_status)
      {
    ...
    case ASYNC_STMT_PREPARE_END:
      ...
      break;
    case ASYNC_STMT_PREPARE_SUCCESSFUL: break;
    case ASYNC_STMT_PREPARE_FAILED: break;
    ...
    case ASYNC_STMT_EXECUTE_END:
    {
      if (query.stmt_result)
      {
        ...
        myds->sess->thread->status_variables.queries_backends_bytes_recv+=
            total_size;
      }
    }
    /*
...
*/
    ...
    case ASYNC_NEXT_RESULT_CONT:
      async_exit_status=
          mysql_next_result_cont(&interr, mysql, mysql_status(event, true));
      if (async_exit_status)
      {
        next_event(ASYNC_NEXT_RESULT_CONT);
      }
    ...
    case ASYNC_NEXT_RESULT_END: break;
    ...
    case ASYNC_USE_RESULT_START:
      ...
      else
      {
        ...
        {
          MyRS= new MySQL_ResultSet(&myds->sess->client_myds->myprot,
                                    mysql_result, mysql);
        }
        else
        {
    ...
    case ASYNC_USE_RESULT_CONT:
    {
          ...
          myds->sess->mirror == false)
      {
        ...
        if (buffered_data >
            (unsigned int)mysql_thread___threshold_resultset_size * 8)
        {
          next_event(
              ASYNC_USE_RESULT_CONT);  // we temporarily pause . See #1232
          break;
        }
      }
    }
      ...
      {
        async_exit_status= mysql_fetch_row_cont(&mysql_row, mysql_result,
                                                mysql_status(event, true));
      }
      if (async_exit_status)
      {
      ...
      else
      {
        ...
        if (mysql_row)
        {
          ...
          myds->sess->thread->status_variables.queries_backends_bytes_recv+=
              br;
          processed_bytes+=
              br;  // issue #527 : this variable will store the amount of bytes processed during this event
          if ((processed_bytes >
               (unsigned int)mysql_thread___threshold_resultset_size * 8) ||
    ...
    case ASYNC_SET_AUTOCOMMIT_SUCCESSFUL:
      options.last_set_autocommit=
          (options.autocommit ? 1 : 0);  // we successfully set autocommit
      if ((mysql->server_status & SERVER_STATUS_AUTOCOMMIT) &&
          options.autocommit == false)
      {
        proxy_warning(
            "It seems we are hitting bug "
            "http://bugs.mysql.com/bug.php?id=66884\n");
      }
      break;
    ...
    case ASYNC_SET_NAMES_END:
      ...
      break;
    case ASYNC_SET_NAMES_SUCCESSFUL: break;
    ...
    case ASYNC_INITDB_END:
      ...
      break;
    case ASYNC_INITDB_SUCCESSFUL: break;
...
void MySQL_Connection::next_event(MDB_ASYNC_ST new_st)
{
  ...
  //event_add(ev_mysql, ptv);
  proxy_debug(PROXY_DEBUG_NET, 8,
              "fd=%d, wait_events=%d , old_ST=%d, new_ST=%d\n", fd,
              wait_events, async_state_machine, new_st);
  async_state_machine= new_st;
};
...
int MySQL_Connection::async_connect(short event)
{
  ...
  switch (async_state_machine)
  {
    case ASYNC_CONNECT_SUCCESSFUL:
      ...
      return 0;
      break;
    case ASYNC_CONNECT_FAILED: return -1; break;
    case ASYNC_CONNECT_TIMEOUT: return -2; break;
    default: return 1;
  }
  return 1;
}
...
bool MySQL_Connection::IsServerOffline()
{
  ...
  server_status=
      parent
          ->status;  // we copy it here to avoid race condition. The caller will see this
  if ((server_status ==
       MYSQL_SERVER_STATUS_OFFLINE_HARD)  // the server is OFFLINE as specific by the user
      || (server_status == MYSQL_SERVER_STATUS_SHUNNED &&
          parent->shunned_automatic == true &&
          parent->shunned_and_kill_all_connections ==
              true)  // the server is SHUNNED due to a serious issue
      ||
      (server_status ==
...
int MySQL_Connection::async_query(short event, char *stmt,
                                  unsigned long length, MYSQL_STMT **_stmt,
...
int MySQL_Connection::async_ping(short event)
{
  ...
  switch (async_state_machine)
  {
    case ASYNC_PING_SUCCESSFUL:
      ...
      return 0;
      break;
    case ASYNC_PING_FAILED: return -1; break;
    case ASYNC_PING_TIMEOUT: return -2; break;
    case ASYNC_IDLE: async_state_machine= ASYNC_PING_START;
    default: handler(event); break;
  }
...
int MySQL_Connection::async_change_user(short event)
{
  ...
  switch (async_state_machine)
  {
    case ASYNC_CHANGE_USER_SUCCESSFUL:
      ...
      return 0;
      break;
    case ASYNC_CHANGE_USER_FAILED: return -1; break;
    case ASYNC_IDLE: async_state_machine= ASYNC_CHANGE_USER_START;
    default: handler(event); break;
  }
...
int MySQL_Connection::async_select_db(short event)
{
  ...
  switch (async_state_machine)
  {
    case ASYNC_INITDB_SUCCESSFUL:
      ...
      return 0;
      break;
    case ASYNC_INITDB_FAILED: return -1; break;
    case ASYNC_IDLE: async_state_machine= ASYNC_INITDB_START;
    default: handler(event); break;
  }
...
int MySQL_Connection::async_set_autocommit(short event, bool ac)
{
  ...
  switch (async_state_machine)
  {
    case ASYNC_SET_AUTOCOMMIT_SUCCESSFUL:
      ...
      return 0;
      break;
    case ASYNC_SET_AUTOCOMMIT_FAILED: return -1; break;
    case ASYNC_IDLE:
      set_autocommit(ac);
...
int MySQL_Connection::async_set_names(short event, uint8_t c)
{
  ...
  switch (async_state_machine)
  {
    case ASYNC_SET_NAMES_SUCCESSFUL:
      ...
      return 0;
      break;
    case ASYNC_SET_NAMES_FAILED: return -1; break;
    case ASYNC_IDLE:
      set_charset(c);
...
bool MySQL_Connection::MultiplexDisabled()
{
  ...
  if (status_flags & (STATUS_MYSQL_CONNECTION_TRANSACTION |
                      ...
                      STATUS_MYSQL_CONNECTION_PREPARED_STATEMENT |
...
void MySQL_Connection::ProcessQueryAndSetStatusFlags(char *query_digest_text)
{
  ...
  if (get_status_user_variable() == false)
  {  // we search for variables only if not already set
    if (mul != 2 && index(query_digest_text, '@'))
    {  // mul = 2 has a special meaning : do not disable multiplex for variables in THIS QUERY ONLY
      if (strncasecmp(query_digest_text, "SELECT @@tx_isolation",
                      strlen("SELECT @@tx_isolation")) &&
          strncasecmp(query_digest_text, "SELECT @@version",
  ...
  if (get_status_prepared_statement() == false)
  {  // we search if prepared was already executed
    if (!strncasecmp(query_digest_text, "PREPARE ", strlen("PREPARE ")))
    {
      set_status_prepared_statement(true);
    }
  }
  if (get_status_temporary_table() == false)
  {  // we search for temporary if not already set
    if (!strncasecmp(query_digest_text, "CREATE TEMPORARY TABLE ",
                     strlen("CREATE TEMPORARY TABLE ")))
    {
      set_status_temporary_table(true);
    }
  }
  if (get_status_lock_tables() == false)
  {  // we search for lock tables only if not already set
  ...
  if (get_status_lock_tables() == false)
  {  // we search for lock tables only if not already set
    if (!strncasecmp(query_digest_text, "FLUSH TABLES WITH READ LOCK",
                     strlen("FLUSH TABLES WITH READ LOCK")))
    {  // issue 613
      set_status_lock_tables(true);
    }
  }
  ...
  {
    if (!strncasecmp(query_digest_text, "UNLOCK TABLES",
                     strlen("UNLOCK TABLES")))
    {
      set_status_lock_tables(false);
    }
  }
  if (get_status_get_lock() == false)
  {  // we search for get_lock if not already set
    if (strcasestr(query_digest_text, "GET_LOCK("))
    {
      set_status_get_lock(true);
    }
  }
  if (get_status_found_rows() == false)
  {  // we search for SQL_CALC_FOUND_ROWS if not already set
...
{
  if (mysql->net.max_packet > 65536)
  {  // FIXME: temporary, maybe for very long time . This needs to become a global variable
    if ((mysql->net.buff == mysql->net.read_pos) &&
        (mysql->net.read_pos == mysql->net.write_pos))
    {
      free(mysql->net.buff);
...
int MySQL_Connection::async_send_simple_command(short event, char *stmt,
                                                unsigned long length)
{
  ...
  if ((parent->status ==
       MYSQL_SERVER_STATUS_OFFLINE_HARD)  // the server is OFFLINE as specific by the user
      || (parent->status == MYSQL_SERVER_STATUS_SHUNNED &&
          parent->shunned_automatic == true &&
          parent->shunned_and_kill_all_connections ==
              true)  // the server is SHUNNED due to a serious issue
  )
  ...
  if (MyRS)
  {
    ...
    proxy_error(
        "Retrieved a resultset while running a simple command. This is an "
        "error!! Simple command: %s\n",
        stmt);
  }
  if (async_state_machine == ASYNC_QUERY_END)
  {
