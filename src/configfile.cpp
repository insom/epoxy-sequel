...
struct _global_configfile_entry_t {
  ...
  void (*func_pre)(global_configfile_entry_t
                       *);  // function called before initializing variable
  void (*func_post)(global_configfile_entry_t
                        *);  // function called after initializing variable
};

ProxySQL_ConfigFile::ProxySQL_ConfigFile() { filename= NULL; };
...
bool ProxySQL_ConfigFile::OpenFile(const char *__filename)
{
  ...
  if (FileUtils::isReadable(filename) == false)
    return false;
  try
  {
...
bool ProxySQL_ConfigFile::ReadGlobals() { return true; };

bool ProxySQL_ConfigFile::configVariable(const char *group, const char *key,
                                         int &variable, int defValue,
  ...
  if (root.exists(group) == true)
  {
    ...
    if (mygroup.isGroup() == true)
    {
      ...
      if (mygroup.exists(key) == true)
      {
        ...
        if ((myvar.getType() == Setting::Type::TypeInt) ||
            (myvar.getType() == Setting::Type::TypeInt64))
        {
          int var= myvar;
          if (var < minValue)
          {
            cerr << "[ERROR] Out of range value for " << group << "." << key
                 << " in config file\n";
            var= minValue;
          }
          if (var > maxValue)
        ...
        {
          cerr << "[ERROR] Incorrect datatype for " << group << "." << key
               << " in config file. Setting default\n";
...
bool ProxySQL_ConfigFile::configVariable(const char *group, const char *key,
                                         int64_t &variable, int64_t defValue,
