...
class QP_rule_text {
 public:
  ...
  QP_rule_text(QP_rule_t *QPr)
  {
    ...
    std::string re_mod;
    re_mod= "";
    if ((QPr->re_modifiers & QP_RE_MOD_CASELESS) == QP_RE_MOD_CASELESS)
      re_mod= "CASELESS";
...
static void __delete_query_rule(QP_rule_t *qr)
{
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
              "Deleting rule in %p : rule_id:%d, active:%d, username=%s, "
              "schemaname=%s, flagIN:%d, %smatch_pattern=\"%s\", flagOUT:%d "
              "replace_pattern=\"%s\", destination_hostgroup:%d, apply:%d\n",
              qr, qr->rule_id, qr->active, qr->username, qr->schemaname,
              qr->flagIN, (qr->negate_match_pattern ? "(!)" : ""),
              qr->match_pattern, qr->flagOUT, qr->replace_pattern,
              qr->destination_hostgroup, qr->apply);
  if (qr->username)
    free(qr->username);
  if (qr->schemaname)
    free(qr->schemaname);
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
              "Resetting rules in Query Processor Table %p\n", qrs);
  if (qrs == NULL)
    return;
  QP_rule_t *qr;
  for (std::vector<QP_rule_t *>::iterator it= qrs->begin(); it != qrs->end();
       ++it)
  {
    qr= *it;
...
Query_Processor::Query_Processor()
{
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
              "Initializing Query Processor with version=0\n");
  ...
  for (int i= 0; i < MYSQL_COM_QUERY___NONE; i++)
    commands_counters[i]= new Command_Counter(i);
  ...
  commands_counters_desc[MYSQL_COM_QUERY_ANALYZE_TABLE]=
      (char *)"ANALYZE_TABLE";
  commands_counters_desc[MYSQL_COM_QUERY_BEGIN]= (char *)"BEGIN";
  commands_counters_desc[MYSQL_COM_QUERY_CALL]= (char *)"CALL";
  commands_counters_desc[MYSQL_COM_QUERY_CHANGE_MASTER]=
      (char *)"CHANGE_MASTER";
  commands_counters_desc[MYSQL_COM_QUERY_COMMIT]= (char *)"COMMIT";
  commands_counters_desc[MYSQL_COM_QUERY_CREATE_DATABASE]=
      (char *)"CREATE_DATABASE";
  commands_counters_desc[MYSQL_COM_QUERY_CREATE_INDEX]=
      (char *)"CREATE_INDEX";
  commands_counters_desc[MYSQL_COM_QUERY_CREATE_TABLE]=
      (char *)"CREATE_TABLE";
  commands_counters_desc[MYSQL_COM_QUERY_CREATE_TEMPORARY]=
      (char *)"CREATE_TEMPORARY";
  commands_counters_desc[MYSQL_COM_QUERY_CREATE_TRIGGER]=
      (char *)"CREATE_TRIGGER";
  ...
  commands_counters_desc[MYSQL_COM_QUERY_DROP_DATABASE]=
      (char *)"DROP_DATABASE";
  ...
  commands_counters_desc[MYSQL_COM_QUERY_DROP_TRIGGER]=
      (char *)"DROP_TRIGGER";
  ...
  commands_counters_desc[MYSQL_COM_QUERY_RENAME_TABLE]=
      (char *)"RENAME_TABLE";
  commands_counters_desc[MYSQL_COM_QUERY_RESET_MASTER]=
      (char *)"RESET_MASTER";
  ...
  commands_counters_desc[MYSQL_COM_QUERY_SELECT_FOR_UPDATE]=
      (char *)"SELECT_FOR_UPDATE";
  commands_counters_desc[MYSQL_COM_QUERY_SET]= (char *)"SET";
  commands_counters_desc[MYSQL_COM_QUERY_SHOW_TABLE_STATUS]=
      (char *)"SHOW_TABLE_STATUS";
  commands_counters_desc[MYSQL_COM_QUERY_SHOW]= (char *)"SHOW";
  commands_counters_desc[MYSQL_COM_QUERY_START_TRANSACTION]=
      (char *)"START_TRANSACTION";
  commands_counters_desc[MYSQL_COM_QUERY_TRUNCATE_TABLE]=
      (char *)"TRUNCATE_TABLE";
  commands_counters_desc[MYSQL_COM_QUERY_UNLOCK_TABLES]=
      (char *)"UNLOCK_TABLES";
...
{
  for (int i= 0; i < MYSQL_COM_QUERY___NONE; i++)
    delete commands_counters[i];
  __reset_rules(&rules);
  rules_fast_routing.clear();
  for (std::unordered_map<uint64_t, void *>::iterator it= digest_umap.begin();
       it != digest_umap.end(); ++it)
  {
...
{
  proxy_debug(
      PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
      "Initializing Per-Thread Query Processor Table with version=0\n");
  _thr_SQP_version= 0;
  _thr_SQP_rules= new std::vector<QP_rule_t *>;
  _thr_SQP_rules_fast_routing= new std::unordered_map<std::string, int>;
  for (int i= 0; i < MYSQL_COM_QUERY___NONE; i++)
    _thr_commands_counters[i]= new Command_Counter(i);
};
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
              "Destroying Per-Thread Query Processor Table with version=%d\n",
              _thr_SQP_version);
  __reset_rules(_thr_SQP_rules);
  delete _thr_SQP_rules;
  delete _thr_SQP_rules_fast_routing;
  for (int i= 0; i < MYSQL_COM_QUERY___NONE; i++)
    delete _thr_commands_counters[i];
};
...
QP_rule_t *Query_Processor::new_query_rule(
    int rule_id, bool active, char *username, char *schemaname, int flagIN,
    char *client_addr, char *proxy_addr, int proxy_port, char *digest,
    char *match_digest, char *match_pattern, bool negate_match_pattern,
    char *re_modifiers, int flagOUT, char *replace_pattern,
    int destination_hostgroup, int cache_ttl, int reconnect, int timeout,
    int retries, int delay, int next_query_flagIN, int mirror_flagOUT,
    int mirror_hostgroup, char *error_msg, char *OK_msg, int sticky_conn,
    int multiplex, int log, bool apply, char *comment)
{
  ...
  newQR->re_modifiers= 0;
  {
    ...
    {
      if (strncasecmp(token, (char *)"CASELESS",
                      strlen((char *)"CASELESS")) == 0)
      {
  ...
  if (newQR->client_addr)
  {
    ...
    if (pct)
    {  // there is a wildcard . We assume Admin did already all the input validation
      ...
      else
      {
        ...
        newQR->client_addr_wildcard_position=
            strlen(newQR->client_addr) - strlen(pct);
      }
    }
  }
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
              "Creating new rule in %p : rule_id:%d, active:%d, username=%s, "
              "schemaname=%s, flagIN:%d, %smatch_digest=\"%s\", "
              "%smatch_pattern=\"%s\", flagOUT:%d replace_pattern=\"%s\", "
              "destination_hostgroup:%d, apply:%d\n",
              newQR, newQR->rule_id, newQR->active, newQR->username,
              newQR->schemaname, newQR->flagIN,
              (newQR->negate_match_pattern ? "(!)" : ""), newQR->match_digest,
              (newQR->negate_match_pattern ? "(!)" : ""),
              newQR->match_pattern, newQR->flagOUT, newQR->replace_pattern,
              newQR->destination_hostgroup, newQR->apply);
  return newQR;
};
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
              "Dumping query rules statistics, using Global version %d\n",
              version);
  ...
  for (std::vector<QP_rule_t *>::iterator it= rules.begin();
       it != rules.end(); ++it)
  {
    qr1= *it;
    if (qr1->active)
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
              "Dumping current query rules, using Global version %d\n",
              version);
...
SQLite3_result *Query_Processor::get_current_query_rules_fast_routing()
{
  proxy_debug(
      PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
      "Dumping current query rules fast_routing, using Global version %d\n",
      version);
...
*/
  for (std::vector<SQLite3_row *>::iterator it=
           fast_routing_resultset->rows.begin();
       it != fast_routing_resultset->rows.end(); ++it)
  {
    SQLite3_row *r= *it;
    result->add_row(r);
  }
...
unsigned long Query_Processor::get_query_digests_total_size()
{
  ...
  ret+=
      ((sizeof(uint64_t) + sizeof(void *) + sizeof(std::_Rb_tree_node_base)) *
       digest_umap.size());
#else
...
#define stackbuffer_size 128
  ...
  if (__sync_add_and_fetch(&version, 0) > _thr_SQP_version)
  {
    // update local rules;
    proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
                "Detected a changed in version. Global:%d , local:%d . "
                "Refreshing...\n",
         ...
         it != rules.end(); ++it)
    {
      ...
      if (qr1->active)
      {
        ...
        qr2= new_query_rule(
            qr1->rule_id, qr1->active, qr1->username, qr1->schemaname,
            qr1->flagIN, qr1->client_addr, qr1->proxy_addr, qr1->proxy_port,
            (qr1->digest ? buf : NULL), qr1->match_digest, qr1->match_pattern,
            qr1->negate_match_pattern, (char *)re_mod.c_str(), qr1->flagOUT,
            qr1->replace_pattern, qr1->destination_hostgroup, qr1->cache_ttl,
            qr1->reconnect, qr1->timeout, qr1->retries, qr1->delay,
            qr1->next_query_flagIN, qr1->mirror_flagOUT,
            qr1->mirror_hostgroup, qr1->error_msg, qr1->OK_msg,
            qr1->sticky_conn, qr1->multiplex, qr1->log, qr1->apply,
            qr1->comment);
        qr2->parent=
            qr1;  // pointer to parent to speed up parent update (hits)
        if (qr2->match_digest)
        {
          proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
                      "Compiling regex for rule_id: %d, match_digest: \n",
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
                      "Compiling regex for rule_id: %d, match_pattern: \n",
    ...
    _thr_SQP_rules_fast_routing=
        new std::unordered_map<std::string, int>(rules_fast_routing);
...
__internal_loop:
  for (std::vector<QP_rule_t *>::iterator it= _thr_SQP_rules->begin();
       it != _thr_SQP_rules->end(); ++it)
  {
    qr= *it;
    if (qr->flagIN != flagIN)
    {
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 6,
                  "query rule %d has no matching flagIN\n", qr->rule_id);
      continue;
    }
    if (qr->username && strlen(qr->username))
    {
      if (strcmp(qr->username,
                 sess->client_myds->myconn->userinfo->username) != 0)
      {
        proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                    "query rule %d has no matching username\n", qr->rule_id);
        continue;
      }
    }
    if (qr->schemaname && strlen(qr->schemaname))
    {
      if (strcmp(qr->schemaname,
                 sess->client_myds->myconn->userinfo->schemaname) != 0)
      {
        proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                    "query rule %d has no matching schemaname\n",
                    qr->rule_id);
        continue;
      }
    }
    ...
    if (qr->client_addr && strlen(qr->client_addr))
    {
      if (sess->client_myds->addr.addr)
      {
        ...
        {  // no wildcard , old algorithm
          if (strcmp(qr->client_addr, sess->client_myds->addr.addr) != 0)
          {
            proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                        "query rule %d has no matching client_addr\n",
                        qr->rule_id);
            continue;
          }
        }
        ...
        {  // client_addr_wildcard_position > 0
          if (strncmp(qr->client_addr, sess->client_myds->addr.addr,
    ...
    if (qr->proxy_addr && strlen(qr->proxy_addr))
    {
      if (sess->client_myds->proxy_addr.addr)
      {
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                      "query rule %d has no matching proxy_addr\n",
                      qr->rule_id);
          continue;
        }
      }
    ...
    if (qr->proxy_port >= 0)
    {
      ...
      {
        proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                    "query rule %d has no matching proxy_port\n",
                    qr->rule_id);
        continue;
      }
    }
    ...
    if (qp && qp->digest)
    {
      if (qr->digest)
      {
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                      "query rule %d has no matching digest\n", qr->rule_id);
          continue;
        }
      }
    }
    ...
    {  // we call this only if we have a query digest
      ...
      if (qr->match_digest)
      {
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                      "query rule %d has no matching pattern\n", qr->rule_id);
          continue;
        }
      }
    }
    ...
    {
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has changed flagOUT\n", qr->rule_id);
      flagIN= qr->flagOUT;
      set_flagOUT= true;
    ...
    if (qr->reconnect >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set reconnect: %d. Query will%s be "
                  "rexecuted if connection is lost\n",
                  qr->rule_id, qr->reconnect,
                  (qr->reconnect == 0 ? " NOT" : ""));
    ...
    if (qr->timeout >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set timeout: %d. Query will%s be "
                  "interrupted if exceeding %dms\n",
                  qr->rule_id, qr->timeout, (qr->timeout == 0 ? " NOT" : ""),
                  qr->timeout);
    ...
    if (qr->retries >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set retries: %d. Query will%s be "
                  "re-executed %d times in case of failure\n",
    ...
    if (qr->delay >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set delay: %d. Session will%s be paused "
                  "for %dms\n",
                  qr->rule_id, qr->delay, (qr->delay == 0 ? " NOT" : ""),
                  qr->delay);
      ret->delay= qr->delay;
    }
    if (qr->next_query_flagIN >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set next query flagIN: %d\n",
    ...
    if (qr->mirror_flagOUT >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set mirror flagOUT: %d\n", qr->rule_id,
                  qr->mirror_flagOUT);
    ...
    if (qr->mirror_hostgroup >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set mirror hostgroup: %d. A new session "
                  "will be created\n",
    ...
    {
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set error_msg: %s\n", qr->rule_id,
                  qr->error_msg);
    ...
    if (qr->cache_ttl >= 0)
    {
      ...
      proxy_debug(
          PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
          "query rule %d has set cache_ttl: %d. Query will%s hit the cache\n",
    ...
    if (qr->sticky_conn >= 0)
    {
      ...
      proxy_debug(
          PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
          "query rule %d has set sticky_conn: %d. Connection will%s stick\n",
    ...
    if (qr->multiplex >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set multiplex: %d. Connection will%s "
                  "multiplex\n",
                  qr->rule_id, qr->multiplex,
                  (qr->multiplex == 0 ? " NOT" : ""));
    ...
    if (qr->log >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set log: %d. Query will%s logged\n",
    ...
    if (qr->destination_hostgroup >= 0)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d has set destination hostgroup: %d\n",
    ...
    {
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d on match_pattern \"%s\" has a "
                  "replace_pattern \"%s\" to apply\n",
                  qr->rule_id, qr->match_pattern, qr->replace_pattern);
      if (ret->new_query == NULL)
        ret->new_query= new std::string(query);
      ...
      if (re2p->re2)
      {
        ...
        {
          re2p->re2->GlobalReplace(ret->new_query, qr->match_pattern,
                                   qr->replace_pattern);
        }
        else
        {
          re2p->re2->Replace(ret->new_query, qr->match_pattern,
                             qr->replace_pattern);
        }
      }
      else
    ...
    {
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d is the last one to apply: exit!\n",
                  qr->rule_id);
      goto __exit_process_mysql_query;
    }
    if (set_flagOUT == true)
...
__exit_process_mysql_query:
  if (qr == NULL || qr->apply == false)
  {
    ...
    if (mapl)
    {  // trigger new routing algorithm only if rules exists
  ...
  {  // we process comments only on original queries, not on mirrors
...
void Query_Processor::update_query_processor_stats()
{
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
              "Updating query rules statistics\n");
#ifdef PROXYSQL_QPRO_PTHREAD_MUTEX
  pthread_rwlock_rdlock(&rwlock);
#else
  spin_rdlock(&rwlock);
  ...
  for (int i= 0; i < MYSQL_COM_QUERY___NONE; i++)
  {
    for (int j= 0; j < 13; j++)
    {
      ...
      {
        __sync_fetch_and_add(&commands_counters[i]->counters[j],
                             _thr_commands_counters[i]->counters[j]);
        _thr_commands_counters[i]->counters[j]= 0;
      }
    }
    if (_thr_commands_counters[i]->total_time)
      __sync_fetch_and_add(&commands_counters[i]->total_time,
                           _thr_commands_counters[i]->total_time);
    _thr_commands_counters[i]->total_time= 0;
  }
};


void Query_Processor::query_parser_init(SQP_par_t *qp, char *query,
                                        int query_length, int flags)
{
  ...
  if (mysql_thread___query_digests)
  {
    qp->digest_text= mysql_query_digest_and_first_comment(
        query, query_length, &qp->first_comment,
        ((query_length < QUERY_DIGEST_BUF) ? qp->buf : NULL));
...
enum MYSQL_COM_QUERY_command Query_Processor::query_parser_command_type(
    SQP_par_t *qp)
{
...
unsigned long long Query_Processor::query_parser_update_counters(
    MySQL_Session *sess, enum MYSQL_COM_QUERY_command c, SQP_par_t *qp,
    unsigned long long t)
{
  if (c >= MYSQL_COM_QUERY___NONE)
    return 0;
  ...
  if (sess->CurrentQuery.stmt_info == NULL && qp->digest_text)
  {
    ...
    update_query_digest(qp, sess->current_hostgroup, ui, t,
                        sess->thread->curtime, NULL);
  }
  if (sess->CurrentQuery.stmt_info &&
      sess->CurrentQuery.stmt_info->digest_text)
  {
    uint64_t   hash2;
    SpookyHash myhash;
    myhash.Init(19, 3);
    ...
    //delete myhash;
    update_query_digest(qp, sess->current_hostgroup, ui, t,
                        sess->thread->curtime, stmt_info);
  }
  return ret;
}

void Query_Processor::update_query_digest(SQP_par_t *qp, int hid,
                                          MySQL_Connection_userinfo *ui,
                                          unsigned long long         t,
                                          unsigned long long         n,
                                          MySQL_STMT_Global_info *_stmt_info)
{
#ifdef PROXYSQL_QPRO_PTHREAD_MUTEX
  pthread_rwlock_wrlock(&digest_rwlock);
#else
  ...
  else
  {
    if (_stmt_info == NULL)
    {
      qds= new QP_query_digest_stats(ui->username, ui->schemaname, qp->digest,
                                     qp->digest_text, hid);
    }
    else
    {
      qds= new QP_query_digest_stats(ui->username, ui->schemaname,
                                     _stmt_info->digest,
                                     _stmt_info->digest_text, hid);
    }
    qds->add_time(t, n);
...
{
  char *text=
      NULL;  // this new variable is a pointer to either qp->digest_text , or to the query
  if (qp->digest_text)
  {
    text= qp->digest_text;
  }
  ...
  switch (c1)
  {
    ...
    case 'A':
      ...
      {  // ALTER [ONLINE | OFFLINE] [IGNORE] TABLE
        ...
        {
          if (!mystrcasecmp("OFFLINE", token) ||
              !mystrcasecmp("ONLINE", token))
          {
            token= (char *)tokenize(&tok);
            if (token == NULL)
              break;
      ...
      {  // ANALYZE [NO_WRITE_TO_BINLOG | LOCAL] TABLE
        ...
        {
          if (!strcasecmp("NO_WRITE_TO_BINLOG", token) ||
              !strcasecmp("LOCAL", token))
          {
            token= (char *)tokenize(&tok);
            if (token == NULL)
              break;
