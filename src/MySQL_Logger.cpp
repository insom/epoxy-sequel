...
MySQL_Event::MySQL_Event(uint32_t _thread_id, char *_username,
                         char *_schemaname, uint64_t _start_time,
                         uint64_t _end_time, uint64_t _query_digest,
                         char *_client, size_t _client_len)
{
...
uint64_t MySQL_Event::write(std::fstream *f)
{
  ...
  switch (et)
  {
    case PROXYSQL_QUERY: total_bytes= write_query(f); break;
    default: break;
  }
  return total_bytes;
}
...
void MySQL_Logger::log_request(MySQL_Session *sess, MySQL_Data_Stream *myds)
{
  ...
  {
    ca= (char *)malloc(cl + 8);
    sprintf(ca, "%s:%d", sess->client_myds->addr.addr,
  ...
  MySQL_Event me(sess->thread_session_id, ui->username, ui->schemaname,
                 ...
                 GloQPro->get_digest(&sess->CurrentQuery.QueryParserArgs), ca,
                 cl);
  ...
  {
    sa= (char *)malloc(sl + 8);
    sprintf(sa, "%s:%d", myds->myconn->parent->address,
