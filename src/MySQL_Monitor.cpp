 ...
 public:
  ...
  void *run()
  {
    ...
    for (int i= 0; (thrn ? i < thrn : 1); i++)
    {
      ...
      if (item->routine)
      {  // NULL is allowed, do nothing for it
    ...
    for (auto it3= lst.begin(); it3 != lst.end();)
    {
      ...
      if (now > (then + mysql_thread___monitor_ping_interval * 1000 * 3))
      {
        MySQL_Monitor_State_Data *mmsd=
            new MySQL_Monitor_State_Data((char *)"", 0, NULL, false);
        mmsd->mysql= my;
...
void MySQL_Monitor_Connection_Pool::put_connection(char *hostname, int port,
                                                   MYSQL *my)
{
  unsigned long long          now= monotonic_time();
  std::lock_guard<std::mutex> lock(mutex);
...
MySQL_Monitor_State_Data::MySQL_Monitor_State_Data(char *h, int p,
                                                   struct event_base *b,
                                                   bool _use_ssl, int g)
{
  task_id= MON_CONNECT;
...
MySQL_Monitor::MySQL_Monitor()
{
  ...
  admindb->open(
      ...
      SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX);
  admindb->execute(
      "ATTACH DATABASE 'file:mem_monitordb?mode=memory&cache=shared' AS "
      "'monitor'");
  ...
  insert_into_tables_defs(tables_defs_monitor, "mysql_server_connect_log",
                          MONITOR_SQLITE_TABLE_MYSQL_SERVER_CONNECT_LOG);
  //insert_into_tables_defs(tables_defs_monitor,"mysql_server_ping", MONITOR_SQLITE_TABLE_MYSQL_SERVER_PING);
  insert_into_tables_defs(tables_defs_monitor, "mysql_server_ping_log",
                          MONITOR_SQLITE_TABLE_MYSQL_SERVER_PING_LOG);
  insert_into_tables_defs(tables_defs_monitor, "mysql_server_read_only_log",
                          MONITOR_SQLITE_TABLE_MYSQL_SERVER_READ_ONLY_LOG);
  insert_into_tables_defs(
      tables_defs_monitor, "mysql_server_replication_lag_log",
      MONITOR_SQLITE_TABLE_MYSQL_SERVER_REPLICATION_LAG_LOG);
  insert_into_tables_defs(
      tables_defs_monitor, "mysql_server_group_replication_log",
      MONITOR_SQLITE_TABLE_MYSQL_SERVER_GROUP_REPLICATION_LOG);
  // create monitoring tables
  check_and_build_standard_tables(monitordb, tables_defs_monitor);
  monitordb->execute(
      "CREATE INDEX IF NOT EXISTS idx_connect_log_time_start ON "
      "mysql_server_connect_log (time_start_us)");
  monitordb->execute(
      "CREATE INDEX IF NOT EXISTS idx_ping_log_time_start ON "
      "mysql_server_ping_log (time_start_us)");
  monitordb->execute(
      "CREATE INDEX IF NOT EXISTS idx_read_only_log_time_start ON "
      "mysql_server_read_only_log (time_start_us)");
  monitordb->execute(
      "CREATE INDEX IF NOT EXISTS idx_replication_lag_log_time_start ON "
      "mysql_server_replication_lag_log (time_start_us)");
  monitordb->execute(
      "CREATE INDEX IF NOT EXISTS idx_group_replication_log_time_start ON "
      "mysql_server_group_replication_log (time_start_us)");
...
void MySQL_Monitor::insert_into_tables_defs(
...
// This function is copied from ProxySQL_Admin
void MySQL_Monitor::check_and_build_standard_tables(
...
void *monitor_connect_thread(void *arg)
{
  ...
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_text(statement, 1, mmsd->hostname, -1, SQLITE_TRANSIENT);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int(statement, 2, mmsd->port);
  assert(rc == SQLITE_OK);
  unsigned long long time_now= realtime_time();
  time_now= time_now - (mmsd->t2 - start_time);
  rc= sqlite3_bind_int64(statement, 3, time_now);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 4,
                         (mmsd->mysql_error_msg ? 0 : mmsd->t2 - mmsd->t1));
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_text(statement, 5, mmsd->mysql_error_msg, -1,
                        SQLITE_TRANSIENT);
  assert(rc == SQLITE_OK);
  ...
  {
    if ((strncmp(mmsd->mysql_error_msg, "Access denied for user",
                 strlen("Access denied for user")) == 0) ||
        (strncmp(mmsd->mysql_error_msg,
                 "ProxySQL Error: Access denied for user",
                 strlen("ProxySQL Error: Access denied for user")) == 0))
    {
      proxy_error(
...
void *monitor_ping_thread(void *arg)
{
  ...
  mmsd->mysql=
      GloMyMon->My_Conn_Pool->get_connection(mmsd->hostname, mmsd->port);
  unsigned long long start_time= mysql_thr->curtime;

  mmsd->t1= start_time;
  bool crc= false;
  if (mmsd->mysql == NULL)
  {  // we don't have a connection, let's create it
    bool rc;
  ...
  {
    mmsd->async_exit_status=
        wait_for_mysql(mmsd->mysql, mmsd->async_exit_status);
    unsigned long long now= monotonic_time();
    ...
    {
      mmsd->async_exit_status= mysql_ping_cont(&mmsd->interr, mmsd->mysql,
                                               mmsd->async_exit_status);
    }
  }
  if (mmsd->interr)
  ...
  else
  {
    if (crc == false)
    {
      GloMyMon->My_Conn_Pool->put_connection(mmsd->hostname, mmsd->port,
                                             mmsd->mysql);
      mmsd->mysql= NULL;
    }
  }
...
__fast_exit_monitor_ping_thread:
  if (mmsd->mysql)
  {
    ...
    else
    {
      ...
      {  // really not sure how we reached here, drop it
        mysql_close(mmsd->mysql);
        mmsd->mysql= NULL;
      }
    }
...
bool MySQL_Monitor_State_Data::set_wait_timeout()
{
  ...
  int   wait_timeout= mysql_thread___monitor_ping_interval * 10 /
                    1000;  // convert to second and multiply by 10
...
bool MySQL_Monitor_State_Data::create_new_connection()
{
  ...
  if (use_ssl)
  {
    mysql_ssl_set(mysql, mysql_thread___ssl_p2s_key,
                  mysql_thread___ssl_p2s_cert, mysql_thread___ssl_p2s_ca,
                  NULL, mysql_thread___ssl_p2s_cipher);
  }
  ...
  if (port)
  {
    myrc= mysql_real_connect(mysql, hostname, mysql_thread___monitor_username,
                             mysql_thread___monitor_password, NULL, port,
                             NULL, 0);
  }
  else
  {
    myrc= mysql_real_connect(
        mysql, "localhost", mysql_thread___monitor_username,
        mysql_thread___monitor_password, NULL, 0, hostname, 0);
  }
  if (myrc == NULL)
  {
...
void *monitor_read_only_thread(void *arg)
{
  ...
  {  // we don't have a connection, let's create it
    ...
    if (rc == false)
    {
      ...
      sprintf(new_error, "timeout on creating new connection: %s",
              mmsd->mysql_error_msg);
      free(mmsd->mysql_error_msg);
      mmsd->mysql_error_msg= new_error;
      proxy_error(
          "Timeout on read_only check for %s:%d after %lldms. Unable to "
          "create a connection. If the server is overload, increase "
          "mysql-monitor_connect_timeout. Error: %s.\n",
  ...
  mmsd->t1= monotonic_time();
  mmsd->async_exit_status= mysql_query_start(&mmsd->interr, mmsd->mysql,
  ...
  while (mmsd->async_exit_status)
  {
    ...
    if (now > mmsd->t1 + mysql_thread___monitor_read_only_timeout * 1000)
    {
      ...
      proxy_error(
          "Timeout on read_only check for %s:%d after %lldms. If the server "
          "is overload, increase mysql-monitor_read_only_timeout.\n",
    ...
    if ((mmsd->async_exit_status & MYSQL_WAIT_TIMEOUT) == 0)
    {
      mmsd->async_exit_status= mysql_query_cont(&mmsd->interr, mmsd->mysql,
                                                mmsd->async_exit_status);
    }
  }
  mmsd->async_exit_status=
      mysql_store_result_start(&mmsd->result, mmsd->mysql);
  while (mmsd->async_exit_status)
  {
    mmsd->async_exit_status=
        wait_for_mysql(mmsd->mysql, mmsd->async_exit_status);
    ...
    if ((mmsd->async_exit_status & MYSQL_WAIT_TIMEOUT) == 0)
    {
      mmsd->async_exit_status= mysql_store_result_cont(
          &mmsd->result, mmsd->mysql, mmsd->async_exit_status);
    }
  }
  if (mmsd->interr)
  {  // ping failed
...
__exit_monitor_read_only_thread:
  mmsd->t2= monotonic_time();
  {
    ...
    if (mmsd->result)
    {
      ...
      //					if (repl_lag>=0) {
      rc= sqlite3_bind_int64(statement, 5, read_only);
      assert(rc == SQLITE_OK);
      //					} else {
    ...
    {
      rc= sqlite3_bind_null(statement, 5);
      assert(rc == SQLITE_OK);
    }
    rc= sqlite3_bind_text(statement, 6, mmsd->mysql_error_msg, -1,
                          SQLITE_TRANSIENT);
    assert(rc == SQLITE_OK);
    SAFE_SQLITE3_STEP2(statement);
    rc= sqlite3_clear_bindings(statement);
    ...
    {
      MyHGM->read_only_action(mmsd->hostname, mmsd->port,
    ...
    else
    {
      ...
      char *buff=
          (char *)malloc(strlen(new_query) + strlen(mmsd->hostname) + 32);
      int max_failures= mysql_thread___monitor_read_only_max_timeout_count;
      sprintf(buff, new_query, mmsd->hostname, mmsd->port, max_failures,
              max_failures);
      mondb->execute_statement(buff, &error, &cols, &affected_rows,
                               &resultset);
      if (!error)
      {
        if (resultset)
        {
          if (resultset->rows_count)
          {
            // disable host
            proxy_error(
                "Server %s:%d missed %d read_only checks. Assuming "
                "read_only=1\n",
                mmsd->hostname, mmsd->port, max_failures);
            MyHGM->read_only_action(mmsd->hostname, mmsd->port,
                                    read_only);  // N timeouts reached
          }
          delete resultset;
          resultset= NULL;
        }
...
void *monitor_group_replication_thread(void *arg)
{
  ...
  mmsd->async_exit_status= mysql_query_start(
      &mmsd->interr, mmsd->mysql,
      "SELECT viable_candidate,read_only,transactions_behind FROM "
      "sys.gr_member_routing_candidate_status");
  while (mmsd->async_exit_status)
  {
    mmsd->async_exit_status=
        wait_for_mysql(mmsd->mysql, mmsd->async_exit_status);
                ...
                1000)
    {
      mmsd->mysql_error_msg= strdup("timeout check");
      proxy_error(
          "Timeout on group replication health check for %s:%d after %lldms. "
          "If the server is overload, increase "
          "mysql-monitor_groupreplication_healthcheck_timeout. Assuming "
          "viable_candidate=nO and read_only=YES\n",
          mmsd->hostname, mmsd->port, (now - mmsd->t1) / 1000);
  ...
  __end_process_group_replication_result:
    ...
    if (it2 != GloMyMon->Group_Replication_Hosts_Map.end())
    {
      ...
      node->add_entry(time_now,
                      (mmsd->mysql_error_msg ? 0 : mmsd->t2 - mmsd->t1),
                      transactions_behind, viable_candidate, read_only,
                      mmsd->mysql_error_msg);
    }
    else
    {
      node= new MyGR_monitor_node(mmsd->hostname, mmsd->port,
                                  mmsd->writer_hostgroup);
      node->add_entry(time_now,
                      (mmsd->mysql_error_msg ? 0 : mmsd->t2 - mmsd->t1),
                      transactions_behind, viable_candidate, read_only,
                      mmsd->mysql_error_msg);
    ...
    if (mmsd->mysql_error_msg)
    {  // there was an error checking the status of the server, surely we need to reconfigure GR
      MyHGM->update_group_replication_set_offline(mmsd->hostname, mmsd->port,
                                                  mmsd->writer_hostgroup,
                                                  mmsd->mysql_error_msg);
    }
    else
    {
      ...
      {
        MyHGM->update_group_replication_set_offline(
            mmsd->hostname, mmsd->port, mmsd->writer_hostgroup,
            (char *)"viable_candidate=NO");
      }
      else
      {
        if (read_only == true)
        {
          if (transactions_behind > mmsd->max_transactions_behind)
          {
            MyHGM->update_group_replication_set_offline(
                mmsd->hostname, mmsd->port, mmsd->writer_hostgroup,
                (char *)"slave is lagging");
          }
          else
          {
            MyHGM->update_group_replication_set_read_only(
                mmsd->hostname, mmsd->port, mmsd->writer_hostgroup,
                (char *)"read_only=YES");
          }
        }
        else
        {
          ...
          MyHGM->update_group_replication_set_writer(
              mmsd->hostname, mmsd->port, mmsd->writer_hostgroup);
        }
      }
    }
...
void *monitor_replication_lag_thread(void *arg)
{
  ...
  if (percona_heartbeat_table)
  {
    ...
    if (l)
    {
      ...
      mmsd->async_exit_status=
          mysql_query_start(&mmsd->interr, mmsd->mysql, replication_query);
  ...
  {
    mmsd->async_exit_status=
        mysql_query_start(&mmsd->interr, mmsd->mysql, "SHOW SLAVE STATUS");
  }
  while (mmsd->async_exit_status)
  {
    mmsd->async_exit_status=
    ...
    unsigned long long now= monotonic_time();
    if (now >
        mmsd->t1 + mysql_thread___monitor_replication_lag_timeout * 1000)
    {
      mmsd->mysql_error_msg= strdup("timeout check");
...
__exit_monitor_replication_lag_thread:
  mmsd->t2= monotonic_time();
  {
    ...
    if (mmsd->result)
    {
      ...
      if (j > -1)
      {
        ...
        if (row)
        {
          ...
          repl_lag=
              mysql_thread___monitor_slave_lag_when_null;  // new behavior, see 669
          if (row[j])
          {  // if Seconds_Behind_Master is not NULL
            repl_lag= atoi(row[j]);
          }
        }
      }
      if (repl_lag >= 0)
      {
        rc= sqlite3_bind_int64(statement, 5, repl_lag);
        assert(rc == SQLITE_OK);
      }
      else
      {
    ...
    rc= sqlite3_reset(statement);
    assert(rc == SQLITE_OK);
    MyHGM->replication_lag_action(mmsd->hostgroup_id, mmsd->hostname,
                                  mmsd->port, repl_lag);
    sqlite3_finalize(statement);
  }
  if (mmsd->interr)
...
void *MySQL_Monitor::monitor_connect()
{
  ...
  while (GloMyMon->shutdown == false &&
         mysql_thread___monitor_enabled == true)
  {
    char *          error= NULL;
    int             cols= 0;
    int             affected_rows= 0;
    ...
    else
    {
      ...
      if (resultset->rows_count)
      {
        us= mysql_thread___monitor_connect_interval / 2 /
           ...
           it != resultset->rows.end(); ++it)
      {
        ...
        if (rc_ping)
        {  // only if server is responding to pings
          MySQL_Monitor_State_Data *mmsd= new MySQL_Monitor_State_Data(
              r->fields[0], atoi(r->fields[1]), NULL, atoi(r->fields[2]));
  ...
  __end_monitor_connect_loop:
    if (mysql_thread___monitor_enabled == true)
    {
      ...
      rc= sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
      assert(rc == SQLITE_OK);
      if (mysql_thread___monitor_history <
          mysql_thread___monitor_ping_interval *
              (mysql_thread___monitor_ping_max_failures + 1))
      {  // issue #626
      ...
      rc= sqlite3_bind_int64(
          statement, 1,
          time_now -
              (unsigned long long)mysql_thread___monitor_history * 1000);
      assert(rc == SQLITE_OK);
      SAFE_SQLITE3_STEP2(statement);
      rc= sqlite3_clear_bindings(statement);
      assert(rc == SQLITE_OK);
...
void *MySQL_Monitor::monitor_ping()
{
  ...
  __end_monitor_ping_loop:
    if (mysql_thread___monitor_enabled == true)
    {
      ...
      query=
          (char
               *)"DELETE FROM mysql_server_ping_log WHERE time_start_us < ?1";
      rc= sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
      assert(rc == SQLITE_OK);
      if (mysql_thread___monitor_history <
          mysql_thread___monitor_ping_interval *
    ...
    else
    {
      ...
      int    j= 0;
      char **addresses=
          (char **)malloc(resultset->rows_count * sizeof(char *));
      ...
      for (j= 0; j < i; j++)
      {
        char *buff= (char *)malloc(strlen(new_query) + strlen(addresses[j]) +
                                   strlen(ports[j]) + 16);
        int   max_failures= mysql_thread___monitor_ping_max_failures;
        sprintf(buff, new_query, addresses[j], ports[j], max_failures,
                max_failures);
        monitordb->execute_statement(buff, &error, &cols, &affected_rows,
                                     &resultset);
        if (!error)
        {
          if (resultset)
          {
            if (resultset->rows_count)
            {
              ...
              {
                proxy_error(
                    "Server %s:%s missed %d heartbeats, shunning it and "
                    "killing all the connections. Disabling other checks "
                    "until the node comes back online.\n",
      ...
      while (i)
      {  // now free all the addresses/ports
        i--;
        free(addresses[i]);
        free(ports[i]);
      }
        ...
        if (!error)
        {
          if (resultset)
          {
            if (resultset->rows_count)
            {
              ...
              {
                SQLite3_row *r=
                    *it;  // this should be called just once, but we create a generic for loop
                // update current_latency_ms
                MyHGM->set_server_current_latency_us(
                    addresses[j], atoi(ports[j]), atoi(r->fields[2]));
              }
            }
            delete resultset;
            resultset= NULL;
...
void *MySQL_Monitor::monitor_read_only()
{
         ...
         mysql_thread___monitor_enabled == true)
  {
    ...
    SQLite3_result *resultset= NULL;
    // add support for SSL
    char *query=(char *)"SELECT hostname, port, MAX(use_ssl) use_ssl FROM mysql_servers JOIN mysql_replication_hostgroups ON hostgroup_id=writer_hostgroup OR hostgroup_id=reader_hostgroup WHERE status NOT LIKE 'OFFLINE\%' GROUP BY hostname, port ORDER BY RANDOM()";
    t1= monotonic_time();
    ...
    else
    {
      ...
      if (resultset->rows_count)
      {
        us= mysql_thread___monitor_read_only_interval / 2 /
            resultset->rows_count;
        us*= 40;
        if (us > 1000000)
        {
    ...
    if (mysql_thread___monitor_enabled == true)
    {
      ...
      if (mysql_thread___monitor_history <
          mysql_thread___monitor_read_only_interval *
              (mysql_thread___monitor_read_only_max_timeout_count + 1))
      {  // issue #626
...
void *MySQL_Monitor::monitor_group_replication()
{
         ...
         mysql_thread___monitor_enabled == true)
  {
    ...
    next_loop_at=
        t1 +
        1000 * mysql_thread___monitor_groupreplication_healthcheck_interval;
    ...
    else
    {
      ...
      {
        us= mysql_thread___monitor_read_only_interval / 2 /
            Group_Replication_Hosts_resultset->rows_count;
      }
      for (std::vector<SQLite3_row *>::iterator it=
               Group_Replication_Hosts_resultset->rows.begin();
           it != Group_Replication_Hosts_resultset->rows.end(); ++it)
      {
        SQLite3_row *             r= *it;
        MySQL_Monitor_State_Data *mmsd= new MySQL_Monitor_State_Data(
            r->fields[1], atoi(r->fields[2]), NULL, atoi(r->fields[3]));
...
void *MySQL_Monitor::monitor_replication_lag()
{
         ...
         mysql_thread___monitor_enabled == true)
  {
    ...
    else
    {
      ...
      if (resultset->rows_count)
      {
        us= mysql_thread___monitor_replication_lag_interval / 2 /
            resultset->rows_count;
      }
      for (std::vector<SQLite3_row *>::iterator it= resultset->rows.begin();
           it != resultset->rows.end(); ++it)
      {
        ...
        {  // only if server is responding to pings
          MySQL_Monitor_State_Data *mmsd= new MySQL_Monitor_State_Data(
              r->fields[1], atoi(r->fields[2]), NULL, atoi(r->fields[4]),
              atoi(r->fields[0]));
          mmsd->mondb= monitordb;
          WorkItem *item;
...
__monitor_run:
  while (queue.size())
  {  // this is a clean up in case Monitor was restarted
  ...
  ConsumerThread **threads=
      (ConsumerThread **)malloc(sizeof(ConsumerThread *) * num_threads);
  for (unsigned int i= 0; i < num_threads; i++)
  {
  ...
  if (pthread_create(&monitor_connect_thread, &attr, &monitor_connect_pthread,
                     NULL) != 0)
  {
  ...
  if (pthread_create(&monitor_ping_thread, &attr, &monitor_ping_pthread,
                     NULL) != 0)
  {
    proxy_error("Thread creation\n");
    assert(0);
  ...
  if (pthread_create(&monitor_read_only_thread, &attr,
                     &monitor_read_only_pthread, NULL) != 0)
  {
    proxy_error("Thread creation\n");
    assert(0);
  }
  pthread_t monitor_group_replication_thread;
  if (pthread_create(&monitor_group_replication_thread, &attr,
                     &monitor_group_replication_pthread, NULL) != 0)
  {
    proxy_error("Thread creation\n");
    assert(0);
  }
  pthread_t monitor_replication_lag_thread;
  if (pthread_create(&monitor_replication_lag_thread, &attr,
                     &monitor_replication_lag_pthread, NULL) != 0)
  {
    proxy_error("Thread creation\n");
    assert(0);
  }
  while (shutdown == false && mysql_thread___monitor_enabled == true)
  {
    ...
    {
      proxy_error(
          "Monitor queue too big, try to reduce frequency of checks: %d\n",
          qsize);
      qsize= qsize / 250;
      proxy_error("Monitor is starting %d helper threads\n", qsize);
      ConsumerThread **threads_aux=
          (ConsumerThread **)malloc(sizeof(ConsumerThread *) * qsize);
...
bool MyGR_monitor_node::add_entry(unsigned long long _st,
                                  unsigned long long _ct, long long _tb,
                                  bool _pp, bool _ro, char *_error)
{
  bool ret= false;
  if (idx_last_entry == -1)
    ret= true;
  ...
  if (ret == false)
  {
    if (last_entries[idx_last_entry].primary_partition !=
        last_entries[prev_last_entry].primary_partition)
    {
      ret= true;
    }
    if (last_entries[idx_last_entry].read_only !=
        last_entries[prev_last_entry].read_only)
    {
      ret= true;
    }
    if ((last_entries[idx_last_entry].error &&
         last_entries[prev_last_entry].error == NULL) ||
        (last_entries[idx_last_entry].error == NULL &&
         last_entries[prev_last_entry].error) ||
        (last_entries[idx_last_entry].error &&
         last_entries[prev_last_entry].error &&
         strcmp(last_entries[idx_last_entry].error,
                last_entries[prev_last_entry].error)))
    {
      ret= true;
    }
...
void MySQL_Monitor::populate_monitor_mysql_server_group_replication_log()
{
  ...
  assert(rc == SQLITE_OK);
  monitordb->execute(
      (char *)"DELETE FROM mysql_server_group_replication_log");
  std::map<std::string, MyGR_monitor_node *>::iterator it2;
  MyGR_monitor_node *                                  node= NULL;
  for (it2= GloMyMon->Group_Replication_Hosts_Map.begin();
       it2 != GloMyMon->Group_Replication_Hosts_Map.end(); ++it2)
  {
    ...
    for (i= 0; i < MyGR_Nentries; i++)
    {
      ...
      {
        rc= sqlite3_bind_text(statement1, 1, host.c_str(), -1,
                              SQLITE_TRANSIENT);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_int64(statement1, 2, atoi(port.c_str()));
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_int64(statement1, 3,
                               node->last_entries[i].start_time);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_int64(statement1, 4,
                               node->last_entries[i].check_time);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_text(
            statement1, 5,
            (node->last_entries[i].primary_partition ? (char *)"YES"
                                                     : (char *)"NO"),
            -1, SQLITE_TRANSIENT);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_text(
            statement1, 6,
            (node->last_entries[i].read_only ? (char *)"YES" : (char *)"NO"),
            -1, SQLITE_TRANSIENT);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_int64(statement1, 7,
                               node->last_entries[i].transactions_behind);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_text(statement1, 8, node->last_entries[i].error, -1,
                              SQLITE_TRANSIENT);
        assert(rc == SQLITE_OK);
