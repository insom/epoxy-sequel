...
static void *HGCU_thread_run()
{
  ...
  while (1)
  {
    ...
    for (i= 0; i < (int)l; i++)
    {
      ...
      if (myconn->mysql->net.vio && myconn->mysql->net.fd &&
          myconn->mysql->net.buff)
      {
        ...
        statuses[i]= mysql_change_user_start(
            &ret[i], myconn->mysql, myconn->userinfo->username, auth_password,
            myconn->userinfo->schemaname);
        if (myconn->mysql->net.vio == NULL || myconn->mysql->net.fd == 0 ||
            myconn->mysql->net.buff == NULL)
        {
          statuses[i]= 0;
          ret[i]= 1;
        }
      ...
      for (i= 0; i < (int)conn_array->len; i++)
      {
            ...
            myconn->mysql->net.buff)
        {
              ...
              myconn->mysql->net.buff)
          {
            ...
            {
              statuses[i]=
                  mysql_change_user_cont(&ret[i], myconn->mysql, statuses[i]);
              if (myconn->mysql->net.vio == NULL ||
                  myconn->mysql->net.fd == 0 ||
                  myconn->mysql->net.buff == NULL)
              {
                statuses[i]= 0;
                ret[i]= 1;
...
void MySrvConnList::add(MySQL_Connection *c) { conns->add(c); }
...
void MySrvConnList::drop_all_connections()
{
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              "Dropping all connections (%lu total) on MySrvConnList %p for "
              "server %s:%d , hostgroup=%d , status=%d\n",
              conns_length(), this, mysrvc->address, mysrvc->port,
              mysrvc->myhgc->hid, mysrvc->status);
  while (conns_length())
  {
    MySQL_Connection *conn= (MySQL_Connection *)conns->remove_index_fast(0);
    delete conn;
...
MySrvC::MySrvC(char *add, uint16_t p, unsigned int _weight,
               enum MySerStatus _status,
               unsigned int     _compression /*, uint8_t _charset */,
               unsigned int     _max_connections,
               unsigned int _max_replication_lag, unsigned int _use_ssl,
               unsigned int _max_latency_ms, char *_comment)
{
  address= strdup(add);
  port= p;
  weight= _weight;
...
void MySrvC::connect_error(int err_num)
{
  ...
  else
  {
    ...
    if (__sync_add_and_fetch(&connect_ERR_at_time_last_detected_error, 1) >=
        (unsigned int)max_failures)
    {
      ...
      {
        proxy_error(
            "Shunning server %s:%d with %u errors/sec. Shunning for %u "
            "seconds\n",
...
#ifdef DEBUG
  mydb->open(
      (char *)"file:mem_mydb?mode=memory&cache=shared",
      SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX);
#else
  mydb->open((char *)"file:mem_mydb?mode=memory", SQLITE_OPEN_READWRITE |
                                                      SQLITE_OPEN_CREATE |
...
void MySQL_HostGroups_Manager::wait_servers_table_version(unsigned v,
                                                          unsigned w)
{
  struct timespec ts;
  ...
  while ((rc == 0 || rc == ETIMEDOUT) && (i < w) &&
         (__sync_fetch_and_add(&glovars.shutdown, 0) == 0) &&
         (__sync_fetch_and_add(&status.servers_table_version, 0) < v))
  {
    i++;
    ts.tv_sec+= 1;
    rc= pthread_cond_timedwait(&status.servers_table_version_cond,
                               &status.servers_table_version_lock, &ts);
  }
...
bool MySQL_HostGroups_Manager::server_add(
    unsigned int hid, char *add, uint16_t p, unsigned int _weight,
    enum MySerStatus status, unsigned int _comp /*, uint8_t _charset */,
    unsigned int _max_connections, unsigned int _max_replication_lag,
    unsigned int _use_ssl, unsigned int _max_latency_ms, char *comment)
{
  bool ret= true;
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              "Adding in mysql_servers_incoming server %s:%d in hostgroup %u "
              "with weight %u , status %u, %s compression, max_connections "
              "%d, max_replication_lag %u, use_ssl=%u, max_latency_ms=%u\n",
              add, p, hid, _weight, status,
              (_comp ? "with" : "without") /*, _charset */, _max_connections,
              _max_replication_lag, _use_ssl, _max_latency_ms);
  int           rc;
  ...
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 1, hid);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_text(statement, 2, add, -1, SQLITE_TRANSIENT);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 3, p);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 4, _weight);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 5, status);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 6, _comp);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 7, _max_connections);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 8, _max_replication_lag);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 9, _use_ssl);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_int64(statement, 10, _max_latency_ms);
  assert(rc == SQLITE_OK);
  rc= sqlite3_bind_text(statement, 11, comment, -1, SQLITE_TRANSIENT);
  assert(rc == SQLITE_OK);
       ...
       it != resultset->rows.end(); ++it)
  {
    ...
    {  // bulk
      ...
      rc= sqlite3_bind_int64(statement32, (idx * 11) + 3,
                             atoi(r1->fields[2]));
      assert(rc == SQLITE_OK);
      rc= sqlite3_bind_int64(statement32, (idx * 11) + 4,
                             atoi(r1->fields[4]));
      assert(rc == SQLITE_OK);
      rc= sqlite3_bind_int64(statement32, (idx * 11) + 5, status1);
      assert(rc == SQLITE_OK);
      ...
      rc= sqlite3_bind_int64(statement32, (idx * 11) + 10,
                             atoi(r1->fields[9]));
      assert(rc == SQLITE_OK);
      rc= sqlite3_bind_text(statement32, (idx * 11) + 11, r1->fields[10], -1,
                            SQLITE_TRANSIENT);
      assert(rc == SQLITE_OK);
      if (idx == 31)
      {
    ...
    {  // single row
      ...
      assert(rc == SQLITE_OK);
      rc= sqlite3_bind_int64(statement1, 4, atoi(r1->fields[4]));
      assert(rc == SQLITE_OK);
      rc= sqlite3_bind_int64(statement1, 5, status1);
      assert(rc == SQLITE_OK);
...
SQLite3_result *MySQL_HostGroups_Manager::execute_query(char * query,
                                                        char **error)
{
  int             cols= 0;
  int             affected_rows= 0;
...
bool MySQL_HostGroups_Manager::commit()
{
  ...
  {
    mydb->execute_statement((char *)"SELECT * FROM mysql_servers_incoming",
  ...
  else
  {
         ...
         it != resultset->rows.end(); ++it)
    {
      ...
      proxy_warning(
          "Removed server at address %lld, hostgroup %s, address %s port %s. "
          "Setting status OFFLINE HARD and immediately dropping all free "
          "connections. Used connections will be dropped when trying to use "
          "them\n",
  ...
  mydb->execute(
      "INSERT OR IGNORE INTO mysql_servers(hostgroup_id, hostname, port, "
      "weight, status, compression, max_connections, max_replication_lag, "
      "use_ssl, max_latency_ms, comment) SELECT hostgroup_id, hostname, "
      "port, weight, status, compression, max_connections, "
      "max_replication_lag, use_ssl, max_latency_ms, comment FROM "
  ...
  else
  {
         ...
         it != resultset->rows.end(); ++it)
    {
      ...
      long long    ptr= atoll(
          r->fields
              [11]);  // increase this index every time a new column is added
      proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                  "Server %s:%d , weight=%d, status=%d, mem_pointer=%llu, "
                  "hostgroup=%d, compression=%d\n",
                  r->fields[1], atoi(r->fields[2]), atoi(r->fields[3]),
                  (MySerStatus)atoi(r->fields[4]), ptr, atoi(r->fields[0]),
                  atoi(r->fields[5]));
      //fprintf(stderr,"%lld\n", ptr);
      if (ptr == 0)
      {
        proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                    "Creating new server %s:%d , weight=%d, status=%d, "
                    "compression=%d\n",
                    r->fields[1], atoi(r->fields[2]), atoi(r->fields[3]),
                    (MySerStatus)atoi(r->fields[4]), atoi(r->fields[5]));
        MySrvC *mysrvc= new MySrvC(
            r->fields[1], atoi(r->fields[2]), atoi(r->fields[3]),
            (MySerStatus)atoi(r->fields[4]), atoi(r->fields[5]),
            atoi(r->fields[6]), atoi(r->fields[7]), atoi(r->fields[8]),
            atoi(r->fields[9]),
            r->fields
                [10]);  // add new fields here if adding more columns in mysql_servers
        proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                    "Adding new server %s:%d , weight=%d, status=%d, "
                    "mem_ptr=%p into hostgroup=%d\n",
                    r->fields[1], atoi(r->fields[2]), atoi(r->fields[3]),
                    (MySerStatus)atoi(r->fields[4]), mysrvc,
                    atoi(r->fields[0]));
        add(mysrvc, atoi(r->fields[0]));
        ptr= (uintptr_t)mysrvc;
        rc= sqlite3_bind_int64(statement1, 1, ptr);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_int64(statement1, 2, atoi(r->fields[0]));
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_text(statement1, 3, r->fields[1], -1,
                              SQLITE_TRANSIENT);
        assert(rc == SQLITE_OK);
        rc= sqlite3_bind_int64(statement1, 4, atoi(r->fields[2]));
        assert(rc == SQLITE_OK);
        SAFE_SQLITE3_STEP2(statement1);
        rc= sqlite3_clear_bindings(statement1);
        assert(rc == SQLITE_OK);
      ...
      else
      {
        ...
        {
          proxy_debug(
              PROXY_DEBUG_MYSQL_CONNPOOL, 5,
              "Changing weight for server %s:%d (%s:%d) from %d (%d) to %d\n",
              mysrvc->address, mysrvc->port, r->fields[1], atoi(r->fields[2]),
              r->fields[3], mysrvc->weight, atoi(r->fields[12]));
        ...
        if (atoi(r->fields[4]) != atoi(r->fields[13]))
        {
          proxy_debug(
              PROXY_DEBUG_MYSQL_CONNPOOL, 5,
              "Changing status for server %s:%d (%s:%d) from %d (%d) to %d\n",
              mysrvc->address, mysrvc->port, r->fields[1], atoi(r->fields[2]),
              r->fields[4], mysrvc->status, atoi(r->fields[13]));
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      "Changing compression for server %s:%d (%s:%d) from %d "
                      "(%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[5], mysrvc->compression,
                      atoi(r->fields[14]));
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      "Changing max_connections for server %s:%d (%s:%d) "
                      "from %d (%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[6],
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      "Changing max_replication_lag for server %s:%d (%s:%d) "
                      "from %d (%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[7],
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      "Changing use_ssl for server %s:%d (%s:%d) from %d "
                      "(%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[8], mysrvc->use_ssl,
                      atoi(r->fields[17]));
        ...
        {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      "Changing max_latency_ms for server %s:%d (%s:%d) from "
                      "%d (%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[9],
                      mysrvc->max_latency_us, atoi(r->fields[18]));
        ...
        if (strcmp(r->fields[10], r->fields[19]))
        {
          proxy_debug(
              PROXY_DEBUG_MYSQL_CONNPOOL, 5,
              "Changing comment for server %s:%d (%s:%d) from '%s' to '%s'\n",
              mysrvc->address, mysrvc->port, r->fields[1], atoi(r->fields[2]),
              r->fields[10], r->fields[19]);
          free(mysrvc->comment);
          mysrvc->comment= strdup(r->fields[19]);
        }
        if (run_update)
        {
          rc= sqlite3_bind_int64(statement2, 1, mysrvc->weight);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 2, mysrvc->status);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 3, mysrvc->compression);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 4, mysrvc->max_connections);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 5, mysrvc->max_replication_lag);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 6, mysrvc->use_ssl);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 7,
                                 mysrvc->max_latency_us / 1000);
          assert(rc == SQLITE_OK);

          rc= sqlite3_bind_text(statement2, 8, mysrvc->comment, -1,
                                SQLITE_TRANSIENT);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 9, mysrvc->myhgc->hid);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_text(statement2, 10, mysrvc->address, -1,
                                SQLITE_TRANSIENT);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement2, 11, mysrvc->port);
          assert(rc == SQLITE_OK);
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4,
              "DELETE FROM mysql_servers_incoming\n");
...
*/
    {
      ...
      mydb->execute_statement(query, &error, &cols, &affected_rows,
                              &resultset);
      if (resultset)
      {
...
void MySQL_HostGroups_Manager::purge_mysql_servers_table()
{
  for (unsigned int i= 0; i < MyHostGroups->len; i++)
  {
    ...
    for (unsigned int j= 0; j < myhgc->mysrvs->servers->len; j++)
    {
      ...
      {
        if (mysrvc->ConnectionsUsed->conns_length() == 0 &&
...
void MySQL_HostGroups_Manager::generate_mysql_servers_table(int *_onlyhg)
{
  ...
  if (mysql_thread___hostgroup_manager_verbose)
  {
    ...
    {
      proxy_info(
          "Dumping current MySQL Servers structures for hostgroup ALL\n");
    }
    else
    {
      int hidonly= *_onlyhg;
      proxy_info(
          "Dumping current MySQL Servers structures for hostgroup %d\n",
          hidonly);
    }
  }
  for (unsigned int i= 0; i < MyHostGroups->len; i++)
  {
    ...
    for (unsigned int j= 0; j < myhgc->mysrvs->servers->len; j++)
    {
      ...
      if (mysql_thread___hostgroup_manager_verbose)
      {
        ...
        {
          case 0: st= (char *)"ONLINE"; break;
          case 2: st= (char *)"OFFLINE_SOFT"; break;
          case 3: st= (char *)"OFFLINE_HARD"; break;
          default:
          case 1:
          case 4: st= (char *)"SHUNNED"; break;
        }
        fprintf(stderr,
                "HID: %d , address: %s , port: %d , weight: %d , status: %s "
                ", max_connections: %u , max_replication_lag: %u , use_ssl: "
                "%u , max_latency_ms: %u , comment: %s\n",
                mysrvc->myhgc->hid, mysrvc->address, mysrvc->port,
                mysrvc->weight, st, mysrvc->max_connections,
                mysrvc->max_replication_lag, mysrvc->use_ssl,
                mysrvc->max_latency_us * 1000, mysrvc->comment);
      }
      lst->add(mysrvc);
      if (lst->len == 32)
      {
        while (lst->len)
        {
          ...
          rc= sqlite3_bind_int64(statement32, (i * 12) + 1,
                                 mysrvc->myhgc->hid);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_text(statement32, (i * 12) + 2, mysrvc->address,
                                -1, SQLITE_TRANSIENT);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 3, mysrvc->port);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 4, mysrvc->weight);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 5, mysrvc->status);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 6,
                                 mysrvc->compression);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 7,
                                 mysrvc->max_connections);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 8,
                                 mysrvc->max_replication_lag);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 9, mysrvc->use_ssl);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 10,
                                 mysrvc->max_latency_us / 1000);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_text(statement32, (i * 12) + 11, mysrvc->comment,
                                -1, SQLITE_TRANSIENT);
          assert(rc == SQLITE_OK);
          rc= sqlite3_bind_int64(statement32, (i * 12) + 12, ptr);
          assert(rc == SQLITE_OK);
        }
        SAFE_SQLITE3_STEP2(statement32);
        rc= sqlite3_clear_bindings(statement32);
  ...
  while (lst->len)
  {
    MySrvC *  mysrvc= (MySrvC *)lst->remove_index_fast(0);
    uintptr_t ptr= (uintptr_t)mysrvc;
    rc= sqlite3_bind_int64(statement1, 1, mysrvc->myhgc->hid);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_text(statement1, 2, mysrvc->address, -1,
                          SQLITE_TRANSIENT);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 3, mysrvc->port);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 4, mysrvc->weight);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 5, mysrvc->status);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 6, mysrvc->compression);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 7, mysrvc->max_connections);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 8, mysrvc->max_replication_lag);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 9, mysrvc->use_ssl);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 10, mysrvc->max_latency_us / 1000);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_text(statement1, 11, mysrvc->comment, -1,
                          SQLITE_TRANSIENT);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 12, ptr);
    assert(rc == SQLITE_OK);
  ...
  if (mysql_thread___hostgroup_manager_verbose)
  {
    ...
    int             affected_rows= 0;
    SQLite3_result *resultset= NULL;
    mydb->execute_statement((char *)"SELECT * FROM mysql_servers", &error,
                            &cols, &affected_rows, &resultset);
    if (error)
    {
...
void MySQL_HostGroups_Manager::generate_mysql_replication_hostgroups_table()
{
  ...
  for (std::vector<SQLite3_row *>::iterator it=
           incoming_replication_hostgroups->rows.begin();
       it != incoming_replication_hostgroups->rows.end(); ++it)
  {
    SQLite3_row *r= *it;
    char *       o= NULL;
    ...
    {  // comment is not null
      sprintf(query,
              "INSERT INTO mysql_replication_hostgroups VALUES(%s,%s,'%s')",
              r->fields[0], r->fields[1], o);
      if (o != r->fields[2])
      {  // there was a copy
        free(o);
      }
    ...
    {
      sprintf(query,
              "INSERT INTO mysql_replication_hostgroups VALUES(%s,%s,NULL)",
    ...
    generate_mysql_group_replication_hostgroups_table()
{
  ...
  for (std::map<int, Group_Replication_Info *>::iterator it1=
           Group_Replication_Info_Map.begin();
       it1 != Group_Replication_Info_Map.end(); ++it1)
  {
  ...
  for (std::vector<SQLite3_row *>::iterator it=
           incoming_group_replication_hostgroups->rows.begin();
       it != incoming_group_replication_hostgroups->rows.end(); ++it)
  {
    SQLite3_row *r= *it;
    ...
    proxy_info(
        "Loading MySQL Group Replication info for "
        "(%d,%d,%d,%d,%s,%d,%d,%d,\"%s\")\n",
        writer_hostgroup, backup_writer_hostgroup, reader_hostgroup,
        offline_hostgroup, (active ? "on" : "off"), max_writers,
        writer_is_also_reader, max_transactions_behind, r->fields[8]);
    rc= sqlite3_bind_int64(statement, 1, writer_hostgroup);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 2, backup_writer_hostgroup);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 3, reader_hostgroup);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 4, offline_hostgroup);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 5, active);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 6, max_writers);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 7, writer_is_also_reader);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement, 8, max_transactions_behind);
    assert(rc == SQLITE_OK);
    ...
    if (it2 != Group_Replication_Info_Map.end())
    {
      info= it2->second;
      bool changed= false;
      changed= info->update(backup_writer_hostgroup, reader_hostgroup,
                            offline_hostgroup, max_writers,
                            max_transactions_behind, (bool)active,
                            (bool)writer_is_also_reader, r->fields[8]);
      if (changed)
      {
    ...
    {
      info= new Group_Replication_Info(
          writer_hostgroup, backup_writer_hostgroup, reader_hostgroup,
          offline_hostgroup, max_writers, max_transactions_behind,
          (bool)active, (bool)writer_is_also_reader, r->fields[8]);
      //info->need_converge=true;
      Group_Replication_Info_Map.insert(
          Group_Replication_Info_Map.begin(),
          std::pair<int, Group_Replication_Info *>(writer_hostgroup, info));
    }
  }
  sqlite3_finalize(statement);
  ...
  // remove missing ones
  for (auto it3= Group_Replication_Info_Map.begin();
       it3 != Group_Replication_Info_Map.end();)
  {
...
SQLite3_result *
MySQL_HostGroups_Manager::dump_table_mysql_replication_hostgroups()
{
  wrlock();
  char *          error= NULL;
  int             cols= 0;
                                                   ...
                                                   bool              _lock)
{
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              "Returning MySQL_Connection %p, server %s:%d with status %d\n",
  ...
  {
    proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                "Destroying MySQL_Connection %p, server %s:%d with status %d "
                ". largest_query_length = %lu\n",
                c, mysrvc->address, mysrvc->port, mysrvc->status,
                c->largest_query_length);
    delete c;
  ...
  if (mysrvc->status == MYSQL_SERVER_STATUS_ONLINE)
  {
    if (c->async_state_machine == ASYNC_IDLE)
    {
      if (c->local_stmts->get_num_backend_stmts() >
          (unsigned int)GloMTH->variables.max_stmts_per_connection)
      {
        proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                    "Destroying MySQL_Connection %p, server %s:%d with "
                    "status %d because has too many prepared statements\n",
                    c, mysrvc->address, mysrvc->port, mysrvc->status);
        //				delete c;
    ...
    {
      proxy_debug(
          PROXY_DEBUG_MYSQL_CONNPOOL, 7,
          "Destroying MySQL_Connection %p, server %s:%d with status %d\n", c,
          mysrvc->address, mysrvc->port, mysrvc->status);
      delete c;
    }
  }
  else
...
void MySQL_HostGroups_Manager::push_MyConn_to_pool_array(
    MySQL_Connection **ca)
{
  unsigned int      i= 0;
...
MySrvC *MyHGC::get_random_MySrvC()
{
  ...
  if (l)
  {
    ...
    for (j= 0; j < l; j++)
    {
      mysrvc= mysrvs->idx(j);
      if (mysrvc->status == MYSQL_SERVER_STATUS_ONLINE)
      {  // consider this server only if ONLINE
        if (mysrvc->ConnectionsUsed->conns_length() < mysrvc->max_connections)
        {  // consider this server only if didn't reach max_connections
          ...
          {  // consider the host only if not too far
            sum+= mysrvc->weight;
    ...
    if (sum == 0)
    {
      ...
      int max_wait_sec=
          (mysql_thread___shun_recovery_time_sec * 1000 >=
                   mysql_thread___connect_timeout_server_max
               ? mysql_thread___connect_timeout_server_max / 10000 - 1
               : mysql_thread___shun_recovery_time_sec / 10);
      if (max_wait_sec < 1)
      {  // min wait time should be at least 1 second
        max_wait_sec= 1;
      }
    ...
    if (sum == 0)
    {
      proxy_debug(
          PROXY_DEBUG_MYSQL_CONNPOOL, 7,
          "Returning MySrvC NULL because no backend ONLINE or with weight\n");
...
MySQL_Connection *MySrvConnList::get_random_MyConn(MySQL_Session *sess,
                                                   bool           ff)
{
  ...
  if (l && ff == false)
  {
        ...
        sess->client_myds->myconn->userinfo)
    {
      ...
      else
      {
        ...
        if ((conns_used > conns_free) &&
            (mysrvc->max_connections > (conns_free / 2 + conns_used / 2)))
        {
          conn= new MySQL_Connection();
          conn->parent= mysrvc;
          __sync_fetch_and_add(&MyHGM->status.server_connections_created, 1);
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                      "Returning MySQL Connection %p, server %s:%d\n", conn,
...
MySQL_Connection *MySQL_HostGroups_Manager::get_MyConn_from_pool(
    unsigned int _hid, MySQL_Session *sess, bool ff)
{
  MySQL_Connection *conn= NULL;
  wrlock();
  ...
  if (mysrvc)
  {  // a MySrvC exists. If not, we return NULL = no targets
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              "Returning MySQL Connection %p, server %s:%d\n", conn,
              (conn ? conn->parent->address : ""),
              (conn ? conn->parent->port : 0));
  return conn;
}
                                                        ...
                                                        bool _lock)
{
  ...
  if (mysrvc->status == MYSQL_SERVER_STATUS_ONLINE && c->send_quit &&
      queue.size() < __sync_fetch_and_add(
                         &GloMTH->variables.connpoll_reset_queue_length, 0))
  {
    if (c->async_state_machine == ASYNC_IDLE)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                  "Trying to reset MySQL_Connection %p, server %s:%d\n", c,
                  mysrvc->address, mysrvc->port);
      to_del= false;
    ...
    else
    {
      ...
      if (mysql_thread___kill_backend_connection_when_disconnect)
      {
        ...
        KillArgs *     ka= new KillArgs(ui->username, auth_password,
                                   c->parent->address, c->parent->port,
                                   c->mysql->thread_id, KILL_CONNECTION);
        pthread_attr_t attr;
  ...
  if (to_del)
  {
    ...
    proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                "Destroying MySQL_Connection %p, server %s:%d\n", c,
                mysrvc->address, mysrvc->port);
    mysrvc->ConnectionsUsed->remove(c);
...
{
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              "Adding MySrvC %p (%s:%d) for hostgroup %d\n", mysrvc,
              mysrvc->address, mysrvc->port, _hid);
  MyHGC *myhgc= MyHGC_lookup(_hid);
  myhgc->mysrvs->add(mysrvc);
}

void MySQL_HostGroups_Manager::replication_lag_action(
    ...
    for (j= 0; j < (int)myhgc->mysrvs->cnt(); j++)
    {
      ...
      if (strcmp(mysrvc->address, address) == 0 && mysrvc->port == port)
      {
        if (mysrvc->status == MYSQL_SERVER_STATUS_ONLINE)
        {
          ...
          {
            proxy_warning(
                "Shunning server %s:%d with replication lag of %d second\n",
        ...
        else
        {
          if (mysrvc->status == MYSQL_SERVER_STATUS_SHUNNED_REPLICATION_LAG)
          {
            ...
            )
            {
              mysrvc->status= MYSQL_SERVER_STATUS_ONLINE;
              proxy_warning(
                  "Re-enabling server %s:%d with replication lag of %d "
                  "second\n",
                  address, port, current_replication_lag);
            }
          }
...
void MySQL_HostGroups_Manager::drop_all_idle_connections()
{
  ...
  for (i= 0; i < (int)MyHostGroups->len; i++)
  {
    ...
    for (j= 0; j < (int)myhgc->mysrvs->cnt(); j++)
    {
      ...
      while (mscl->conns_length() > mysql_thread___free_connections_pct *
                                        mysrvc->max_connections / 100)
      {
 ...
 *   dropped as well
 */
int MySQL_HostGroups_Manager::get_multiple_idle_connections(
    int _hid, unsigned long long _max_last_time_used,
    MySQL_Connection **conn_list, int num_conn)
{
  wrlock();
...
__exit_get_multiple_idle_connections:
  ...
  wrunlock();
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              "Returning %d idle connections\n", num_conn_current);
  return num_conn_current;
}

void MySQL_HostGroups_Manager::set_incoming_replication_hostgroups(
    SQLite3_result *s)
{
  incoming_replication_hostgroups= s;
}

void MySQL_HostGroups_Manager::set_incoming_group_replication_hostgroups(
    SQLite3_result *s)
{
  if (incoming_group_replication_hostgroups)
  {
...
SQLite3_result *MySQL_HostGroups_Manager::SQL3_Connection_Pool(bool _reset)
{
  ...
  for (i= 0; i < (int)MyHostGroups->len; i++)
  {
    ...
    for (j= 0; j < (int)myhgc->mysrvs->cnt(); j++)
    {
      ...
      switch (mysrvc->status)
      {
        case 0: pta[3]= strdup("ONLINE"); break;
        case 1: pta[3]= strdup("SHUNNED"); break;
        case 2: pta[3]= strdup("OFFLINE_SOFT"); break;
        case 3: pta[3]= strdup("OFFLINE_HARD"); break;
        case 4: pta[3]= strdup("SHUNNED_REPLICATION_LAG"); break;
...
void MySQL_HostGroups_Manager::read_only_action(char *hostname, int port,
                                                int read_only)
{
  // define queries
  ...
  resultset= NULL;
  if (admindb == NULL)
  {  // we initialize admindb only if needed
      ...
      if (num_rows == 0)
      {
        ...
        if (GloMTH->variables.hostgroup_manager_verbose)
        {
          ...
          else
          {
            ...
            {
              proxy_info(
                  "read_only_action RO=0 phase 1 : Dumping mysql_servers for "
                  "%s:%d\n",
                  hostname, port);
        ...
        GloAdmin->save_mysql_servers_runtime_to_database(
            false);  // SAVE MYSQL SERVERS FROM RUNTIME
        if (GloMTH->variables.hostgroup_manager_verbose)
        {
          char *          error2= NULL;
          int             cols2= 0;
          ...
          else
          {
            ...
            {
              proxy_info(
                  "read_only_action RO=0 phase 2 : Dumping mysql_servers for "
                  "%s:%d\n",
                  hostname, port);
              resultset2->dump_to_stderr();
            }
        ...
        if (GloMTH->variables.hostgroup_manager_verbose)
        {
          ...
          else
          {
            ...
            {
              proxy_info(
                  "read_only_action RO=0 phase 3 : Dumping mysql_servers for "
                  "%s:%d\n",
                  hostname, port);
              resultset2->dump_to_stderr();
            }
        ...
        GloAdmin
            ->load_mysql_servers_to_runtime();  // LOAD MYSQL SERVERS TO RUNTIME
        GloAdmin->mysql_servers_wrunlock();
      }
      else
      {
             ...
             it != resultset->rows.end(); ++it)
        {
          SQLite3_row *r= *it;
          int          status=
              MYSQL_SERVER_STATUS_OFFLINE_HARD;  // default status, even for missing
          if (r->fields[1])
          {  // has status
        ...
        if (act == true)
        {  // there are servers either missing, or with stats=OFFLINE_HARD
          GloAdmin->mysql_servers_wrlock();
          if (GloMTH->variables.hostgroup_manager_verbose)
          {
            char *          error2= NULL;
            ...
            query2= (char *)malloc(strlen(q) + strlen(hostname) + 32);
            sprintf(query2, q, hostname, port);
            admindb->execute_statement(query2, &error2, &cols2,
                                       &affected_rows2, &resultset2);
            if (error2)
            {
              proxy_error("Error on read from mysql_servers : %s\n", error2);
            }
            else
            {
              ...
              {
                proxy_info(
                    "read_only_action RO=0 , rows=%d , phase 1 : Dumping "
                    "mysql_servers for %s:%d\n",
          ...
          if (GloMTH->variables.hostgroup_manager_verbose)
          {
            ...
            else
            {
              ...
              {
                proxy_info(
                    "read_only_action RO=0 , rows=%d , phase 2 : Dumping "
                    "mysql_servers for %s:%d\n",
                    num_rows, hostname, port);
                resultset2->dump_to_stderr();
              }
          ...
          if (GloMTH->variables.hostgroup_manager_verbose)
          {
            ...
            else
            {
              ...
              {
                proxy_info(
                    "read_only_action RO=0 , rows=%d , phase 3 : Dumping "
                    "mysql_servers for %s:%d\n",
                    num_rows, hostname, port);
                resultset2->dump_to_stderr();
              }
      ...
      if (num_rows)
      {
        ...
        if (GloMTH->variables.hostgroup_manager_verbose)
        {
          ...
          else
          {
            ...
            {
              proxy_info(
                  "read_only_action RO=1 phase 1 : Dumping mysql_servers for "
                  "%s:%d\n",
                  hostname, port);
              resultset2->dump_to_stderr();
            }
        ...
        if (GloMTH->variables.hostgroup_manager_verbose)
        {
          ...
          else
          {
            ...
            {
              proxy_info(
                  "read_only_action RO=1 phase 2 : Dumping mysql_servers for "
                  "%s:%d\n",
                  hostname, port);
              resultset2->dump_to_stderr();
            }
        ...
        if (GloMTH->variables.hostgroup_manager_verbose)
        {
          ...
          else
          {
            ...
            {
              proxy_info(
                  "read_only_action RO=1 phase 3 : Dumping mysql_servers for "
                  "%s:%d\n",
                  hostname, port);
              resultset2->dump_to_stderr();
            }
...
void MySQL_HostGroups_Manager::set_server_current_latency_us(
    char *hostname, int port, unsigned int _current_latency_us)
{
  wrlock();
  MySrvC *mysrvc= NULL;
  for (unsigned int i= 0; i < MyHostGroups->len; i++)
  ...
  for (unsigned int i= 0; i < MyHostGroups->len; i++)
  {
    ...
    if (l)
    {
      for (j= 0; j < l; j++)
      {
        ...
        mysrvc= myhgc->mysrvs->idx(j);
        intsize+= ((mysrvc->ConnectionsUsed->conns_length()) *
                   sizeof(MySQL_Connection *));
...
Group_Replication_Info::Group_Replication_Info(int w, int b, int r, int o,
                                               int mw, int mtb, bool _a,
                                               bool _w, char *c)
{
  comment= NULL;
  if (c)
...
bool Group_Replication_Info::update(int b, int r, int o, int mw, int mtb,
                                    bool _a, bool _w, char *c)
{
  bool ret= false;
  __active= true;
...
void MySQL_HostGroups_Manager::update_group_replication_set_offline(
    char *_hostname, int _port, int _writer_hostgroup, char *_error)
{
  int             cols= 0;
  int             affected_rows= 0;
  SQLite3_result *resultset= NULL;
  ...
  free(query);
  if (resultset)
  {  // we lock only if needed
    if (resultset->rows_count)
    {
      proxy_warning(
          "Group Replication: setting host %s:%d offline because: %s\n",
          _hostname, _port, _error);
      GloAdmin->mysql_servers_wrlock();
      mydb->execute("DELETE FROM mysql_servers_incoming");
      mydb->execute(
          "INSERT INTO mysql_servers_incoming SELECT hostgroup_id, hostname, "
          "port, weight, status, compression, max_connections, "
          "max_replication_lag, use_ssl, max_latency_ms, comment FROM "
          "mysql_servers");
      ...
      if (resultset2)
      {
        ...
        {
          for (std::vector<SQLite3_row *>::iterator it=
                   resultset2->rows.begin();
               it != resultset2->rows.end(); ++it)
          {
            SQLite3_row *r= *it;
            int          writer_hostgroup= atoi(r->fields[0]);
            int          backup_writer_hostgroup= atoi(r->fields[1]);
...
void MySQL_HostGroups_Manager::update_group_replication_set_read_only(
    char *_hostname, int _port, int _writer_hostgroup, char *_error)
{
  int             cols= 0;
  int             affected_rows= 0;
  ...
  {  // we lock only if needed
    ...
    {
      proxy_warning(
          "Group Replication: setting host %s:%d (part of cluster with "
          "writer_hostgroup=%d) in read_only because: %s\n",
...
void MySQL_HostGroups_Manager::update_group_replication_set_writer(
    char *_hostname, int _port, int _writer_hostgroup)
{
  int             cols= 0;
  int             affected_rows= 0;
  SQLite3_result *resultset= NULL;
  ...
  if (resultset)
  {
    ...
    if (need_converge == false)
    {
      if (found_writer)
      {  // maybe no-op
        if (writer_is_also_reader == found_reader)
      ...
      if (resultset2)
      {
        if (resultset2->rows_count)
        {
               ...
               it != resultset2->rows.end(); ++it)
          {
            ...
            q=(char *)"DELETE FROM mysql_servers WHERE hostgroup_id IN (%d , %d , %d , %d)";
            sprintf(query, q, _writer_hostgroup, backup_writer_hostgroup,
                    reader_hostgroup, offline_hostgroup);
            mydb->execute(query);
            generate_mysql_servers_table(&writer_hostgroup);
            generate_mysql_servers_table(&backup_writer_hostgroup);
...
void MySQL_HostGroups_Manager::converge_group_replication_config(
    int _writer_hostgroup)
{
  ...
  if (it2 != Group_Replication_Info_Map.end())
  {
    ...
    sprintf(query, q, info->writer_hostgroup, info->backup_writer_hostgroup,
            info->reader_hostgroup, info->offline_hostgroup);
    mydb->execute_statement(query, &error, &cols, &affected_rows, &resultset);
    free(query);
    if (resultset)
    {
      if (resultset->rows_count)
      {
        ...
        if (num_writers > info->max_writers)
        {  // there are more writers than allowed
          int to_move= num_writers - info->max_writers;
          proxy_info(
              "Group replication: max_writers=%d , moving %d nodes from "
              "writer HG %d to backup HG %d\n",
              info->max_writers, to_move, info->writer_hostgroup,
              info->backup_writer_hostgroup);
          for (std::vector<SQLite3_row *>::reverse_iterator it=
                   resultset->rows.rbegin();
               it != resultset->rows.rend(); ++it)
          {
            SQLite3_row *r= *it;
            if (to_move)
            {
              ...
              if (hostgroup == info->writer_hostgroup)
              {
                ...
                sprintf(query, q, info->backup_writer_hostgroup,
                        info->writer_hostgroup, r->fields[1],
                        atoi(r->fields[2]));
                mydb->execute(query);
                free(query);
                to_move--;
        ...
        {
          if (num_writers < info->max_writers && num_backup_writers)
          {  // or way too low writer
            int to_move=
                ((info->max_writers - num_writers) < num_backup_writers
                     ? (info->max_writers - num_writers)
                     : num_backup_writers);
            proxy_info(
                "Group replication: max_writers=%d , moving %d nodes from "
                "backup HG %d to writer HG %d\n",
                info->max_writers, to_move, info->backup_writer_hostgroup,
                info->writer_hostgroup);
            for (std::vector<SQLite3_row *>::iterator it=
                     resultset->rows.begin();
                 it != resultset->rows.end(); ++it)
            {
              SQLite3_row *r= *it;
              if (to_move)
              {
                ...
                if (hostgroup == info->backup_writer_hostgroup)
                {
                  q=(char *)"UPDATE OR REPLACE mysql_servers_incoming SET status=0, hostgroup_id=%d WHERE hostgroup_id=%d AND hostname='%s' AND port=%d";
                  query=
                      (char *)malloc(strlen(q) + strlen(r->fields[1]) + 128);
                  sprintf(query, q, info->writer_hostgroup,
                          info->backup_writer_hostgroup, r->fields[1],
                          atoi(r->fields[2]));
                  mydb->execute(query);
                  free(query);
                  to_move--;
