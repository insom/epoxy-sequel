...
#define SAFE_SQLITE3_STEP(_stmt)                 \
  ...
  {                                              \
    ...
    {                                            \
      ...
      if (rc != SQLITE_DONE)                     \
      {                                          \
        assert(rc == SQLITE_LOCKED) usleep(100); \
      }                                          \
...
ProxySQL_Statistics::ProxySQL_Statistics()
{
  ...
  statsdb_mem->open(
      (char *)"file:statsdb_mem?mode=memory&cache=shared",
...
void ProxySQL_Statistics::init()
{
  ...
  insert_into_tables_defs(tables_defs_statsdb_mem, "mysql_connections",
                          STATSDB_SQLITE_TABLE_MYSQL_CONNECTIONS);
  insert_into_tables_defs(tables_defs_statsdb_disk, "mysql_connections",
                          STATSDB_SQLITE_TABLE_MYSQL_CONNECTIONS);
  insert_into_tables_defs(tables_defs_statsdb_disk, "system_cpu",
                          STATSDB_SQLITE_TABLE_SYSTEM_CPU);
#ifndef NOJEM
  insert_into_tables_defs(tables_defs_statsdb_disk, "system_memory",
                          STATSDB_SQLITE_TABLE_SYSTEM_MEMORY);
#endif
  insert_into_tables_defs(tables_defs_statsdb_disk, "mysql_connections_hour",
                          STATSDB_SQLITE_TABLE_MYSQL_CONNECTIONS_HOUR);
  insert_into_tables_defs(tables_defs_statsdb_disk, "system_cpu_hour",
                          STATSDB_SQLITE_TABLE_SYSTEM_CPU_HOUR);
#ifndef NOJEM
  insert_into_tables_defs(tables_defs_statsdb_disk, "system_memory_hour",
                          STATSDB_SQLITE_TABLE_SYSTEM_MEMORY_HOUR);
#endif
  insert_into_tables_defs(tables_defs_statsdb_disk, "mysql_connections_day",
                          STATSDB_SQLITE_TABLE_MYSQL_CONNECTIONS_DAY);
  insert_into_tables_defs(tables_defs_statsdb_disk, "system_cpu_day",
                          STATSDB_SQLITE_TABLE_SYSTEM_CPU_DAY);
#ifndef NOJEM
  insert_into_tables_defs(tables_defs_statsdb_disk, "system_memory_day",
                          STATSDB_SQLITE_TABLE_SYSTEM_MEMORY_DAY);
#endif

  insert_into_tables_defs(tables_defs_statsdb_disk, "mysql_query_cache",
                          STATSDB_SQLITE_TABLE_MYSQL_QUERY_CACHE);
  insert_into_tables_defs(tables_defs_statsdb_disk, "mysql_query_cache_hour",
                          STATSDB_SQLITE_TABLE_MYSQL_QUERY_CACHE_HOUR);
  insert_into_tables_defs(tables_defs_statsdb_disk, "mysql_query_cache_day",
                          STATSDB_SQLITE_TABLE_MYSQL_QUERY_CACHE_DAY);
...
void ProxySQL_Statistics::check_and_build_standard_tables(
...
void ProxySQL_Statistics::insert_into_tables_defs(
...
void ProxySQL_Statistics::drop_tables_defs(
...
bool ProxySQL_Statistics::MySQL_Threads_Handler_timetoget(
    unsigned long long curtime)
{
  unsigned int i= (unsigned int)variables.stats_mysql_connections;
  if (i)
  {
    if ((curtime > next_timer_MySQL_Threads_Handler) ||
        (curtime + i * 1000 * 1000 < next_timer_MySQL_Threads_Handler))
    {
      next_timer_MySQL_Threads_Handler= curtime / 1000 / 1000 + i;
      next_timer_MySQL_Threads_Handler=
          next_timer_MySQL_Threads_Handler * 1000 * 1000;
      return true;
    }
  }
  return false;
...
bool ProxySQL_Statistics::MySQL_Query_Cache_timetoget(
    unsigned long long curtime)
{
  unsigned int i= (unsigned int)variables.stats_mysql_query_cache;
  if (i)
  {
    if ((curtime > next_timer_MySQL_Query_Cache) ||
        (curtime + i * 1000 * 1000 < next_timer_MySQL_Query_Cache))
    {
      next_timer_MySQL_Query_Cache= curtime / 1000 / 1000 + i;
      next_timer_MySQL_Query_Cache=
          next_timer_MySQL_Query_Cache * 1000 * 1000;
      return true;
    }
  }
  return false;
...
bool ProxySQL_Statistics::system_cpu_timetoget(unsigned long long curtime)
{
  ...
  if (i)
  {
    if ((curtime > next_timer_system_cpu) ||
...
bool ProxySQL_Statistics::system_memory_timetoget(unsigned long long curtime)
{
  ...
  if (i)
  {
    if ((curtime > next_timer_system_memory) ||
...
SQLite3_result *ProxySQL_Statistics::get_mysql_metrics(int interval)
{
  ...
  //fprintf(stderr,"%s\n", query);
  statsdb_disk->execute_statement(query, &error, &cols, &affected_rows,
                                  &resultset);
  free(query);
  if (error)
  {
...
SQLite3_result *ProxySQL_Statistics::get_MySQL_Query_Cache_metrics(
    int interval)
{
  SQLite3_result *resultset= NULL;
  int             cols;
...
void ProxySQL_Statistics::system_cpu_sets()
{
  ...
  if (times(&buf) > -1)
  {
    ...
    rc= sqlite3_bind_int64(statement1, 1, ts);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 2, buf.tms_utime);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 3, buf.tms_stime);
    assert(rc == SQLITE_OK);
    ...
    else
    {
      ...
      {
        sprintf(buf,
                "INSERT INTO system_cpu_hour SELECT timestamp/3600*3600 , "
                "SUM(tms_utime), SUM(tms_stime) FROM system_cpu WHERE "
                "timestamp < %ld GROUP BY timestamp/3600",
                (ts / 3600) * 3600);
      ...
      delete resultset;
      resultset= NULL;
      sprintf(buf, "DELETE FROM system_cpu WHERE timestamp < %ld",
              ts - 3600 * 24 * 7);
      statsdb_disk->execute(buf);
      sprintf(buf, "DELETE FROM system_cpu_hour WHERE timestamp < %ld",
              ts - 3600 * 24 * 365);
      statsdb_disk->execute(buf);
    }
  }
...
void ProxySQL_Statistics::system_memory_sets()
{
  ...
  if (times(&buf) > -1)
  {
    ...
    rc= sqlite3_bind_int64(statement1, 1, ts);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 2, allocated);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 3, resident);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 4, active);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 5, mapped);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 6, metadata);
    assert(rc == SQLITE_OK);
    rc= sqlite3_bind_int64(statement1, 7, retained);
    assert(rc == SQLITE_OK);
    ...
    else
    {
      ...
      {
        sprintf(buf,
                "INSERT INTO system_memory_hour SELECT timestamp/3600*3600 , "
                "AVG(allocated), AVG(resident), AVG(active), AVG(mapped), "
                "AVG(metadata), AVG(retained) FROM system_memory WHERE "
                "timestamp < %ld GROUP BY timestamp/3600",
                (ts / 3600) * 3600);
        statsdb_disk->execute(buf);
      }
      else
      {
        ...
        if (r->fields[0])
        {
          ...
          {
            sprintf(buf,
                    "INSERT INTO system_memory_hour SELECT "
                    "timestamp/3600*3600 , AVG(allocated), AVG(resident), "
                    "AVG(active), AVG(mapped), AVG(metadata), AVG(retained) "
                    "FROM system_memory WHERE timestamp >= %ld AND timestamp "
                    "< %ld GROUP BY timestamp/3600",
                    t + 3600, (ts / 3600) * 3600);
            statsdb_disk->execute(buf);
          }
        }
      ...
      delete resultset;
      resultset= NULL;
      sprintf(buf, "DELETE FROM system_memory WHERE timestamp < %ld",
              ts - 3600 * 24 * 7);
      statsdb_disk->execute(buf);
      sprintf(buf, "DELETE FROM system_memory_hour WHERE timestamp < %ld",
              ts - 3600 * 24 * 365);
      statsdb_disk->execute(buf);
    }
  }
...
void ProxySQL_Statistics::MySQL_Threads_Handler_sets(
    SQLite3_result *resultset)
{
  ...
  for (int i= 0; i < 12; i++)
  {
    rc= sqlite3_bind_int64(statement1, i + 1, mysql_connections_values[i]);
    assert(rc == SQLITE_OK);
  }
  ...
  else
  {
    ...
    if (resultset2->rows_count == 0)
    {
      sprintf(
          buf,
          "INSERT INTO mysql_connections_hour SELECT timestamp/3600*3600 , "
          "MAX(Client_Connections_aborted), "
          "AVG(Client_Connections_connected), "
          "MAX(Client_Connections_created), MAX(Server_Connections_aborted), "
          "AVG(Server_Connections_connected), "
          "MAX(Server_Connections_created), MAX(ConnPool_get_conn_failure), "
          "MAX(ConnPool_get_conn_immediate), MAX(ConnPool_get_conn_success), "
          "MAX(Questions), MAX(Slow_queries) FROM mysql_connections WHERE "
          "timestamp < %ld GROUP BY timestamp/3600",
          (ts / 3600) * 3600);
      statsdb_disk->execute(buf);
    }
    else
    {
      ...
      if (r->fields[0])
      {
        ...
        {
          sprintf(buf,
                  "INSERT INTO mysql_connections_hour SELECT "
                  "timestamp/3600*3600 , MAX(Client_Connections_aborted), "
                  "AVG(Client_Connections_connected), "
                  "MAX(Client_Connections_created), "
                  "MAX(Server_Connections_aborted), "
                  "AVG(Server_Connections_connected), "
                  "MAX(Server_Connections_created), "
                  "MAX(ConnPool_get_conn_failure), "
                  "MAX(ConnPool_get_conn_immediate), "
                  "MAX(ConnPool_get_conn_success), MAX(Questions), "
                  "MAX(Slow_queries) FROM mysql_connections WHERE timestamp "
                  ">= %ld AND timestamp < %ld GROUP BY timestamp/3600",
                  t + 3600, (ts / 3600) * 3600);
          statsdb_disk->execute(buf);
        }
      }
    ...
    delete resultset2;
    resultset2= NULL;
    sprintf(buf, "DELETE FROM mysql_connections WHERE timestamp < %ld",
            ts - 3600 * 24 * 7);
    statsdb_disk->execute(buf);
    sprintf(buf, "DELETE FROM mysql_connections_hour WHERE timestamp < %ld",
            ts - 3600 * 24 * 365);
    statsdb_disk->execute(buf);
  }
}
...
void ProxySQL_Statistics::MySQL_Query_Cache_sets(SQLite3_result *resultset)
{
  ...
  for (int i= 0; i < 9; i++)
  {
    rc= sqlite3_bind_int64(statement1, i + 1, qc_values[i]);
    assert(rc == SQLITE_OK);
  }
  ...
  else
  {
    ...
    if (resultset2->rows_count == 0)
    {
      sprintf(
          buf,
          "INSERT INTO mysql_query_cache_hour SELECT timestamp/3600*3600 , "
          "MAX(count_GET), MAX(count_GET_OK), MAX(count_SET), MAX(bytes_IN), "
          "MAX(bytes_OUT), MAX(Entries_Purged), AVG(Entries_In_Cache), "
          "AVG(Memory_bytes) FROM mysql_query_cache WHERE timestamp < %ld "
    ...
    else
    {
      ...
      if (r->fields[0])
      {
        ...
        {
          sprintf(buf,
                  "INSERT INTO mysql_query_cache_hour SELECT "
                  "timestamp/3600*3600 , MAX(count_GET), MAX(count_GET_OK), "
                  "MAX(count_SET), MAX(bytes_IN), MAX(bytes_OUT), "
                  "MAX(Entries_Purged), AVG(Entries_In_Cache), "
                  "AVG(Memory_bytes) FROM mysql_query_cache WHERE timestamp "
                  ">= %ld AND timestamp < %ld GROUP BY timestamp/3600",
                  t + 3600, (ts / 3600) * 3600);
          statsdb_disk->execute(buf);
        }
      ...
      else
      {
        sprintf(
            buf,
            "INSERT INTO mysql_query_cache_hour SELECT timestamp/3600*3600 , "
            "MAX(count_GET), MAX(count_GET_OK), MAX(count_SET), "
            "MAX(bytes_IN), MAX(bytes_OUT), MAX(Entries_Purged), "
            "AVG(Entries_In_Cache), AVG(Memory_bytes) FROM mysql_query_cache "
            "WHERE timestamp < %ld GROUP BY timestamp/3600",
            (ts / 3600) * 3600);
        statsdb_disk->execute(buf);
      }
    }
    delete resultset2;
    resultset2= NULL;
    sprintf(buf, "DELETE FROM mysql_query_cache WHERE timestamp < %ld",
            ts - 3600 * 24 * 7);
    statsdb_disk->execute(buf);
    sprintf(buf, "DELETE FROM mysql_query_cache_hour WHERE timestamp < %ld",
            ts - 3600 * 24 * 365);
    statsdb_disk->execute(buf);
  }
}
