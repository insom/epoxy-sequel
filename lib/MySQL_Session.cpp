...
#define PROXYSQL_VERSION_COMMENT                                             \
  "\x01\x00\x00\x01\x01\x27\x00\x00\x02\x03\x64\x65\x66\x00\x00\x00\x11\x40" \
  "\x40\x76\x65\x72\x73\x69\x6f\x6e\x5f\x63\x6f\x6d\x6d\x65\x6e\x74\x00\x0c" \
  "\x21\x00\x18\x00\x00\x00\xfd\x00\x00\x1f\x00\x00\x05\x00\x00\x03\xfe\x00" \
  "\x00\x02\x00\x0b\x00\x00\x04\x0a(ProxySQL)"                               \
...
extern CHARSET_INFO *      proxysql_find_charset_collate_names(
          const char *csname, const char *collatename);
...
void *kill_query_thread(void *arg)
{
  ...
  if (ka->port)
  {
    ...
    ret= mysql_real_connect(mysql, ka->hostname, ka->username, ka->password,
                            NULL, ka->port, NULL, 0);
  }
  else
  {
    ...
    ret= mysql_real_connect(mysql, "localhost", ka->username, ka->password,
                            NULL, 0, ka->hostname, 0);
  }
  if (!ret)
  {
    goto __exit_kill_query_thread;
  ...
  switch (ka->kill_type)
  {
    case KILL_QUERY: sprintf(buf, "KILL QUERY %lu", ka->id); break;
    case KILL_CONNECTION: sprintf(buf, "KILL CONNECTION %lu", ka->id); break;
    default: sprintf(buf, "KILL %lu", ka->id); break;
  }
...
void Query_Info::end()
{
  ...
  if ((end_time - start_time) >
      (unsigned int)mysql_thread___long_query_time * 1000)
  {
  ...
  if (stmt_meta)
  {  // fix bug #796: memory is not freed in case of error during STMT_EXECUTE
    if (stmt_meta->pkt)
    {
...
{
  GloQPro->query_parser_init(&QueryParserArgs, (char *)QueryPointer,
                             QueryLength, 0);
}
...
unsigned long long Query_Info::query_parser_update_counters()
{
  ...
  if (MyComQueryCmd == MYSQL_COM_QUERY___NONE)
    return 0;  // this means that it was never initialized
  if (MyComQueryCmd == MYSQL_COM_QUERY__UNINITIALIZED)
    return 0;  // this means that it was never initialized
  unsigned long long ret= GloQPro->query_parser_update_counters(
      sess, MyComQueryCmd, &QueryParserArgs, end_time - start_time);
  MyComQueryCmd= MYSQL_COM_QUERY___NONE;
  QueryPointer= NULL;
  QueryLength= 0;
  return ret;
...
{
  if (stmt_info)
  {  // we are processing a prepared statement. We already have the information
...
void *MySQL_Session::operator new(size_t size) { return l_alloc(size); }
...
MySQL_Session::~MySQL_Session()
{
  ...
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p -- Shutdown Session %p\n", this->thread,
              this, this);
  delete command_counters;
  if (session_type == PROXYSQL_SESSION_MYSQL &&
      connections_handler == false && mirror == false)
  {
...
*/
  ...
  if (mirror)
  {
    __sync_sub_and_fetch(&GloMTH->status_variables.mirror_sessions_current,
                         1);
  }
}
...
MySQL_Backend *MySQL_Session::create_backend(int                hostgroup_id,
                                             MySQL_Data_Stream *_myds)
{
...
MySQL_Backend *MySQL_Session::find_or_create_backend(int hostgroup_id,
                                                     MySQL_Data_Stream *_myds)
{
...
void MySQL_Session::writeout()
{
  ...
  int                mwpl=
      mysql_thread___throttle_max_bytes_per_second_to_client;  // max writes per call
  mwpl= mwpl / tps;
  if (client_myds)
      ...
      mybe->server_myds->myds_type == MYDS_BACKEND)
  {
    if (session_type == PROXYSQL_SESSION_MYSQL)
    {
      ...
      {
        if (mybe->server_myds->poll_fds_idx > -1)
        {  // NOTE: attempt to force writes
  ...
  if (client_myds && thread->curtime >= client_myds->pause_until)
  {
    if (mirror == false)
    {
      ...
      {
        last_sent_=
            client_myds->mypolls->last_sent[client_myds->poll_fds_idx];
      }
      int retbytes= client_myds->write_to_net_poll();
      total_written+= retbytes;
      if (retbytes == QUEUE_T_DEFAULT_SIZE)
      {  // optimization to solve memory bloat
        runloop= true;
      }
  ...
  if (total_written > 0)
  {
    ...
    {
      unsigned long long add_=
          1000000 / tps +
          1000000 / tps *
              ((unsigned long long)total_written - (unsigned long long)mwpl) /
              mwpl;
    ...
    else
    {
      if (total_written >= QUEUE_T_DEFAULT_SIZE)
      {
        ...
        if (time_diff == 0)
        {  // sending data really too fast!
          unsigned long long add_=
              1000000 / tps + 1000000 / tps *
                                  ((unsigned long long)total_written -
                                   (unsigned long long)mwpl) /
                                  mwpl;
          pause_until= thread->curtime + add_;
          client_myds->remove_pollout();
          client_myds->pause_until= thread->curtime + add_;
  ...
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p -- Writeout Session %p\n", this->thread,
              this, this);
}
...
bool MySQL_Session::handler_CommitRollback(PtrSize_t *pkt)
{
  ...
  else
  {
    ...
    client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 1, 0, 0, setStatus,
                                        0, NULL);
  ...
  if (pkt->size >= 7 + sal)
  {
    if (strncasecmp((char *)"set autocommit", (char *)pkt->ptr + 5, sal) == 0)
    {
      ...
      __ret_autocommit_OK:
        ...
        if (autocommit)
          setStatus+= SERVER_STATUS_AUTOCOMMIT;
        client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 1, 0, 0,
                                            setStatus, 0, NULL);
        client_myds->DSS= STATE_SLEEP;
        status= WAITING_CLIENT_DATA;
        l_free(pkt->size, pkt->ptr);
...
bool MySQL_Session::handler_special_queries(PtrSize_t *pkt)
{
  ...
  {
    if (pkt->size > (5 + 4) &&
        strncasecmp((char *)"USE ", (char *)pkt->ptr + 5, 4) == 0)
    {
      handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___MYSQL_COM_QUERY_USE_DB(
          pkt);
      return true;
    }
  }
...
*/
  if (pkt->size == SELECT_VERSION_COMMENT_LEN + 5 &&
      strncmp((char *)SELECT_VERSION_COMMENT, (char *)pkt->ptr + 5,
              pkt->size - 5) == 0)
  {
  ...
  if (pkt->size == strlen((char *)"select USER()") + 5 &&
      strncmp((char *)"select USER()", (char *)pkt->ptr + 5, pkt->size - 5) ==
          0)
  {
    // FIXME: this doesn't return AUTOCOMMIT or IN_TRANS
    char *query1= (char *)"SELECT \"%s\" AS 'USER()'";
    char *query2=
        (char *)malloc(strlen(query1) +
                       strlen(client_myds->myconn->userinfo->username) + 10);
    sprintf(query2, query1, client_myds->myconn->userinfo->username);
    char *          error;
    int             cols;
    int             affected_rows;
    SQLite3_result *resultset;
    GloAdmin->admindb->execute_statement(query2, &error, &cols,
  ...
  if ((pkt->size < 60) && (pkt->size > 38) &&
      (strncasecmp((char *)"SET SESSION character_set_server",
                   (char *)pkt->ptr + 5, 32) == 0))
  {  // issue #601
    char *idx= NULL;
    ...
    {  // we found =
      PtrSize_t pkt_2;
      pkt_2.size= 5 + strlen((char *)"SET NAMES ") + pkt->size - 1 -
                  (idx - (char *)pkt->ptr);
      pkt_2.ptr= l_alloc(pkt_2.size);
      mysql_hdr Hdr;
      ...
      memcpy((char *)pkt_2.ptr + 15, idx + 1,
             pkt->size - 1 - (idx - (char *)pkt->ptr));
      l_free(pkt->size, pkt->ptr);
      pkt->size= pkt_2.size;
      pkt->ptr= pkt_2.ptr;
    }
  }
  if ((pkt->size < 100) && (pkt->size > 15) &&
      (strncasecmp((char *)"SET NAMES ", (char *)pkt->ptr + 5, 10) == 0))
  {
    ...
    if (strcasestr(csname, " COLLATE "))
    {
      ...
      collation_name_unstripped=
          strcasestr(csname, " COLLATE ") + strlen(" COLLATE ");
      collation_name=
          trim_spaces_and_quotes_in_place(collation_name_unstripped);
    ...
    if (!c)
    {
      ...
      if (collation_specified)
      {
        ...
        errmsg= (char *)malloc(strlen(csname) + strlen(collation_name) +
                               strlen(m));
      ...
      client_myds->myprot.generate_pkt_ERR(true, NULL, NULL, 1, 1115,
                                           (char *)"42000", errmsg);
      free(errmsg);
    }
    else
  ...
  if ((pkt->size == 18) &&
      (strncasecmp((char *)"SHOW WARNINGS", (char *)pkt->ptr + 5, 13) == 0))
  {
...
{
  if (pktH->size < 15 * 1024 * 1024 &&
      (qpo->mirror_hostgroup >= 0 || qpo->mirror_flagOUT >= 0))
  {
    // check if there are too many mirror sessions in queue
    if (thread->mirror_queue_mysql_sessions->len >=
        (unsigned int)mysql_thread___mirror_max_queue_length)
    {
      return;
    }
    ...
    if (thread->mirror_queue_mysql_sessions_cache->len == 0)
    {
      ...
      {
        newsess->thread_session_id=
            __sync_fetch_and_add(&glovars.thread_id, 1);
      }
      ...
      newsess->client_myds->myprot.init(
          &newsess->client_myds, newsess->client_myds->myconn->userinfo,
          newsess);
      newsess->mirror= true;
    ...
    else
    {
      newsess=
          (MySQL_Session *)
              thread->mirror_queue_mysql_sessions_cache->remove_index_fast(0);
    }
    newsess->client_myds->myconn->userinfo->set(
        client_myds->myconn->userinfo);
    newsess->to_process= 1;
    newsess->default_hostgroup= default_hostgroup;
    if (qpo->mirror_hostgroup >= 0)
    {
      newsess->mirror_hostgroup=
          qpo->mirror_hostgroup;  // in the new session we copy the mirror hostgroup
    }
    else
    {
      newsess->mirror_hostgroup= default_hostgroup;  // copy the default
    }
    newsess->mirror_flagOUT=
        qpo->mirror_flagOUT;  // in the new session we copy the mirror flagOUT
    ...
    if (thread->mirror_queue_mysql_sessions->len == 0)
    {
      ...
      else
      {
        ...
        {  // the mirror session has completed
          ...
          if (thread->mirror_queue_mysql_sessions->len * 0.3 > l)
            l= thread->mirror_queue_mysql_sessions->len * 0.3;
...
int MySQL_Session::handler_again___status_PINGING_SERVER()
{
  ...
  if (rc == 0)
  {
    ...
    if (mysql_thread___multiplexing && (myconn->reusable == true) &&
        myds->myconn->IsActiveTransaction() == false &&
  ...
  else
  {
    if (rc == -1 || rc == -2)
    {
      ...
      {  // rc==-1
        ...
        proxy_error(
            "Detected a broken connection during ping on (%d,%s,%d) , FD "
            "(Conn:%d , MyDS:%d) : %d, %s\n",
            myconn->parent->myhgc->hid, myconn->parent->address,
            myconn->parent->port, myds->fd, myds->myconn->fd, myerr,
    ...
    else
    {
      ...
      {
        thread->mypolls.add(POLLIN | POLLOUT, myds->fd, myds,
                            thread->curtime);
      }
      // tring to fix bug
    }
...
void MySQL_Session::handler_again___new_thread_to_kill_connection()
{
  ...
  if (myds->myconn && myds->myconn->mysql)
  {
    if (myds->killed_at == 0)
    {
      ...
      {
        if (ui->password[0] == '*')
        {  // we don't have the real password, let's pass sha1
      ...
      KillArgs *     ka= new KillArgs(ui->username, auth_password,
                                 myds->myconn->parent->address,
                                 myds->myconn->parent->port,
                                 myds->myconn->mysql->thread_id, KILL_QUERY);
      pthread_attr_t attr;
...
#define NEXT_IMMEDIATE_NEW(new_st) \
  do                               \
  {                                \
    set_status(new_st);            \
    return true;                   \
...
{
  if (client_myds->myconn->options.charset !=
      mybe->server_myds->myconn->mysql->charset->nr)
  {
    //previous_status.push(PROCESSING_QUERY);
    switch (status)
    {  // this switch can be replaced with a simple previous_status.push(status), but it is here for readibility
      case PROCESSING_QUERY: previous_status.push(PROCESSING_QUERY); break;
...
bool MySQL_Session::handler_again___verify_backend_sql_mode()
{
  if (mybe->server_myds->myconn->options.sql_mode_int == 0)
  {
    ...
    mybe->server_myds->myconn->options.sql_mode=
        strdup(mysql_thread___default_sql_mode);
    uint32_t sql_mode_int= SpookyHash::Hash32(
        mybe->server_myds->myconn->options.sql_mode,
        strlen(mybe->server_myds->myconn->options.sql_mode), 10);
    ...
    {
      {
        ...
        if (mybe->server_myds->myconn->options.sql_mode)
        {
          ...
          {
            mybe->server_myds->myconn->options.sql_mode=
                strdup(client_myds->myconn->options.sql_mode);
          }
        }
      }
      switch (status)
...
bool MySQL_Session::handler_again___verify_backend_time_zone()
{
  if (mybe->server_myds->myconn->options.time_zone_int == 0)
  {
    ...
    uint32_t time_zone_int= SpookyHash::Hash32(
        mybe->server_myds->myconn->options.time_zone,
        strlen(mybe->server_myds->myconn->options.time_zone), 10);
    ...
    {
      {
        ...
        if (mybe->server_myds->myconn->options.time_zone)
        {
          ...
          {
            mybe->server_myds->myconn->options.time_zone=
                strdup(client_myds->myconn->options.time_zone);
          }
        }
      }
      switch (status)
...
bool MySQL_Session::handler_again___verify_init_connect()
{
  if (mybe->server_myds->myconn->options.init_connect_sent == false)
  {
    ...
    if (mysql_thread___init_connect)
    {
      ...
      mybe->server_myds->myconn->options.init_connect=
          strdup(mysql_thread___init_connect);
      switch (status)
      {  // this switch can be replaced with a simple previous_status.push(status), but it is here for readibility
        case PROCESSING_QUERY: previous_status.push(PROCESSING_QUERY); break;
        case PROCESSING_STMT_PREPARE:
...
{
  MySQL_Data_Stream *myds= mybe->server_myds;
  if (client_myds->myconn->userinfo->hash !=
      mybe->server_myds->myconn->userinfo->hash)
  {
    if (strcmp(client_myds->myconn->userinfo->username,
               myds->myconn->userinfo->username))
    {
      //previous_status.push(PROCESSING_QUERY);
      switch (status)
      {  // this switch can be replaced with a simple previous_status.push(status), but it is here for readibility
    ...
    if (strcmp(client_myds->myconn->userinfo->schemaname,
               myds->myconn->userinfo->schemaname))
    {
      //previous_status.push(PROCESSING_QUERY);
      switch (status)
      {  // this switch can be replaced with a simple previous_status.push(status), but it is here for readibility
...
bool MySQL_Session::handler_again___status_SETTING_INIT_CONNECT(int *_rc)
{
  ...
  if (myds->mypolls == NULL)
  {
    thread->mypolls.add(POLLIN | POLLOUT, mybe->server_myds->fd,
                        mybe->server_myds, thread->curtime);
  }
  int rc= myconn->async_send_simple_command(
      myds->revents, myconn->options.init_connect,
      strlen(myconn->options.init_connect));
  if (rc == 0)
  {
    myds->revents|=
        POLLOUT;  // we also set again POLLOUT to send a query immediately!
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        proxy_error(
            "Detected a broken connection while setting INIT CONNECT on %s , "
            "%d : %d, %s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        //if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status & SERVER_STATUS_IN_TRANS)==0)) {
        if ((myds->myconn->reusable == true) &&
            myds->myconn->IsActiveTransaction() == false &&
            myds->myconn->MultiplexDisabled() == false)
        {
          retry_conn= true;
      ...
      else
      {
        ...
        client_myds->myprot.generate_pkt_ERR(
            true, NULL, NULL, 1, mysql_errno(myconn->mysql), sqlstate,
            mysql_error(myconn->mysql));
        myds->destroy_MySQL_Connection_From_Pool(true);
        myds->fd= 0;
        status= WAITING_CLIENT_DATA;
...
bool MySQL_Session::handler_again___status_SETTING_SQL_LOG_BIN(int *_rc)
{
  ...
  int rc=
      myconn->async_send_simple_command(myds->revents, query, query_length);
  if (query)
  {
    free(query);
    query= NULL;
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection while setting SQL_LOG_BIN on %s , "
            "%d : %d, %s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        //if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status & SERVER_STATUS_IN_TRANS)==0)) {
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection while setting SQL_MODE on %s , %d "
            ": %d, %s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        //if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status & SERVER_STATUS_IN_TRANS)==0)) {
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection while setting TIME_ZONE on %s , %d "
            ": %d, %s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        //if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status & SERVER_STATUS_IN_TRANS)==0)) {
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection during INIT_DB on %s , %d : %d, "
            "%s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        //if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status & SERVER_STATUS_IN_TRANS)==0)) {
...
bool MySQL_Session::handler_again___status_CONNECTING_SERVER(int *_rc)
{
  ...
  if (mirror)
  {
    ...
    mybe->server_myds->wait_until=
        thread->curtime + mysql_thread___connect_timeout_server * 1000;
    pause_until= 0;
  }
  if (mybe->server_myds->max_connect_time)
  {
    if (thread->curtime >= mybe->server_myds->max_connect_time)
    {
      ...
      char buf[256];
      sprintf(buf,
              "Max connect timeout reached while reaching hostgroup %d after "
              "%llums",
              current_hostgroup,
              (thread->curtime - CurrentQuery.start_time) / 1000);
      client_myds->myprot.generate_pkt_ERR(true, NULL, NULL, 1, 9001,
                                           (char *)"HY000", buf);
  ...
  {
    pause_until=
        thread->curtime + mysql_thread___connect_retries_delay * 1000;
    *_rc= 1;
    return false;
  }
  else
    ...
    switch (rc)
    {
      ...
      case -2:
        ...
        __exit_handler_again___status_CONNECTING_SERVER_with_err:
          ...
          {
            char buf[256];
            sprintf(buf, "Max connect failure while reaching hostgroup %d",
                    current_hostgroup);
            client_myds->myprot.generate_pkt_ERR(true, NULL, NULL, 1, 9002,
                                                 (char *)"HY000", buf);
          }
...
bool MySQL_Session::handler_again___status_CHANGING_USER_SERVER(int *_rc)
{
  ...
  myconn->local_stmts=
      new MySQL_STMTs_local_v14(false);  // false by default, it is a backend
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection during change user on %s, %d : %d, "
            "%s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        if ((myds->myconn->reusable == true) &&
...
bool MySQL_Session::handler_again___status_CHANGING_CHARSET(int *_rc)
{
  ...
  int rc= myconn->async_set_names(myds->revents,
                                  client_myds->myconn->options.charset);
  if (rc == 0)
  {
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection during SET NAMES on %s , %d : %d, "
            "%s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        if ((myds->myconn->reusable == true) &&
  ...
  else
  {
    if (rc == -1)
    {
      ...
      if (myerr > 2000)
      {
        ...
        // client error, serious
        proxy_error(
            "Detected a broken connection during SET AUTOCOMMIT on %s , %d : "
            "%d, %s\n",
            myconn->parent->address, myconn->parent->port, myerr,
            mysql_error(myconn->mysql));
        if ((myds->myconn->reusable == true) &&
...
int MySQL_Session::handler()
{
  ...
  if (to_process == 0)
    return 0;  // this should be redundant if the called does the same check
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p -- Processing session %p\n",
  ...
  if (session_fast_forward == false)
  {
    if (client_myds == NULL)
    {
      ...
      proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                  "Processing session %p without client_myds\n", this);
      assert(mybe);
    ...
    else
    {
      ...
      {
        if (mirrorPkt.ptr)
        {  // this is the first time we call handler()
...
__get_pkts_from_client:
  ...
  for (j= 0; j < (client_myds->PSarrayIN ? client_myds->PSarrayIN->len : 0) ||
             (mirror == true && status == WAITING_CLIENT_DATA);)
  {
    if (mirror == false)
    {
    ...
    switch (status)
    {
        ...
        if (pkt.size == (0xFFFFFF + sizeof(mysql_hdr)))
        {
          ...
          {  // real traffic only
            case STATE_SLEEP:
              ...
              break;
            case STATE_SLEEP_MULTI_PACKET: break;
            default: assert(0); break;
          }
        }
        switch (client_myds->DSS)
        {
          case STATE_SLEEP_MULTI_PACKET:
            ...
            else
            {
              ...
              client_myds->multi_pkt.size=
                  pkt.size + tmp_pkt.size - sizeof(mysql_hdr);
              client_myds->multi_pkt.ptr=
                  l_alloc(client_myds->multi_pkt.size);
              memcpy(client_myds->multi_pkt.ptr, tmp_pkt.ptr, tmp_pkt.size);
              memcpy(
                  (char *)client_myds->multi_pkt.ptr + tmp_pkt.size,
                  (char *)pkt.ptr + sizeof(mysql_hdr),
                  pkt.size - sizeof(mysql_hdr));  // the header is not copied
            ...
            if (client_myds->DSS !=
                STATE_SLEEP)  // if DSS==STATE_SLEEP , we continue
              break;
          case STATE_SLEEP:  // only this section can be executed ALSO by mirror
            ...
            proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                        "Statuses: WAITING_CLIENT_DATA - STATE_SLEEP\n");
            if (session_fast_forward == true)
            {  // if it is fast forward
              mybe=
                  find_or_create_backend(current_hostgroup);  // set a backend
              mybe->server_myds
                  ->reinit_queues();  // reinitialize the queues in the myds . By default, they are not active
              mybe->server_myds->PSarrayOUT->add(
                  pkt.ptr, pkt.size);  // move the first packet
              previous_status.push(
                  FAST_FORWARD);  // next status will be FAST_FORWARD . Now we need a connection
              NEXT_IMMEDIATE(
                  CONNECTING_SERVER);  // we create a connection . next status will be FAST_FORWARD
            }
            ...
            if (session_type == PROXYSQL_SESSION_CLICKHOUSE)
            {
              if ((enum_mysql_command)c == _MYSQL_COM_INIT_DB)
              {
                ...
                *_c= ' ';
                ...
                memcpy((char *)_new_pkt.ptr + 9, (char *)pkt.ptr + 5,
                       pkt.size - 5);
                l_free(pkt.size, pkt.ptr);
                pkt.size+= 4;
            ...
            switch ((enum_mysql_command)c)
            {
              case _MYSQL_COM_QUERY:
                ...
                if (session_type == PROXYSQL_SESSION_MYSQL)
                {
                  ...
                  if (session_fast_forward == false)
                  {
                    ...
                    CurrentQuery.begin((unsigned char *)pkt.ptr, pkt.size,
                                       true);
                  }
                  ...
                  qpo= GloQPro->process_mysql_query(this, pkt.ptr, pkt.size,
                                                    &CurrentQuery);
                  if (thread->variables.stats_time_query_processor)
                  {
                  ...
                  assert(
                      qpo);  // GloQPro->process_mysql_query() should always return a qpo
                  rc_break=
                  ...
                  // set query retries
                  mybe->server_myds->query_retries_on_failure=
                  ...
                  if (mysql_thread___default_query_delay)
                  {
                    pause_until= thread->curtime +
                                 mysql_thread___default_query_delay * 1000;
                  }
                  if (qpo)
                  {
                    if (qpo->delay > 0)
                  ...
                  proxy_debug(PROXY_DEBUG_MYSQL_COM, 5,
                              "Received query to be processed with MariaDB "
                              "Client library\n");
              ...
              case _MYSQL_COM_CHANGE_USER:
                handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___MYSQL_COM_CHANGE_USER(
                    &pkt, &wrong_pass);
                break;
              case _MYSQL_COM_STMT_RESET:
              {
                uint32_t stmt_global_id= 0;
                memcpy(&stmt_global_id, (char *)pkt.ptr + 5,
                       sizeof(uint32_t));
              ...
              case _MYSQL_COM_STMT_SEND_LONG_DATA:
              {
                ...
                SLDH->add(stmt_global_id, stmt_param_id, (char *)pkt.ptr + 11,
                          pkt.size - 11);
              }
                client_myds->DSS= STATE_SLEEP;
                status= WAITING_CLIENT_DATA;
              ...
              case _MYSQL_COM_STMT_PREPARE:
                if (session_type != PROXYSQL_SESSION_MYSQL)
                {  // only MySQL module supports prepared statement!!
                  l_free(pkt.size, pkt.ptr);
                  client_myds->setDSS_STATE_QUERY_SENT_NET();
                  client_myds->myprot.generate_pkt_ERR(
                      true, NULL, NULL, 1, 1045, (char *)"#28000",
                      (char *)"Command not supported");
                  client_myds->DSS= STATE_SLEEP;
                  status= WAITING_CLIENT_DATA;
                  break;
                }
                else
                {
                  ...
                  {
                    client_myds->myconn->local_stmts=
                        new MySQL_STMTs_local_v14(true);
                  }
                  uint64_t hash=
                      client_myds->myconn->local_stmts->compute_hash(
                          current_hostgroup,
                  ...
                  GloMyStmt->wrlock();
                  stmt_info=
                      GloMyStmt->find_prepared_statement_by_hash(hash, false);
                  if (stmt_info)
                  {
                    ...
                    uint32_t new_stmt_id= client_myds->myconn->local_stmts
                                              ->generate_new_client_stmt_id(
                                                  stmt_info->statement_id);
                    l_free(pkt.size, pkt.ptr);
                    client_myds->setDSS_STATE_QUERY_SENT_NET();
                    client_myds->myprot.generate_STMT_PREPARE_RESPONSE(
                        client_myds->pkt_sid + 1, stmt_info, new_stmt_id);
                    client_myds->DSS= STATE_SLEEP;
                    status= WAITING_CLIENT_DATA;
                  ...
                  else
                  {
                    ...
                    mybe->server_myds->killed_at= 0;
                    mybe->server_myds->kill_type= 0;
                    mybe->server_myds->mysql_real_query.init(
                        &pkt);  // fix memory leak for PREPARE in prepared statements #796
                    client_myds->setDSS_STATE_QUERY_SENT_NET();
                  }
                  GloMyStmt->unlock();
              ...
              case _MYSQL_COM_STMT_EXECUTE:
                ...
                else
                {
                  ...
                  uint64_t stmt_global_id= 0;
                  memcpy(&client_stmt_id, (char *)pkt.ptr + 5,
                         sizeof(uint32_t));
                  stmt_global_id=
                      client_myds->myconn->local_stmts
                          ->find_global_stmt_id_from_client(client_stmt_id);
                  if (stmt_global_id == 0)
                  {
                  ...
                  if (stmt_info == NULL)
                  {
                    ...
                    client_myds->myprot.generate_pkt_ERR(
                        true, NULL, NULL, 1, 1045, (char *)"#28000",
                        (char *)"Prepared statement doesn't exist");
                    client_myds->DSS= STATE_SLEEP;
                    status= WAITING_CLIENT_DATA;
                    break;
                  }
                  ...
                  bool stmt_meta_found=
                      true;  // let's be optimistic and we assume we will found it
                  stmt_execute_metadata_t *stmt_meta=
                      sess_STMTs_meta->find(stmt_global_id);
                  if (stmt_meta == NULL)
                  {  // we couldn't find any metadata
                    stmt_meta_found= false;
                  }
                  stmt_meta= client_myds->myprot.get_binds_from_pkt(
                      pkt.ptr, pkt.size, stmt_info, &stmt_meta);
                  if (stmt_meta == NULL)
                  {
                    l_free(pkt.size, pkt.ptr);
                    client_myds->setDSS_STATE_QUERY_SENT_NET();
                    client_myds->myprot.generate_pkt_ERR(
                        true, NULL, NULL, 1, 1045, (char *)"#28000",
                        (char *)"Error in prepared statement execution");
                    client_myds->DSS= STATE_SLEEP;
                    status= WAITING_CLIENT_DATA;
              ...
              case _MYSQL_COM_PING:
                handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___MYSQL_COM_PING(
                    &pkt);
                break;
              case _MYSQL_COM_SET_OPTION:
                handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___MYSQL_COM_SET_OPTION(
                    &pkt);
                break;
              ...
              case _MYSQL_COM_INIT_DB:
                handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___MYSQL_COM_INIT_DB(
                    &pkt);
                break;
              ...
              case _MYSQL_COM_PROCESS_KILL:
                handler___status_WAITING_CLIENT_DATA___STATE_SLEEP___MYSQL_COM_PROCESS_KILL(
                    &pkt);
                break;
              default:
                proxy_error(
                    "RECEIVED AN UNKNOWN COMMAND: %d -- PLEASE REPORT A "
                    "BUG\n",
                    c);
                l_free(pkt.size, pkt.ptr);
          ...
          default:
            proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                        "Statuses: WAITING_CLIENT_DATA - STATE_UNKNOWN\n");
            {
              if (mirror == false)
              {
                ...
                switch (client_myds->client_addr->sa_family)
                {
                  case AF_INET:
                  {
                    struct sockaddr_in *ipv4=
                        (struct sockaddr_in *)client_myds->client_addr;
                    inet_ntop(client_myds->client_addr->sa_family,
                  ...
                  case AF_INET6:
                  {
                    struct sockaddr_in6 *ipv6=
                        (struct sockaddr_in6 *)client_myds->client_addr;
                    inet_ntop(client_myds->client_addr->sa_family,
                              &ipv6->sin6_addr, buf, INET6_ADDRSTRLEN);
                    break;
                  }
                  default: sprintf(buf, "localhost"); break;
                }
                proxy_error(
                    "Unexpected packet from client %s . Session_status: %d , "
                    "client_status: %d Disconnecting it\n",
      ...
      default:
        proxy_error(
            "Unexpected packet from client, disconnecting the client\n");
        handler_ret= -1;
        return handler_ret;
        break;
    }
...
handler_again:
  ...
  switch (status)
  {
    ...
    case PINGING_SERVER:
    {
      ...
      if (rc == -1)
      {  // if the ping fails, we destroy the session
        handler_ret= -1;
        return handler_ret;
      }
    }
    ...
    case PROCESSING_QUERY:
      ...
      if (mysql_thread___connect_timeout_server_max)
      {
        if (mybe->server_myds->max_connect_time == 0)
          mybe->server_myds->max_connect_time=
              thread->curtime +
              mysql_thread___connect_timeout_server_max * 1000;
      }
      else
      {
        mybe->server_myds->max_connect_time= 0;
      ...
      if ((mybe->server_myds->myconn &&
           ...
           mybe->server_myds->wait_until &&
           thread->curtime >= mybe->server_myds->wait_until)
          // query timed out
      ...
      else
      {
        ...
        if (default_hostgroup >= 0)
        {
          ...
          if (mirror == false)
          {  // do not care about autocommit and charset if mirror
            ...
            if (status == PROCESSING_STMT_EXECUTE)
            {
              CurrentQuery.mysql_stmt=
                  myconn->local_stmts->find_backend_stmt_by_global_id(
                      CurrentQuery.stmt_global_id);
        ...
        if (myconn->async_state_machine == ASYNC_IDLE)
        {
          ...
          if (qpo)
          {
            ...
            {
              mybe->server_myds->wait_until=
                  thread->curtime + qpo->timeout * 1000;
            }
          }
          if (mysql_thread___default_query_timeout)
          {
            ...
            {
              mybe->server_myds->wait_until= thread->curtime;
              unsigned long long def_query_timeout=
        ...
        {
          case PROCESSING_QUERY:
            rc= myconn->async_query(myds->revents,
                                    myds->mysql_real_query.QueryPtr,
                                    myds->mysql_real_query.QuerySize);
            break;
          case PROCESSING_STMT_PREPARE:
            rc= myconn->async_query(
                myds->revents, (char *)CurrentQuery.QueryPointer,
                CurrentQuery.QueryLength, &CurrentQuery.mysql_stmt);
            break;
          case PROCESSING_STMT_EXECUTE:
        ...
        {
          clock_gettime(CLOCK_THREAD_CPUTIME_ID, &endt);
          thread->status_variables.backend_query_time=
              thread->status_variables.backend_query_time +
              (endt.tv_sec * 1000000000 + endt.tv_nsec) -
              (begint.tv_sec * 1000000000 + begint.tv_nsec);
        }
        if (rc == 0)
        {
            ...
            if (myconn->mysql->affected_rows)
            {
              if (myconn->mysql->affected_rows != ULLONG_MAX)
              {
                ...
                if (mysql_thread___auto_increment_delay_multiplex &&
                    myconn->mysql->insert_id)
                {
                  myconn->auto_increment_delay_token=
                      mysql_thread___auto_increment_delay_multiplex + 1;
                }
              }
            }
          }
          ...
          switch (status)
          {
            ...
            case PROCESSING_STMT_PREPARE:
            {
              ...
              stmt_info= GloMyStmt->add_prepared_statement(
                  ...
                  (char *)client_myds->myconn->userinfo->username,
                  (char *)client_myds->myconn->userinfo->schemaname,
                  (char *)CurrentQuery.QueryPointer, CurrentQuery.QueryLength,
                  CurrentQuery.mysql_stmt, qpo->cache_ttl, qpo->timeout,
                  qpo->delay, false);
              if (CurrentQuery.QueryParserArgs.digest_text)
              {
                if (stmt_info->digest_text == NULL)
                {
                  stmt_info->digest_text=
                      strdup(CurrentQuery.QueryParserArgs.digest_text);
                  stmt_info->digest=
                      CurrentQuery.QueryParserArgs.digest;  // copy digest
                  stmt_info->MyComQueryCmd=
                      CurrentQuery.MyComQueryCmd;  // copy MyComQueryCmd
                }
              }
              global_stmtid= stmt_info->statement_id;
              myds->myconn->local_stmts->backend_insert(
                  global_stmtid, CurrentQuery.mysql_stmt);
              if (previous_status.size() == 0)
                client_stmtid=
                    client_myds->myconn->local_stmts
                        ->generate_new_client_stmt_id(global_stmtid);
              ...
              {
                client_myds->myprot.generate_STMT_PREPARE_RESPONSE(
                    client_myds->pkt_sid + 1, stmt_info, client_stmtid);
            ...
            case PROCESSING_STMT_EXECUTE:
            {
              ...
              if (CurrentQuery.stmt_meta)
                ...
                {
                  uint32_t stmt_global_id= 0;
                  memcpy(&stmt_global_id,
                         (char *)(CurrentQuery.stmt_meta->pkt) + 5,
                         sizeof(uint32_t));
                  SLDH->reset(stmt_global_id);
          ...
          if (mysql_thread___multiplexing &&
              (myds->myconn->reusable == true) &&
              myds->myconn->IsActiveTransaction() == false &&
              myds->myconn->MultiplexDisabled() == false)
          {
            if (mysql_thread___connection_delay_multiplex_ms &&
                mirror == false)
            {
              myds->wait_until=
                  thread->curtime +
                  mysql_thread___connection_delay_multiplex_ms * 1000;
              myconn->async_state_machine= ASYNC_IDLE;
              myconn->multiplex_delayed= true;
              myds->DSS= STATE_MARIADB_GENERIC;
            }
            else if (prepared_stmt_with_no_params == true)
            {  // see issue #1432
              myconn->async_state_machine= ASYNC_IDLE;
              myds->DSS= STATE_MARIADB_GENERIC;
              myds->wait_until= 0;
            ...
            else
            {
              ...
              myds->wait_until= 0;
              myds->DSS= STATE_NOT_INITIALIZED;
              if (mysql_thread___autocommit_false_not_reusable &&
                  myds->myconn->IsAutoCommit() == false)
              {
                myds->destroy_MySQL_Connection_From_Pool(true);
              }
              else
          ...
          else
          {
            ...
            {
              if (transaction_persistent_hostgroup == -1)
              {  // change only if not set already, do not allow to change it again
                if (myds->myconn->IsActiveTransaction() == true)
                {  // only active transaction is important here. Ignore other criterias
                  transaction_persistent_hostgroup= current_hostgroup;
                }
              }
              else
              {
                if (myds->myconn->IsActiveTransaction() == false)
                {  // a transaction just completed
                  transaction_persistent_hostgroup= -1;
                }
              }
            }
        ...
        else
        {
          if (rc == -1)
          {
            ...
            if (
                ...
                (myconn->server_status ==
                 MYSQL_SERVER_STATUS_OFFLINE_HARD)  // the query failed because the server is offline hard
                ||
                (myconn->server_status == MYSQL_SERVER_STATUS_SHUNNED &&
                 myconn->parent->shunned_automatic == true &&
                 myconn->parent->shunned_and_kill_all_connections ==
                     true)  // the query failed because the server is shunned due to a serious failure
                ||
                (myconn->server_status ==
                 MYSQL_SERVER_STATUS_SHUNNED_REPLICATION_LAG)  // slave is lagging! see #774
            )
            {
              if (mysql_thread___connect_timeout_server_max)
              {
                myds->max_connect_time=
                    thread->curtime +
                    mysql_thread___connect_timeout_server_max * 1000;
              }
              bool retry_conn= false;
              ...
              if (myds->query_retries_on_failure > 0)
              {
                ...
                {
                  if (myds->myconn->MyRS &&
                      myds->myconn->MyRS->transfer_started)
                  {
            ...
            if (myerr >= 2000 && myerr < 3000)
            {
              ...
              // client error, serious
              proxy_error(
                  "Detected a broken connection during query on (%d,%s,%d) , "
                  "FD (Conn:%d , MyDS:%d) : %d, %s\n",
                  myconn->parent->myhgc->hid, myconn->parent->address,
                  myconn->parent->port, myds->fd, myds->myconn->fd, myerr,
                  (errmsg ? errmsg : mysql_error(myconn->mysql)));
              if (myds->query_retries_on_failure > 0)
              {
                myds->query_retries_on_failure--;
                if ((myds->myconn->reusable == true) &&
            ...
            else
            {
              ...
              {
                proxy_warning(
                    "Error during query on (%d,%s,%d) , user \"%s@%s\" , "
                    "schema \"%s\" , %d, %s . digest_text = \"%s\"\n",
                    myconn->parent->myhgc->hid, myconn->parent->address,
                    myconn->parent->port,
                    client_myds->myconn->userinfo->username,
                    (client_myds->addr.addr ? client_myds->addr.addr
                                            : (char *)"unknown"),
                    client_myds->myconn->userinfo->schemaname, myerr,
                    (errmsg ? errmsg : mysql_error(myconn->mysql)),
              ...
              switch (myerr)
              {
                ...
                case 1153:  // ER_NET_PACKET_TOO_LARGE
                  proxy_warning(
                      "Error ER_NET_PACKET_TOO_LARGE during query on "
                      "(%d,%s,%d): %d, %s\n",
                      myconn->parent->myhgc->hid, myconn->parent->address,
                      myconn->parent->port, myerr,
                      mysql_error(myconn->mysql));
                  break;
              ...
              switch (status)
              {
                ...
                case PROCESSING_STMT_PREPARE:
                {
                  ...
                  if (myconn->mysql)
                  {
                    sprintf(sqlstate, "%s", mysql_sqlstate(myconn->mysql));
                    client_myds->myprot.generate_pkt_ERR(
                        true, NULL, NULL, client_myds->pkt_sid + 1,
                        mysql_errno(myconn->mysql), sqlstate,
                        (char *)mysql_stmt_error(myconn->query.stmt));
                  }
                  else
                  {
          ...
          else
          {
            switch (rc)
            {
              ...
              case 1:
                if (myconn->MyRS && myconn->MyRS->result &&
                    myconn->MyRS->resultset_size >
                        (unsigned int)mysql_thread___threshold_resultset_size)
                {
              ...
              case 2:
                MySQL_Result_to_MySQL_wire(myconn->mysql, myconn->MyRS);
                if (myconn->MyRS)
                {  // we also need to clear MyRS, so that the next staement will recreate it if needed
                  delete myconn->MyRS;
                  myconn->MyRS= NULL;
                }
    ...
    case CHANGING_USER_SERVER:
    {
      ...
      if (rc == -1)
      {  // we have an error we can't handle
        handler_ret= -1;
        return handler_ret;
      }
    }
...
__exit_DSS__STATE_NOT_INITIALIZED:
  ...
  if (mybe && mybe->server_myds)
  {
    if (mybe->server_myds->DSS > STATE_MARIADB_BEGIN &&
        mybe->server_myds->DSS < STATE_MARIADB_END)
    {
#ifdef DEBUG
      MySQL_Data_Stream *myds= mybe->server_myds;
...
#endif /* DEBUG */
      proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                  "Sess=%p, status=%d, server_myds->DSS==%d , revents==%d , "
                  "async_state_machine=%d\n",
                  this, status, mybe->server_myds->DSS, myds->revents,
                  myconn->async_state_machine);
    }
  }
...
void MySQL_Session::
    handler___status_WAITING_SERVER_DATA___STATE_READING_COM_STMT_PREPARE_RESPONSE(
        PtrSize_t *pkt)
{
  unsigned char c;
  c= *((unsigned char *)pkt->ptr + sizeof(mysql_hdr));
...
void MySQL_Session::
    handler___status_CHANGING_USER_CLIENT___STATE_CLIENT_HANDSHAKE(
        PtrSize_t *pkt, bool *wrong_pass)
{
  // FIXME: no support for SSL yet
  if (client_myds->myprot.process_pkt_auth_swich_response(
          (unsigned char *)pkt->ptr, pkt->size) == true)
  {
    l_free(pkt->size, pkt->ptr);
    client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 2, 0, 0, 0, 0,
                                        NULL);
    status= WAITING_CLIENT_DATA;
    client_myds->DSS= STATE_SLEEP;
  }
  else
  {
    l_free(pkt->size, pkt->ptr);
    proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                "Wrong credentials for frontend: disconnecting\n");
    *wrong_pass= true;
    ...
    if (client_myds->client_addr)
    {
      ...
      switch (client_myds->client_addr->sa_family)
      {
        case AF_INET:
        {
          ...
          if (ipv4->sin_port)
          {
            inet_ntop(client_myds->client_addr->sa_family, &ipv4->sin_addr,
                      buf, INET_ADDRSTRLEN);
        ...
        case AF_INET6:
        {
          struct sockaddr_in6 *ipv6=
              (struct sockaddr_in6 *)client_myds->client_addr;
          inet_ntop(client_myds->client_addr->sa_family, &ipv6->sin6_addr,
                    buf, INET6_ADDRSTRLEN);
          client_addr= strdup(buf);
          break;
        }
        default: client_addr= strdup((char *)"localhost"); break;
      }
    }
    else
    {
    ...
    char *_s= (char *)malloc(strlen(client_myds->myconn->userinfo->username) +
                             100 + strlen(client_addr));
    sprintf(_s,
            "ProxySQL Error: Access denied for user '%s'@'%s' (using "
            "password: %s)",
            client_myds->myconn->userinfo->username, client_addr,
            (client_myds->myconn->userinfo->password ? "YES" : "NO"));
    client_myds->myprot.generate_pkt_ERR(true, NULL, NULL, 2, 1045,
                                         (char *)"28000", _s);
    free(_s);
  }
}
...
void MySQL_Session::
    handler___status_CONNECTING_CLIENT___STATE_SERVER_HANDSHAKE(
        PtrSize_t *pkt, bool *wrong_pass)
{
  if ((client_myds->myprot.process_pkt_handshake_response(
           (unsigned char *)pkt->ptr, pkt->size) == true) &&
      (
          ...
          strncmp(client_myds->myconn->userinfo->username,
                  mysql_thread___monitor_username,
                  strlen(mysql_thread___monitor_username)) ==
              0)  // Do not delete this line. See bug #492
  )
  {
    if (session_type == PROXYSQL_SESSION_ADMIN)
    {
      if ((default_hostgroup < 0) ||
          (strncmp(client_myds->myconn->userinfo->username,
                   mysql_thread___monitor_username,
                   strlen(mysql_thread___monitor_username)) == 0))
      {
    ...
    if (client_myds->encrypted == false)
    {
      ...
      if (max_connections_reached == true || free_users <= 0)
      {
        ...
        *wrong_pass= true;
        ...
        if (max_connections_reached == true)
        {
          ...
          client_myds->myprot.generate_pkt_ERR(
              true, NULL, NULL, 2, 1040, (char *)"08004",
              (char *)"Too many connections");
        }
        else
        {  // see issue #794
          proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                      "User '%s' has exceeded the 'max_user_connections' "
                      "resource (current value: %d)\n",
                      client_myds->myconn->userinfo->username, used_users);
          char *a=(char *)"User '%s' has exceeded the 'max_user_connections' resource (current value: %d)";
          char *b= (char *)malloc(
              strlen(a) + strlen(client_myds->myconn->userinfo->username) +
              16);
          sprintf(b, a, client_myds->myconn->userinfo->username, used_users);
          client_myds->myprot.generate_pkt_ERR(true, NULL, NULL, 2, 1226,
                                               (char *)"42000", b);
          free(b);
        }
      ...
      {
        if ((default_hostgroup == ADMIN_HOSTGROUP &&
             strcmp(client_myds->myconn->userinfo->username,
                    (char *)"admin") == 0) ||
                    ...
                    (char *)"monitor") == 0))
        {
          ...
          struct sockaddr *addr=
              (struct sockaddr *)malloc(sizeof(custom_sockaddr));
          ...
          if ((strcmp(client_addr, (char *)"127.0.0.1") == 0) ||
              (strcmp(client_addr, (char *)"localhost") == 0) ||
              (strcmp(client_addr, (char *)"::1") == 0))
          {
            // we are good!
            client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 2, 0, 0, 0,
                                                0, NULL);
            status= WAITING_CLIENT_DATA;
  ...
  else
  {
    ...
    *wrong_pass= true;
    ...
    {
      char *_s=
          (char *)malloc(strlen(client_myds->myconn->userinfo->username) +
                         100 + strlen(client_addr));
      sprintf(_s,
              "ProxySQL Error: Access denied for user '%s'@'%s' (using "
              "password: %s)",
...
void MySQL_Session::handler___status_CONNECTING_CLIENT___STATE_SSL_INIT(
    PtrSize_t *pkt)
{
  if (client_myds->myprot.process_pkt_handshake_response(
          (unsigned char *)pkt->ptr, pkt->size) == true)
  {
    l_free(pkt->size, pkt->ptr);
    client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 3, 0, 0, 0, 0,
                                        NULL);
  ...
  else
  {
    ...
    // FIXME: this should become close connection
    perror(
        "Hitting a not implemented feature: "
        "https://github.com/sysown/proxysql-0.2/issues/124");
    assert(0);
  }
}
        ...
        PtrSize_t *pkt)
{
  ...
  v= *((char *)pkt->ptr + 3);
  proxy_debug(PROXY_DEBUG_MYSQL_COM, 5,
              "Got COM_SET_OPTION packet , value %d\n", v);
  client_myds->setDSS_STATE_QUERY_SENT_NET();
  unsigned int nTrx= NumActiveTransactions();
  uint16_t     setStatus= (nTrx ? SERVER_STATUS_IN_TRANS : 0);
  if (autocommit)
    setStatus+= SERVER_STATUS_AUTOCOMMIT;
  if (v == 1)
  {  // disabled. MYSQL_OPTION_MULTI_STATEMENTS_OFF == 1
    client_myds->myprot.generate_pkt_EOF(true, NULL, NULL, 1, 0, setStatus);
  }
  else
  {  // enabled, MYSQL_OPTION_MULTI_STATEMENTS_ON == 0
    client_myds->myprot.generate_pkt_EOF(true, NULL, NULL, 1, 0, setStatus);
  }
  client_myds->DSS= STATE_SLEEP;
  l_free(pkt->size, pkt->ptr);
        ...
        PtrSize_t *pkt)
{
  ...
  if (autocommit)
    setStatus+= SERVER_STATUS_AUTOCOMMIT;
  client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 1, 0, 0, setStatus, 0,
                                      NULL);
  client_myds->DSS= STATE_SLEEP;
}
        ...
        PtrSize_t *pkt)
{
  if (session_type == PROXYSQL_SESSION_MYSQL)
  {
    ...
    l_free(pkt->size, pkt->ptr);
    client_myds->setDSS_STATE_QUERY_SENT_NET();
    client_myds->myprot.generate_pkt_ERR(true, NULL, NULL, 1, 1045,
                                         (char *)"#28000",
                                         (char *)"Command not supported");
    client_myds->DSS= STATE_SLEEP;
  }
        ...
        PtrSize_t *pkt)
{
  ...
  if (session_type == PROXYSQL_SESSION_MYSQL)
  {
    ...
    client_myds->myconn->userinfo->set_schemaname(
        (char *)pkt->ptr + sizeof(mysql_hdr) + 1,
        ...
        PtrSize_t *pkt)
{
  ...
  if (session_type == PROXYSQL_SESSION_MYSQL)
  {
    ...
    char *schemaname= strndup((char *)pkt->ptr + sizeof(mysql_hdr) + 5,
                              pkt->size - sizeof(mysql_hdr) - 5);
...
*/
    ...
    if (schemanameptr[0] == '`' &&
        schemanameptr[strlen(schemanameptr) - 1] == '`')
    {
      schemanameptr[strlen(schemanameptr) - 1]= '\0';
      schemanameptr++;
    }
    client_myds->myconn->userinfo->set_schemaname(schemanameptr,
                                                  strlen(schemanameptr));
    free(schemaname);
    l_free(pkt->size, pkt->ptr);
    client_myds->setDSS_STATE_QUERY_SENT_NET();
    unsigned int nTrx= NumActiveTransactions();
        ...
        PtrSize_t *pkt, bool prepared)
{
  ...
  if (qpo->new_query)
  {
    ...
    memcpy((unsigned char *)pkt->ptr, &hdr,
           sizeof(mysql_hdr));  // copy header
    unsigned char *c= (unsigned char *)pkt->ptr + sizeof(mysql_hdr);
    *c= (unsigned char)_MYSQL_COM_QUERY;  // set command type
    memcpy((unsigned char *)pkt->ptr + sizeof(mysql_hdr) + 1,
           qpo->new_query->data(), qpo->new_query->length());  // copy query
  ...
  if (pkt->size > (unsigned int)mysql_thread___max_allowed_packet)
  {
    ...
    client_myds->DSS= STATE_QUERY_SENT_NET;
    client_myds->myprot.generate_pkt_ERR(
        true, NULL, NULL, client_myds->pkt_sid + 1, 1153, (char *)"08S01",
        (char *)"Got a packet bigger than 'max_allowed_packet' bytes");
    l_free(pkt->size, pkt->ptr);
    RequestEnd(NULL);
    return true;
  }
  ...
  if (qpo->OK_msg)
  {
    ...
    if (autocommit)
      setStatus+= SERVER_STATUS_AUTOCOMMIT;
    client_myds->myprot.generate_pkt_OK(true, NULL, NULL,
                                        client_myds->pkt_sid + 1, 0, 0,
                                        setStatus, 0, qpo->OK_msg);
    l_free(pkt->size, pkt->ptr);
    RequestEnd(NULL);
    return true;
  }
  ...
  {
    client_myds->DSS= STATE_QUERY_SENT_NET;
    client_myds->myprot.generate_pkt_ERR(true, NULL, NULL,
                                         client_myds->pkt_sid + 1, 1148,
                                         (char *)"42000", qpo->error_msg);
    l_free(pkt->size, pkt->ptr);
    RequestEnd(NULL);
    return true;
  }
  ...
  if (CurrentQuery.QueryParserArgs.digest_text)
  {
    ...
    if (strncasecmp(dig, (char *)"SET ", 4) == 0)
    {
      int    rc;
      string nq=
          string((char *)CurrentQuery.QueryPointer, CurrentQuery.QueryLength);
      RE2::GlobalReplace(&nq, (char *)"^/\\*!\\d\\d\\d\\d\\d SET(.*)\\*/",
                         (char *)"SET\\1");
      ...
      if (match_regexes && match_regexes[0]->match(dig))
      {
        ...
        else
        {
          ...
          {
            string nqn= string((char *)CurrentQuery.QueryPointer,
                               CurrentQuery.QueryLength);
            proxy_error(
      ...
      if (match_regexes && match_regexes[1]->match(dig))
      {
        ...
        if (rc)
        {
          ...
          uint32_t sql_mode_int=
              SpookyHash::Hash32(s.c_str(), s.length(), 10);
        ...
        else
        {
          ...
          string    s2;
          string    s3;
          rc= RE2::FullMatch(nq, *re, &s1, (void *)NULL, &s2, (void *)NULL,
                             &s3);
          //proxy_info("s1 = %s\n",s1.c_str());
          //proxy_info("s2 = %s\n",s2.c_str());
          //proxy_info("s3 = %s\n",s3.c_str());
          ...
          if (rc)
          {
            ...
            {
              uint32_t sql_mode_int=
                  SpookyHash::Hash32(s1.c_str(), s1.length(), 10);
              if (client_myds->myconn->options.sql_mode_int != sql_mode_int)
              {
                client_myds->myconn->options.sql_mode_int= sql_mode_int;
                if (client_myds->myconn->options.sql_mode)
          ...
          else
          {
            ...
            else
            {
              ...
              kq= strncmp(
                  (const char *)CurrentQuery.QueryPointer,
                  (const char *)"/*!40101 SET SQL_MODE=@OLD_SQL_MODE */",
                  CurrentQuery.QueryLength);
              if (kq == 0)
              {
                client_myds->DSS= STATE_QUERY_SENT_NET;
              ...
              else
              {
                string nqn= string((char *)CurrentQuery.QueryPointer,
                                   CurrentQuery.QueryLength);
                proxy_error(
                    "Unable to parse query. If correct, report it as a bug: "
                    "%s\n",
                    nqn.c_str());
                return false;
              }
      ...
      if (match_regexes && match_regexes[2]->match(dig))
      {
        ...
        else
        {
          ...
          kq= strncmp(
              (const char *)CurrentQuery.QueryPointer,
              (const char *)"/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */",
              CurrentQuery.QueryLength);
          if (kq == 0)
          {
            client_myds->DSS= STATE_QUERY_SENT_NET;
  ...
  if (CurrentQuery.QueryParserArgs.digest_text)
  {
    ...
    if (strcasestr(dig, "LAST_INSERT_ID"))
    {
      ...
      if (last_HG_affected_rows >= 0)
      {
        ...
        if (_mybe)
        {
          if (_mybe->server_myds)
          {
            ...
            {
              if (_mybe->server_myds->myconn->mysql)
      ...
      if ((pkt->size == SELECT_LAST_INSERT_ID_LEN + 5 &&
           strncasecmp((char *)SELECT_LAST_INSERT_ID, (char *)pkt->ptr + 5,
                       pkt->size - 5) == 0) ||
          (pkt->size == SELECT_LAST_INSERT_ID_LIMIT1_LEN + 5 &&
           strncasecmp((char *)SELECT_LAST_INSERT_ID_LIMIT1,
                       (char *)pkt->ptr + 5, pkt->size - 5) == 0))
      {
        char buf[32];
        sprintf(buf, "%llu", last_insert_id);
        unsigned int nTrx= NumActiveTransactions();
        ...
        myprot->generate_pkt_field(
            ...
            (char *)"LAST_INSERT_ID()", (char *)"", 63, 31,
            MYSQL_TYPE_LONGLONG, 161, 0, false, 0, NULL);
        sid++;
        myds->DSS= STATE_COLUMN_DEFINITION;
        ...
        char **        p= (char **)malloc(sizeof(char *) * 1);
        unsigned long *l=
            (unsigned long *)malloc(sizeof(unsigned long *) * 1);
        l[0]= strlen(buf);
        p[0]= buf;
  ...
  if (qpo->cache_ttl > 0)
  {
    uint32_t       resbuf= 0;
    unsigned char *aa=
        GloQC->get(client_myds->myconn->userinfo->hash,
                   (const unsigned char *)CurrentQuery.QueryPointer,
                   CurrentQuery.QueryLength, &resbuf, thread->curtime / 1000);
    if (aa)
    {
      l_free(pkt->size, pkt->ptr);
      client_myds->buffer2resultset(aa, resbuf);
      free(aa);
      client_myds->PSarrayOUT->copy_add(client_myds->resultset, 0,
                                        client_myds->resultset->len);
      while (client_myds->resultset->len)
        client_myds->resultset->remove_index(client_myds->resultset->len - 1,
                                             NULL);
      if (transaction_persistent_hostgroup == -1)
      {
        ...
        PtrSize_t *pkt, bool *wrong_pass)
{
      ...
      session_type == PROXYSQL_SESSION_SQLITE)
  {
    ...
    client_authenticated= false;
    if (client_myds->myprot.process_pkt_COM_CHANGE_USER(
            (unsigned char *)pkt->ptr, pkt->size) == true)
    {
      l_free(pkt->size, pkt->ptr);
      client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 1, 0, 0, 0, 0,
                                          NULL);
      client_myds->DSS= STATE_SLEEP;
      status= WAITING_CLIENT_DATA;
      *wrong_pass= false;
    ...
    handler___client_DSS_QUERY_SENT___server_DSS_NOT_INITIALIZED__get_connection()
{
  ...
  if (session_fast_forward == false)
  {
    mc= thread->get_MyConn_local(mybe->hostgroup_id,
                                 this);  // experimental , #644
  }
  if (mc == NULL)
  {
...
#endif
  proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
              "Sess=%p -- server_myds=%p -- MySQL_Connection %p\n", this,
              mybe->server_myds, mybe->server_myds->myconn);
  if (mybe->server_myds->myconn == NULL)
  {
    // we couldn't get a connection for whatever reason, ex: no backends, or too busy
    if (thread->mypolls.poll_timeout == 0)
    {  // tune poll timeout
      if (thread->mypolls.poll_timeout >
          (unsigned int)mysql_thread___poll_timeout_on_failure)
      {
  ...
  if (mybe->server_myds->myconn->fd == -1)
  {
    ...
    proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                "Sess=%p -- MySQL Connection has no FD\n", this);
    MySQL_Connection *myconn= mybe->server_myds->myconn;
  ...
  {
    proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                "Sess=%p -- MySQL Connection found = %p\n", this,
    ...
    {
      status= FAST_FORWARD;
      mybe->server_myds->myconn->reusable=
          false;  // the connection cannot be usable anymore
    }
  }
}
                                                    ...
                                                    MySQL_Connection *myconn)
{
  ...
  if (stmt_result)
  {
    MySQL_ResultSet *MyRS= new MySQL_ResultSet(
        &client_myds->myprot, stmt_result, stmt->mysql, stmt);
  ...
  else
  {
    ...
    if (myerrno == 0)
    {
      ...
      setStatus= setStatus &
                 ~SERVER_STATUS_CURSOR_EXISTS;  // Do not send cursor #1128
      client_myds->myprot.generate_pkt_OK(
          true, NULL, NULL, client_myds->pkt_sid + 1, num_rows,
          mysql->insert_id, setStatus, mysql->warning_count, mysql->info);
      client_myds->pkt_sid++;
    }
    else
    {
      ...
      client_myds->myprot.generate_pkt_ERR(
          true, NULL, NULL, client_myds->pkt_sid + 1, mysql_errno(mysql),
          sqlstate, mysql_error(mysql));
      client_myds->pkt_sid++;
    }
  }
}

void MySQL_Session::MySQL_Result_to_MySQL_wire(MYSQL *            mysql,
                                               ...
                                               MySQL_Data_Stream *_myds)
{
  if (mysql == NULL)
  {
    // error
    client_myds->myprot.generate_pkt_ERR(
        true, NULL, NULL, client_myds->pkt_sid + 1, 2013, (char *)"HY000",
        (char *)"Lost connection to MySQL server during query");
    return;
  }
  if (MyRS)
  {
    ...
    assert(
        resultset_completed);  // the resultset should always be completed if MySQL_Result_to_MySQL_wire is called
    if (transfer_started == false)
    {  // we have all the resultset when MySQL_Result_to_MySQL_wire was called
      if (qpo && qpo->cache_ttl > 0)
      {  // the resultset should be cached
        if (mysql_errno(mysql) == 0)
        {  // no errors
          client_myds->resultset->copy_add(client_myds->PSarrayOUT, 0,
          ...
          while (client_myds->resultset->len)
            client_myds->resultset->remove_index(
                client_myds->resultset->len - 1, NULL);
          GloQC->set(client_myds->myconn->userinfo->hash,
                     (const unsigned char *)CurrentQuery.QueryPointer,
                     CurrentQuery.QueryLength, aa,
                     client_myds->resultset_length, thread->curtime / 1000,
  ...
  {  // no result set
    ...
    else
    {
      ...
      {  // see case #750
        ...
        {
          client_myds->myprot.generate_pkt_ERR(
              true, NULL, NULL, client_myds->pkt_sid + 1, 1317, sqlstate,
              (char *)"Query execution was interrupted");
        }
      }
      else
      {
...
void MySQL_Session::SQLite3_to_MySQL(SQLite3_result *result, char *error,
                                     int             affected_rows,
                                     MySQL_Protocol *myprot)
{
  ...
  if (result)
  {
    myprot->generate_pkt_column_count(true, NULL, NULL, sid, result->columns);
    sid++;
    for (int i= 0; i < result->columns; i++)
    {
      myprot->generate_pkt_field(
          true, NULL, NULL, sid, (char *)"", (char *)"", (char *)"",
          result->column_definition[i]->name, (char *)"", 33, 15,
          MYSQL_TYPE_VAR_STRING, 1, 0x1f, false, 0, NULL);
      sid++;
    }
    myds->DSS= STATE_COLUMN_DEFINITION;
    ...
    unsigned long *l=
        (unsigned long *)malloc(sizeof(unsigned long *) * result->columns);
    for (int r= 0; r < result->rows_count; r++)
    {
      ...
      myprot->generate_pkt_row(true, NULL, NULL, sid, result->columns, l, p);
      sid++;
    }
    myds->DSS= STATE_ROW;
    myprot->generate_pkt_EOF(true, NULL, NULL, sid, 0, 2 | setStatus);
    sid++;
    myds->DSS= STATE_SLEEP;
    free(l);
    free(p);
  ...
  {  // no result set
    ...
    {
      // there was an error
      myprot->generate_pkt_ERR(true, NULL, NULL, sid, 1045, (char *)"#28000",
                               error);
    }
    else
    {
      ...
      if (autocommit)
        setStatus+= SERVER_STATUS_AUTOCOMMIT;
      myprot->generate_pkt_OK(true, NULL, NULL, sid, affected_rows, 0,
                              setStatus, 0, NULL);
    }
    myds->DSS= STATE_SLEEP;
  }
...
void MySQL_Session::RequestEnd(MySQL_Data_Stream *myds)
{
  ...
  if (qpo)
  {
    if (qpo->log == 1)
    {
      GloMyLogger->log_request(
          this,
          myds);  // we send for logging only if logging is enabled for this query
    }
  }
    ...
    if (command_type == _MYSQL_COM_QUERY)
    {
      if (client_myds && client_myds->myconn)
      {
        ...
        if (mc->userinfo && mc->userinfo->username)
        {
          ...
          {
            char *qu= mysql_query_strip_comments(
            ...
            opt2->set_case_sensitive(false);
            char *pattern=
                (char *)"^KILL\\s+(CONNECTION |QUERY |)\\s*(\\d+)\\s*$";
            re2::RE2 *re= new RE2(pattern, *opt2);
            int       id= 0;
            string    tk;
            int       rc;
            ...
            delete re;
            delete opt2;
            proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 2,
                        "filtered query= \"%s\"\n", qu);
            free(qu);
            if (id)
            {
              ...
              if (tk.c_str())
              {
                if ((strlen(tk.c_str()) == 0) ||
                    (strcasecmp(tk.c_str(), "CONNECTION ") == 0))
                {
                  tki= 0;
                }
                else
              ...
              if (tki >= 0)
              {
                ...
                GloMTH->kill_connection_or_query(
                    id, (tki == 0 ? false : true), mc->userinfo->username);
                client_myds->DSS= STATE_QUERY_SENT_NET;
                unsigned int nTrx= NumActiveTransactions();
                uint16_t     setStatus= (nTrx ? SERVER_STATUS_IN_TRANS : 0);
                if (autocommit)
