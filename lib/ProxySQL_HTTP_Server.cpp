...
#define RATE_LIMIT_PAGE                                                \
  "<html><head><title>Rate Limit Page</title></head><body>Rate Limit " \
  "Reached</body></html>"


#define DENIED                                                          \
  "<html><head><title>ProxySQL status page</title></head><body>Access " \
  "denied</body></html>"
...
static char *div1=
    (char *)"<div style=\"margin-bottom: 1px;\"><a href=stats?metric=";
...
static char *generate_home()
{
  ...
  html.append("<div style=\"margin-top: 15px;\"><a href=\"/\"");
  html.append(style2);
  html.append("Home</a>\n");
  html.append("<a href=stats?metric=system");
  html.append(style2);
  html.append("System</a>\n");
  html.append("<a href=stats?metric=mysql");
  html.append(style2);
  html.append("MySQL</a>\n");
  html.append("<a href=stats?metric=cache");
  html.append(style2);
  html.append("Query Cache</a></div>\n");
  html.append("</span>\n</div>\n");
  ...
  html.append("<p></p>\n");
  html.append(
      "<h2 style=\"font-size: 25px; margin-top: 0em; margin-bottom: "
      "0em;\">General information</h2>\n<hr style=\"align: centre\" "
      "width=\"100%\">\n");
  ...
  html.append("<b>OS version = </b>");
  {
    ...
    if (rc == 0)
    {
      html.append(unameData.sysname);
      html.append(" ");
      html.append(unameData.nodename);
      html.append(" ");
      html.append(unameData.release);
      html.append(" ");
  ...
  {
    html.append(
        "<b>MySQL interface(s)</b> = <span style=\"background-color: red;\"> "
        "Not started </span><br>");
  }
  if (GloVars.global.sqlite3_server == false)
  {
    html.append(
        "<b>SQLite3 = </b><span style=\"background-color: yellow;\"> "
        "Disabled </span><br>\n");
  }
  else
  {
  ...
  html.append("</td>\n");
  html.append("<td width=\"33%\">\n");
  html.append("<b>ProxySQL version = </b>");
  html.append(PROXYSQL_VERSION);
  html.append("<br>\n");
  html.append("<b>ProxySQL latest  = </b>");
  {
    ...
    {
      html.append(
          GloAdmin->AdminHTTPServer->variables.proxysql_latest_version);
    }
  }
  html.append("<br>\n");
  html.append("</td>\n");
...
static char *generate_buttons(char *base)
{
  char * s= NULL;
  string html=
      "<div class=\"menu\" style=\"height: auto;\"><span style=\"float: "
      "left; width: 100%;\">\n";
  ...
  html.append(
      ...
      "border-bottom-color: #2969a5; border-bottom-width: 0px; "
      "margin-bottom: 1px; color: #2969a5 ; background-color: white; "
      "font-size: 15px; font-family: Verdana, sans-serif; font-weight: bold; "
      "text-decoration: none; padding-left: 5px; padding-right: "
      "5px;\"></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=1800");
  html.append(style1);
  html.append("Last 1/2 hour</a></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=3600");
  html.append(style1);
  html.append("Last 1 hour</a></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=7200");
  html.append(style1);
  html.append("Last 2 hours</a></div>\n");
  html.append("</span>\n</div>\n");
  ...
  html.append(div1);
  html.append(base);
  html.append("&interval=28800");
  html.append(style1);
  html.append("Last 8 hours</a></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=86400");
  html.append(style1);
  html.append("Last 1 day</a></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=259200");
  html.append(style1);
  html.append("Last 3 days</a></div>\n");
  html.append("</span>\n</div>\n");
  ...
  html.append(div1);
  html.append(base);
  html.append("&interval=604800");
  html.append(style1);
  html.append("Last 7 days</a></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=2592000");
  html.append(style1);
  html.append("Last 1 month</a></div>\n");
  html.append(div1);
  html.append(base);
  html.append("&interval=7776000");
  html.append(style1);
  html.append("Last 3 months</a></div>\n");
  html.append("</span>\n</div>\n");
...
char *ProxySQL_HTTP_Server::extract_values(SQLite3_result *result, int idx,
                                           bool relative, double mult)
{
  string s= "[";
  int    i;
...
{
  fprintf(stderr,
          "Standard ProxySQL HTTP Server Handler rev. %s -- %s -- %s\n",
          PROXYSQL_HTTP_SERVER_VERSION, __FILE__, __TIMESTAMP__);
}

#define EMPTY_PAGE                                                  \
  "<html><head><title>File not found</title></head><body>File not " \
  "found<br/><a href=\"/\">Go back home</a></body></html>"

int ProxySQL_HTTP_Server::handler(void *                 cls,
                                  ...
                                  size_t *upload_data_size, void **ptr)
{
  ...
  }
  {
    ...
    int   max_connections;
    void *sha1_pass= NULL;
    password= GloMyAuth->lookup(username, USERNAME_FRONTEND, &_ret_use_ssl,
    ...
    if ((default_hostgroup != STATS_HOSTGROUP) || (password == NULL))
    {
      if (password)
      {
  ...
  if ((ret == MHD_INVALID_NONCE) || (ret == MHD_NO))
  {
    ...
    if (NULL == response)
      return MHD_NO;
    ret= MHD_queue_auth_fail_response(
        connection, realm, OPAQUE, response,
  ...
  {
    response= MHD_create_response_from_buffer(strlen(RATE_LIMIT_PAGE),
                                              (void *)RATE_LIMIT_PAGE,
                                              MHD_RESPMEM_PERSISTENT);
  ...
  {
    valmetric= (char *)MHD_lookup_connection_value(
        connection, MHD_GET_ARGUMENT_KIND, (char *)"metric");
    interval_s= (char *)MHD_lookup_connection_value(
        connection, MHD_GET_ARGUMENT_KIND, (char *)"interval");
    if (valmetric == NULL)
    {
      response= MHD_create_response_from_buffer(
    ...
    switch (tmp_)
    {
      ...
      case 604800:
      case 2592000:
      case 7776000: interval_i= tmp_; break;
      default: break;
    }
    /*
		if (valunit == NULL) {
...
*/
    if (strcmp(valmetric, "system") == 0)
    {
      ...
      free(buttons);
      s->append(
          "<div class=\"graphs\" style=\"clear: both; height: auto;\">\n");
      ...
      s->append(s1->c_str());
      s->append("</div>\n");
      SQLite3_result *cpu_sqlite=
          GloProxyStats->get_system_cpu_metrics(interval_i);
#ifndef NOJEM
      SQLite3_result *memory_sqlite=
          GloProxyStats->get_system_memory_metrics(interval_i);
#endif
      char **nm= NULL;
      char **nl= NULL;
      char **nv= NULL;
      ...
      nv[0]= extract_values(cpu_sqlite, 2, true,
                            (double)1 / sysconf(_SC_CLK_TCK));
      nv[1]= extract_values(cpu_sqlite, 3, true,
                            (double)1 / sysconf(_SC_CLK_TCK));
      ...
      s->append("</body></html>");
      response= MHD_create_response_from_buffer(
          s->length(), (void *)s->c_str(), MHD_RESPMEM_PERSISTENT);
      ret= MHD_queue_response(connection, MHD_HTTP_OK, response);
      MHD_destroy_response(response);
      return ret;
    }
    ...
    if (strcmp(valmetric, "mysql") == 0)
    {
      ...
      s->append(s1->c_str());
      s->append("</div>\n");
      SQLite3_result *mysql_metrics_sqlite=
          GloProxyStats->get_mysql_metrics(interval_i);
      char **nm= NULL;
      char **nl= NULL;
      char **nv= NULL;
      char * ts= NULL;
    ...
    if (strcmp(valmetric, "cache") == 0)
    {
      ...
      s->append("</div>\n");
      SQLite3_result *mysql_metrics_sqlite=
          GloProxyStats->get_MySQL_Query_Cache_metrics(interval_i);
      char **nm= NULL;
      char **nl= NULL;
      char **nv= NULL;
      char * ts= NULL;
      ...
      nv[2]= extract_values(mysql_metrics_sqlite, 9, false,
                            (double)1 / 1024 / 1024);
      ts= extract_ts(mysql_metrics_sqlite, true);
  ...
  {
    response= MHD_create_response_from_buffer(
        strlen(Chart_bundle_js_c), Chart_bundle_js_c, MHD_RESPMEM_PERSISTENT);
    ret= MHD_queue_response(connection, MHD_HTTP_OK, response);
    MHD_destroy_response(response);
    return ret;
  }
  ...
  {
    response= MHD_create_response_from_buffer(
        strlen(font_awesome), font_awesome, MHD_RESPMEM_PERSISTENT);
    ret= MHD_queue_response(connection, MHD_HTTP_OK, response);
    MHD_destroy_response(response);
    return ret;
  }
  ...
  {
    response= MHD_create_response_from_buffer(strlen(main_bundle_min_css_c),
                                              main_bundle_min_css_c,
                                              MHD_RESPMEM_PERSISTENT);
    ret= MHD_queue_response(connection, MHD_HTTP_OK, response);
    MHD_destroy_response(response);
    return ret;
  ...
  if (strcmp(url, "/") == 0)
  {
    ...
    free(home);
    s->append("</body></html>");
    response= MHD_create_response_from_buffer(s->length(), (void *)s->c_str(),
...
string *ProxySQL_HTTP_Server::generate_header(char *s)
{
  ...
  a->append(
      ...
      "css?family=Source+Sans+Pro:300,400,400i,600,900|Fira+Mono\"/>\n");
  ...
  a->append(
      ...
      "href=\"http://www.proxysql.com\"><span><strong>Proxy</strong>SQL</"
      "span>\n</a>");
  return a;
}
...
string *ProxySQL_HTTP_Server::generate_canvas(char *s)
{
  string *a= new string();
  a->append(
      "<div class=\"wrapper\" style=\"width: 750px; height: 350px\"><canvas "
      "id=\"");
  a->append(s);
...
string *ProxySQL_HTTP_Server::generate_chart(char *chart_name, char *ts,
                                             int nsets, char **dname,
                                             char **llabel, char **values)
{
