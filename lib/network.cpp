...
int listen_on_port(char *ip, uint16_t port, int backlog, bool reuseport)
{
  ...
  {
    if ((sd= socket(next->ai_family, next->ai_socktype, next->ai_protocol)) ==
        -1)
      continue;
#ifdef IPV6_V6ONLY
    if (next->ai_family == AF_INET6)
    {
      if (setsockopt(sd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&arg_on,
                     sizeof(arg_on)) == -1)
        proxy_error("setsockopt() IPV6_V6ONLY: %d\n", gai_strerror(errno));
    }
#endif

    if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, (char *)&arg_on,
                   sizeof(arg_on)) == -1)
    {
    ...
    if (reuseport)
    {
      if (setsockopt(sd, SOL_SOCKET, SO_REUSEPORT, (char *)&arg_on,
                     sizeof(arg_on)) == -1)
      {
...
int listen_on_unix(char *path, int backlog)
{
  ...
  if (bind(sd, (struct sockaddr *)&serveraddr, sizeof(struct sockaddr_un)) !=
      0)
  {
  ...
  r= chmod(path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP |
