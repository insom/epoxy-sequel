...
static char *NODE_COMPUTE_DELIMITER=
    (char *)"-gtyw23a-";  // a random string used for hashing
...
void *ProxySQL_Cluster_Monitor_thread(void *args)
{
  ...
  if (!pthread_attr_init(&thread_attr))
  {
    ...
    {
      __sync_fetch_and_add(&GloVars.statuses.stack_memory_cluster_threads,
                           tmp_stack_size);
    }
  }
  ...
  char *query1=
      (char
           *)"SELECT GLOBAL_CHECKSUM()";  // in future this will be used for "light check"
  char *query2=
      (char *)"SELECT * FROM stats_mysql_global ORDER BY Variable_Name";
  char *query3=
      (char *)"SELECT * FROM runtime_checksums_values ORDER BY name";
  char * username= NULL;
  char * password= NULL;
  bool   rc_bool= true;
  ...
  while (glovars.shutdown == 0 && rc_bool == true)
  {
    ...
    if (strlen(username))
    {  // do not monitor if the username is empty
      ...
      rc_conn= mysql_real_connect(conn, node->hostname, username, password,
                                  NULL, node->port, NULL, 0);
      //char *query = query1;
      if (rc_conn)
      {
        while (glovars.shutdown == 0 && rc_query == 0 && rc_bool == true)
        {
          ...
          if (rc_query == 0)
          {
            ...
            update_checksum= GloProxyCluster->Update_Global_Checksum(
                node->hostname, node->port, result);
...
*/
            ...
            if (update_checksum)
            {
              ...
              if (rc_query == 0)
              {
                ...
                rc_bool= GloProxyCluster->Update_Node_Checksums(
                    node->hostname, node->port, result);
                mysql_free_result(result);
              ...
              else
              {
                ...
                {
                  proxy_error(
                      "Cluster: unable to run query on %s:%d using user %s : "
                      "%s\n",
                      node->hostname, node->port, username, query_error);
                }
                if (++query_error_counter == QUERY_ERROR_RATE)
                  query_error_counter= 0;
              }
            }
            else
            {
              GloProxyCluster->Update_Node_Checksums(node->hostname,
                                                     node->port);
              //int ci = __sync_fetch_and_add(&GloProxyCluster->cluster_check_interval_ms,0);
            ...
            if (rc_query == 0)
            {
              ...
              int freq= __sync_fetch_and_add(
                  &GloProxyCluster->cluster_check_status_frequency, 0);
              if (freq && cluster_check_status_frequency_count >= freq)
              {
                ...
                if (rc_query == 0)
                {
                  ...
                  unsigned long long elapsed_time_us=
                      (after_query_time - before_query_time);
                  rc_bool= GloProxyCluster->Update_Node_Metrics(
                      node->hostname, node->port, result, elapsed_time_us);
                  mysql_free_result(result);
                }
                else
                {
          ...
          if (rc_query == 0)
          {
            ...
            int                ci= __sync_fetch_and_add(
                &GloProxyCluster->cluster_check_interval_ms, 0);
            if ((unsigned)ci > e_ms)
            {
              usleep((ci - e_ms) * 1000);  // remember, usleep is in us
            }
      ...
      {
        proxy_warning(
            "Cluster: unable to connect to peer %s:%d . Error: %s\n",
    ...
    rc_bool= GloProxyCluster->Update_Node_Metrics(
        node->hostname, node->port, NULL, 0);  // added extra check, see #1323
  }
__exit_monitor_thread:
  ...
  __sync_fetch_and_sub(&GloVars.statuses.stack_memory_cluster_threads,
                       tmp_stack_size);
...
ProxySQL_Node_Entry::ProxySQL_Node_Entry(char *_hostname, uint16_t _port,
                                         uint64_t _weight, char *_comment)
{
  hash= 0;
  global_checksum= 0;
  hostname= NULL;
  ...
  metrics_idx= 0;
  metrics= (ProxySQL_Node_Metrics **)malloc(sizeof(ProxySQL_Node_Metrics *) *
                                            PROXYSQL_NODE_METRICS_LEN);
...
bool ProxySQL_Node_Entry::get_active() { return active; }

void ProxySQL_Node_Entry::set_active(bool a) { active= a; }

uint64_t ProxySQL_Node_Entry::get_weight() { return weight; }

void ProxySQL_Node_Entry::set_weight(uint64_t w) { weight= w; }
...
void ProxySQL_Node_Entry::set_checksums(MYSQL_RES *_r)
{
  ...
  while (_r && (row= mysql_fetch_row(_r)))
  {
    if (strcmp(row[0], "admin_variables") == 0)
    {
      ...
      if (strcmp(checksums_values.admin_variables.checksum,
                 GloVars.checksums_values.admin_variables.checksum) == 0)
      {
    ...
    if (strcmp(row[0], "mysql_query_rules") == 0)
    {
      ...
      if (strcmp(checksums_values.mysql_query_rules.checksum, row[3]))
      {
        ...
        proxy_info(
            "Cluster: detected a new checksum for mysql_query_rules from "
            "peer %s:%d, version %llu, epoch %llu, checksum %s . Not syncing "
            "yet ...\n",
            hostname, port, checksums_values.mysql_query_rules.version,
            checksums_values.mysql_query_rules.epoch,
            checksums_values.mysql_query_rules.checksum);
        if (strcmp(checksums_values.mysql_query_rules.checksum,
                   GloVars.checksums_values.mysql_query_rules.checksum) == 0)
        {
          proxy_info(
              "Cluster: checksum for mysql_query_rules from peer %s:%d "
              "matches with local checksum %s , we won't sync.\n",
              hostname, port,
    ...
    if (strcmp(row[0], "mysql_servers") == 0)
    {
      ...
      if (strcmp(checksums_values.mysql_servers.checksum, row[3]))
      {
        ...
        proxy_info(
            "Cluster: detected a new checksum for mysql_servers from peer "
            "%s:%d, version %llu, epoch %llu, checksum %s . Not syncing yet "
            "...\n",
            hostname, port, checksums_values.mysql_servers.version,
            checksums_values.mysql_servers.epoch,
            checksums_values.mysql_servers.checksum);
        if (strcmp(checksums_values.mysql_servers.checksum,
                   GloVars.checksums_values.mysql_servers.checksum) == 0)
        {
          proxy_info(
              "Cluster: checksum for mysql_servers from peer %s:%d matches "
              "with local checksum %s , we won't sync.\n",
              hostname, port,
              GloVars.checksums_values.mysql_servers.checksum);
        }
      }
      else
      {
    ...
    if (strcmp(row[0], "mysql_users") == 0)
    {
      ...
      if (strcmp(checksums_values.mysql_users.checksum, row[3]))
      {
        ...
        proxy_info(
            "Cluster: detected a new checksum for mysql_users from peer "
            "%s:%d, version %llu, epoch %llu, checksum %s . Not syncing yet "
            "...\n",
            hostname, port, checksums_values.mysql_users.version,
            checksums_values.mysql_users.epoch,
            checksums_values.mysql_users.checksum);
        if (strcmp(checksums_values.mysql_users.checksum,
                   GloVars.checksums_values.mysql_users.checksum) == 0)
        {
          proxy_info(
              "Cluster: checksum for mysql_users from peer %s:%d matches "
              "with local checksum %s , we won't sync.\n",
    ...
    if (strcmp(row[0], "mysql_variables") == 0)
    {
      ...
      if (strcmp(checksums_values.mysql_variables.checksum,
                 GloVars.checksums_values.mysql_variables.checksum) == 0)
      {
    ...
    if (strcmp(row[0], "proxysql_servers") == 0)
    {
      ...
      if (strcmp(checksums_values.proxysql_servers.checksum, row[3]))
      {
        ...
        proxy_info(
            "Cluster: detected a new checksum for proxysql_servers from peer "
            "%s:%d, version %llu, epoch %llu, checksum %s . Not syncing yet "
            "...\n",
            hostname, port, checksums_values.proxysql_servers.version,
            checksums_values.proxysql_servers.epoch,
            checksums_values.proxysql_servers.checksum);
        if (strcmp(checksums_values.proxysql_servers.checksum,
                   GloVars.checksums_values.proxysql_servers.checksum) == 0)
        {
          proxy_info(
              "Cluster: checksum for proxysql_servers from peer %s:%d "
              "matches with local checksum %s , we won't sync.\n",
              hostname, port,
  ...
  if (_r == NULL)
  {
    ...
    v->last_updated= now;
    if (strcmp(v->checksum, GloVars.checksums_values.mysql_users.checksum) ==
        0)
    {
      v->diff_check= 0;
    }
  ...
  unsigned int diff_mqr= (unsigned int)__sync_fetch_and_add(
      &GloProxyCluster->cluster_mysql_query_rules_diffs_before_sync, 0);
  unsigned int diff_ms= (unsigned int)__sync_fetch_and_add(
      &GloProxyCluster->cluster_mysql_servers_diffs_before_sync, 0);
  unsigned int diff_mu= (unsigned int)__sync_fetch_and_add(
      &GloProxyCluster->cluster_mysql_users_diffs_before_sync, 0);
  unsigned int diff_ps= (unsigned int)__sync_fetch_and_add(
      &GloProxyCluster->cluster_proxysql_servers_diffs_before_sync, 0);
  ProxySQL_Checksum_Value_2 *v= NULL;
  if (diff_mqr)
  {
    unsigned long long own_version= __sync_fetch_and_add(
        &GloVars.checksums_values.mysql_query_rules.version, 0);
    unsigned long long own_epoch= __sync_fetch_and_add(
        &GloVars.checksums_values.mysql_query_rules.epoch, 0);
    v= &checksums_values.mysql_query_rules;
    if (v->version > 1)
    {
      if ((own_version == 1)         // we just booted
      ...
      )
      {
        ...
        {
          proxy_info(
              "Cluster: detected a peer %s:%d with mysql_query_rules version "
              "%llu, epoch %llu, diff_check %u. Own version: %llu, epoch: "
              "%llu. Proceeding with remote sync\n",
              hostname, port, v->version, v->epoch, v->diff_check,
              own_version, own_epoch);
    ...
    else
    {
      if (v->diff_check && (v->diff_check % (diff_mqr * 10)) == 0)
      {
        proxy_warning(
            "Cluster: detected a peer %s:%d with mysql_query_rules version "
            "%llu, epoch %llu, diff_check %u. Own version: %llu, epoch: "
            "%llu. diff_check is increasing, but version 1 doesn't allow "
            "sync. This message will be repeated every %llu checks until "
            "LOAD MYSQL QUERY RULES TO RUNTIME is executed on candidate "
            "master.\n",
            hostname, port, v->version, v->epoch, v->diff_check, own_version,
            own_epoch, (diff_mqr * 10));
      }
    }
  }
  if (diff_ms)
  {
    v= &checksums_values.mysql_servers;
    unsigned long long own_version= __sync_fetch_and_add(
        &GloVars.checksums_values.mysql_servers.version, 0);
    unsigned long long own_epoch= __sync_fetch_and_add(
        &GloVars.checksums_values.mysql_servers.epoch, 0);
    if (v->version > 1)
    {
      if ((own_version == 1)         // we just booted
          || (v->epoch > own_epoch)  // epoch is newer
      )
      {
        ...
        {
          proxy_info(
              "Cluster: detected a peer %s:%d with mysql_servers version "
              "%llu, epoch %llu, diff_check %u. Own version: %llu, epoch: "
              "%llu. Proceeding with remote sync\n",
              hostname, port, v->version, v->epoch, v->diff_check,
              own_version, own_epoch);
    ...
    else
    {
      ...
      {
        proxy_warning(
            "Cluster: detected a peer %s:%d with mysql_servers version %llu, "
            "epoch %llu, diff_check %u. Own version: %llu, epoch: %llu. "
            "diff_check is increasing, but version 1 doesn't allow sync. "
            "This message will be repeated every %llu checks until LOAD "
            "MYSQL SERVERS TO RUNTIME is executed on candidate master.\n",
            hostname, port, v->version, v->epoch, v->diff_check, own_version,
            own_epoch, (diff_ms * 10));
      }
    }
  }
  if (diff_mu)
  {
    v= &checksums_values.mysql_users;
    unsigned long long own_version= __sync_fetch_and_add(
        &GloVars.checksums_values.mysql_users.version, 0);
    unsigned long long own_epoch=
        __sync_fetch_and_add(&GloVars.checksums_values.mysql_users.epoch, 0);
    if (v->version > 1)
    {
      if ((own_version == 1)         // we just booted
          || (v->epoch > own_epoch)  // epoch is newer
      )
      {
        ...
        {
          proxy_info(
              "Cluster: detected a peer %s:%d with mysql_users version %llu, "
              "epoch %llu, diff_check %u. Own version: %llu, epoch: %llu. "
              "Proceeding with remote sync\n",
              hostname, port, v->version, v->epoch, v->diff_check,
              own_version, own_epoch);
    ...
    else
    {
      if (v->diff_check && (v->diff_check % (diff_mu * 10)) == 0)
      {
        proxy_warning(
            ...
            "diff_check is increasing, but version 1 doesn't allow sync. "
            "This message will be repeated every %llu checks until LOAD "
            "MYSQL USERS TO RUNTIME is executed on candidate master.\n",
            hostname, port, v->version, v->epoch, v->diff_check, own_version,
            own_epoch, (diff_mu * 10));
      }
    }
  }
  if (diff_ps)
  {
    v= &checksums_values.proxysql_servers;
    unsigned long long own_version= __sync_fetch_and_add(
        &GloVars.checksums_values.proxysql_servers.version, 0);
    unsigned long long own_epoch= __sync_fetch_and_add(
        &GloVars.checksums_values.proxysql_servers.epoch, 0);
    if (v->version > 1)
    {
      if ((own_version == 1)         // we just booted
          || (v->epoch > own_epoch)  // epoch is newer
      )
      {
        ...
        {
          proxy_info(
              "Cluster: detected a peer %s:%d with proxysql_servers version "
              "%llu, epoch %llu, diff_check %u. Own version: %llu, epoch: "
              "%llu. Proceeding with remote sync\n",
              hostname, port, v->version, v->epoch, v->diff_check,
              own_version, own_epoch);
    ...
    else
    {
      if (v->diff_check && (v->diff_check % (diff_ps * 10)) == 0)
      {
        proxy_warning(
            ...
            "%llu. diff_check is increasing, but version 1 doesn't allow "
            "sync. This message will be repeated every %llu checks until "
            "LOAD PROXYSQL SERVERS TO RUNTIME is executed on candidate "
            "master.\n",
            hostname, port, v->version, v->epoch, v->diff_check, own_version,
            own_epoch, (diff_ps * 10));
      }
    }
  }
}
...
void ProxySQL_Cluster::pull_mysql_query_rules_from_peer()
{
  ...
  if (hostname)
  {
    ...
    {  // do not monitor if the username is empty
      ...
      mysql_options(conn, MYSQL_OPT_WRITE_TIMEOUT, &timeout);
      proxy_info(
          "Cluster: Fetching MySQL Query Rules from peer %s:%d started\n",
          hostname, port);
      rc_conn= mysql_real_connect(conn, hostname, username, password, NULL,
                                  port, NULL, 0);
      if (rc_conn)
      {
        ...
        rc_query= mysql_query(
            conn,
            "SELECT rule_id, username, schemaname, flagIN, client_addr, "
            "proxy_addr, proxy_port, digest, match_digest, match_pattern, "
            "negate_match_pattern, re_modifiers, flagOUT, replace_pattern, "
            "destination_hostgroup, cache_ttl, reconnect, timeout, retries, "
            "delay, next_query_flagIN, mirror_flagOUT, mirror_hostgroup, "
            "error_msg, ok_msg, sticky_conn, multiplex, log, apply, comment "
        ...
        if (rc_query == 0)
        {
          ...
          rc_query= mysql_query(
              conn,
              "SELECT username, schemaname, flagIN, destination_hostgroup, "
              "comment FROM runtime_mysql_query_rules_fast_routing");
          if (rc_query == 0)
          {
            result2= mysql_store_result(conn);
            proxy_info(
                "Cluster: Fetching MySQL Query Rules from peer %s:%d "
                "completed\n",
                hostname, port);
            proxy_info(
                "Cluster: Loading to runtime MySQL Query Rules from peer "
                "%s:%d\n",
                hostname, port);
            GloAdmin->admindb->execute("DELETE FROM mysql_query_rules");
            GloAdmin->admindb->execute(
            ...
            {
              rc= sqlite3_bind_int64(statement1, 1, atoll(row[0]));
              assert(rc == SQLITE_OK);  // rule_id
              rc= sqlite3_bind_int64(statement1, 2, 1);
              assert(rc == SQLITE_OK);  // active
              rc= sqlite3_bind_text(statement1, 3, row[1], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // username
              rc= sqlite3_bind_text(statement1, 4, row[2], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // schemaname
              rc= sqlite3_bind_text(statement1, 5, row[3], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // flagIN
              rc= sqlite3_bind_text(statement1, 6, row[4], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // client_addr
              rc= sqlite3_bind_text(statement1, 7, row[5], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // proxy_addr
              rc= sqlite3_bind_text(statement1, 8, row[6], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // proxy_port
              rc= sqlite3_bind_text(statement1, 9, row[7], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // digest
              rc= sqlite3_bind_text(statement1, 10, row[8], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // match_digest
              rc= sqlite3_bind_text(statement1, 11, row[9], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // match_pattern
              rc= sqlite3_bind_text(statement1, 12, row[10], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // negate_match_pattern
              rc= sqlite3_bind_text(statement1, 13, row[11], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // re_modifiers
              rc= sqlite3_bind_text(statement1, 14, row[12], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // flagOUT
              rc= sqlite3_bind_text(statement1, 15, row[13], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // replace_pattern
              rc= sqlite3_bind_text(statement1, 16, row[14], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // destination_hostgroup
              rc= sqlite3_bind_text(statement1, 17, row[15], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // cache_ttl
              rc= sqlite3_bind_text(statement1, 18, row[16], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // reconnect
              rc= sqlite3_bind_text(statement1, 19, row[17], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // timeout
              rc= sqlite3_bind_text(statement1, 20, row[18], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // retries
              rc= sqlite3_bind_text(statement1, 21, row[19], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // delay
              rc= sqlite3_bind_text(statement1, 22, row[20], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // next_query_flagIN
              rc= sqlite3_bind_text(statement1, 23, row[21], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // mirror_flagOUT
              rc= sqlite3_bind_text(statement1, 24, row[22], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // mirror_hostgroup
              rc= sqlite3_bind_text(statement1, 25, row[23], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // error_msg
              rc= sqlite3_bind_text(statement1, 26, row[24], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // OK_msg
              rc= sqlite3_bind_text(statement1, 27, row[25], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // sticky_conn
              rc= sqlite3_bind_text(statement1, 28, row[26], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // multiplex
              rc= sqlite3_bind_text(statement1, 29, row[27], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // log
              rc= sqlite3_bind_text(statement1, 30, row[28], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // apply
              rc= sqlite3_bind_text(statement1, 31, row[29], -1,
                                    SQLITE_TRANSIENT);
              assert(rc == SQLITE_OK);  // comment
            ...
            while ((row= mysql_fetch_row(result2)))
            {
              ...
              {  // bulk
                rc= sqlite3_bind_text(statement32fr, (idx * 5) + 1, row[0],
                                      -1, SQLITE_TRANSIENT);
                assert(rc == SQLITE_OK);  // username
                rc= sqlite3_bind_text(statement32fr, (idx * 5) + 2, row[1],
                                      -1, SQLITE_TRANSIENT);
                assert(rc == SQLITE_OK);  // schemaname
                rc= sqlite3_bind_int64(statement32fr, (idx * 5) + 3,
                                       atoll(row[2]));
                assert(rc == SQLITE_OK);  // flagIN
                rc= sqlite3_bind_int64(statement32fr, (idx * 5) + 4,
                                       atoll(row[3]));
                assert(rc == SQLITE_OK);  // destination_hostgroup
                rc= sqlite3_bind_text(statement32fr, (idx * 5) + 5, row[4],
                                      -1, SQLITE_TRANSIENT);
                assert(rc == SQLITE_OK);  // comment
                if (idx == 31)
                {
                  SAFE_SQLITE3_STEP2(statement32fr);
                  rc= sqlite3_clear_bindings(statement32fr);
                  assert(rc == SQLITE_OK);
                  rc= sqlite3_reset(statement32fr);
                  assert(rc == SQLITE_OK);
                }
              }
              else
              {  // single row
                rc= sqlite3_bind_text(statement1fr, 1, row[0], -1,
                                      SQLITE_TRANSIENT);
                assert(rc == SQLITE_OK);  // username
                rc= sqlite3_bind_text(statement1fr, 2, row[1], -1,
                                      SQLITE_TRANSIENT);
                assert(rc == SQLITE_OK);  // schemaname
                rc= sqlite3_bind_int64(statement1fr, 3, atoll(row[2]));
                assert(rc == SQLITE_OK);  // flagIN
                rc= sqlite3_bind_int64(statement1fr, 4, atoll(row[3]));
                assert(rc == SQLITE_OK);  // destination_hostgroup
                rc= sqlite3_bind_text(statement1fr, 5, row[4], -1,
                                      SQLITE_TRANSIENT);
                assert(rc == SQLITE_OK);  // comment
                SAFE_SQLITE3_STEP2(statement1fr);
                rc= sqlite3_clear_bindings(statement1fr);
                assert(rc == SQLITE_OK);
                rc= sqlite3_reset(statement1fr);
                assert(rc == SQLITE_OK);
              }
              row_idx++;
            }
            GloAdmin->load_mysql_query_rules_to_runtime();
            if (GloProxyCluster->cluster_mysql_query_rules_save_to_disk ==
                true)
            {
              proxy_info(
                  "Cluster: Saving to disk MySQL Query Rules from peer "
                  "%s:%d\n",
                  hostname, port);
          ...
          {
            proxy_info(
                "Cluster: Fetching MySQL Query Rules from peer %s:%d failed: "
                "%s\n",
    ...
    {  // do not monitor if the username is empty
      ...
      if (rc_conn)
      {
        ...
        if (rc_query == 0)
        {
          ...
          {
            rc= sqlite3_bind_text(statement1, 1, row[0], -1,
                                  SQLITE_TRANSIENT);
            assert(rc == SQLITE_OK);  // username
            rc= sqlite3_bind_text(statement1, 2, row[1], -1,
                                  SQLITE_TRANSIENT);
            assert(rc == SQLITE_OK);  // password
            rc= sqlite3_bind_int64(statement1, 3, atoll(row[2]));
            assert(rc == SQLITE_OK);  // active
            rc= sqlite3_bind_int64(statement1, 4, atoll(row[3]));
            assert(rc == SQLITE_OK);  // use_ssl
            rc= sqlite3_bind_int64(statement1, 5, atoll(row[4]));
            assert(rc == SQLITE_OK);  // default_hostgroup
            rc= sqlite3_bind_text(statement1, 6, row[5], -1,
                                  SQLITE_TRANSIENT);
            assert(rc == SQLITE_OK);  // default_schema
            rc= sqlite3_bind_int64(statement1, 7, atoll(row[6]));
            assert(rc == SQLITE_OK);  // schema_locked
            rc= sqlite3_bind_int64(statement1, 8, atoll(row[7]));
            assert(rc == SQLITE_OK);  // transaction_persistent
            rc= sqlite3_bind_int64(statement1, 9, atoll(row[8]));
            assert(rc == SQLITE_OK);  // fast_forward
            rc= sqlite3_bind_int64(statement1, 10, atoll(row[9]));
            assert(rc == SQLITE_OK);  // backend
            rc= sqlite3_bind_int64(statement1, 11, atoll(row[10]));
            assert(rc == SQLITE_OK);  // frontend
            rc= sqlite3_bind_int64(statement1, 12, atoll(row[11]));
            assert(rc == SQLITE_OK);  // max_connection
          ...
          mysql_free_result(result);
          proxy_info(
              "Cluster: Fetching MySQL Users from peer %s:%d completed\n",
              hostname, port);
          proxy_info(
              "Cluster: Loading to runtime MySQL Users from peer %s:%d\n",
              hostname, port);
          GloAdmin->init_users();
          if (GloProxyCluster->cluster_mysql_query_rules_save_to_disk == true)
          {
            proxy_info(
                "Cluster: Saving to disk MySQL Query Rules from peer %s:%d\n",
                hostname, port);
        ...
        {
          proxy_info(
              "Cluster: Fetching MySQL Users from peer %s:%d failed: %s\n",
              hostname, port, mysql_error(conn));
        }
      }
      else
...
void ProxySQL_Cluster::pull_mysql_servers_from_peer()
{
  ...
  if (hostname)
  {
    ...
    {  // do not monitor if the username is empty
      ...
      mysql_options(conn, MYSQL_OPT_WRITE_TIMEOUT, &timeout);
      proxy_info(
          "Cluster: Fetching MySQL Servers from peer %s:%d started. Expected "
          "checksum %s\n",
      ...
      if (rc_conn)
      {
        ...
        rc_query= mysql_query(
            conn,
            CLUSTER_QUERY_MYSQL_SERVERS);  // for bug #1188 , ProxySQL Admin needs to know the exact query
        if (rc_query == 0)
        {
          ...
          rc_query=
              mysql_query(conn, CLUSTER_QUERY_MYSQL_REPLICATION_HOSTGROUPS);
          if (rc_query == 0)
          {
            result2= mysql_store_result(conn);
            proxy_info(
                ...
                hostname, port);
            proxy_info(
                "Cluster: Fetching checksum for MySQL Servers from peer "
                "%s:%d before proceessing\n",
                hostname, port);
            rc_query= mysql_query(conn,
                                  "SELECT * FROM runtime_checksums_values "
                                  "WHERE name='mysql_servers' LIMIT 1");
            if (rc_query == 0)
            {
              ...
              if (checks)
              {
                if (strcmp(checks, peer_checksum) == 0)
                {
                  // we are OK to sync!
                  proxy_info(
                      "Cluster: Fetching checksum for MySQL Servers from "
                      "peer %s:%d successful. Checksum: %s\n",
                  ...
                  while ((row= mysql_fetch_row(result1)))
                  {
                    ...
                    char *query=
                        (char *)malloc(strlen(q) + i + strlen(o) + 64);

                    sprintf(
                        query, q, row[0], row[1], row[2], row[3],
                        (strcmp(row[4], "SHUNNED") == 0 ? "ONLINE" : row[4]),
                        row[5], row[6], row[7], row[8], row[9], o);
                    if (o != row[10])
                    {  // there was a copy
                      free(o);
                    }
                  ...
                  proxy_info(
                      "Cluster: Writing mysql_replication_hostgroups "
                      "table\n");
                  GloAdmin->admindb->execute(
                  ...
                  proxy_info(
                      "Cluster: Loading to runtime MySQL Servers from peer "
                      "%s:%d\n",
                      hostname, port);
                  GloAdmin->load_mysql_servers_to_runtime();
                  if (GloProxyCluster->cluster_mysql_servers_save_to_disk ==
                      true)
                  {
                    proxy_info(
                        "Cluster: Saving to disk MySQL Servers from peer "
                        "%s:%d\n",
                        hostname, port);
          ...
          {
            proxy_info(
                "Cluster: Fetching MySQL Servers from peer %s:%d failed: "
                "%s\n",
                hostname, port, mysql_error(conn));
          }
          if (result1)
        ...
        {
          proxy_info(
              "Cluster: Fetching MySQL Servers from peer %s:%d failed: %s\n",
              hostname, port, mysql_error(conn));
        }
...
void ProxySQL_Cluster::pull_proxysql_servers_from_peer()
{
  ...
  if (hostname)
  {
    ...
    {  // do not monitor if the username is empty
      ...
      mysql_options(conn, MYSQL_OPT_WRITE_TIMEOUT, &timeout);
      proxy_info(
          "Cluster: Fetching ProxySQL Servers from peer %s:%d started\n",
          hostname, port);
      rc_conn= mysql_real_connect(conn, hostname, username, password, NULL,
                                  port, NULL, 0);
      if (rc_conn)
      {
        rc_query= mysql_query(conn,
                              "SELECT hostname, port, weight, comment FROM "
        ...
        if (rc_query == 0)
        {
          ...
          mysql_free_result(result);
          proxy_info(
              "Cluster: Fetching ProxySQL Servers from peer %s:%d "
              "completed\n",
              hostname, port);
          proxy_info(
              "Cluster: Loading to runtime ProxySQL Servers from peer "
              "%s:%d\n",
              hostname, port);
          ...
          {
            proxy_info(
                "Cluster: Saving to disk ProxySQL Servers from peer %s:%d\n",
                hostname, port);
        ...
        {
          proxy_info(
              "Cluster: Fetching ProxySQL Servers from peer %s:%d failed: "
              "%s\n",
              hostname, port, mysql_error(conn));
        }
      }
      ...
      {
        proxy_info(
            "Cluster: Fetching ProxySQL Servers from peer %s:%d failed: %s\n",
            hostname, port, mysql_error(conn));
      }
    }
...
void ProxySQL_Node_Entry::set_metrics(MYSQL_RES *        _r,
...
{
  for (std::unordered_map<uint64_t, ProxySQL_Node_Entry *>::iterator it=
           umap_proxy_nodes.begin();
       it != umap_proxy_nodes.end();)
  {
...
uint64_t ProxySQL_Cluster_Nodes::generate_hash(char *   _hostname,
                                               uint16_t _port)
{
                                               ...
                                               bool            _lock)
{
       ...
       it != resultset->rows.end(); ++it)
  {
    ...
    std::unordered_map<uint64_t, ProxySQL_Node_Entry *>::iterator ite=
        umap_proxy_nodes.find(hash_);
    if (ite == umap_proxy_nodes.end())
    {
      node= new ProxySQL_Node_Entry(h_, p_, w_, c_);
      node->set_active(true);
      umap_proxy_nodes.insert(std::make_pair(hash_, node));
      proxy_node_address_t *a=
          (proxy_node_address_t *)malloc(sizeof(proxy_node_address_t));
      ...
      if (pthread_create(&a->thrid, &attr, ProxySQL_Cluster_Monitor_thread,
                         (void *)a) != 0)
      {
...
bool ProxySQL_Cluster_Nodes::Update_Node_Checksums(char *_h, uint16_t _p,
                                                   MYSQL_RES *_r)
{
  bool     ret= false;
...
bool ProxySQL_Cluster_Nodes::Update_Global_Checksum(char *_h, uint16_t _p,
                                                    MYSQL_RES *_r)
{
  bool     ret= true;
  uint64_t hash_= generate_hash(_h, _p);
...
// if it returns false , the node doesn't exist anymore and the monitor should stop
bool ProxySQL_Cluster_Nodes::Update_Node_Metrics(
    char *_h, uint16_t _p, MYSQL_RES *_r, unsigned long long _response_time)
{
  bool     ret= false;
  uint64_t hash_= generate_hash(_h, _p);
  pthread_mutex_lock(&mutex);
...
void ProxySQL_Cluster_Nodes::get_peer_to_sync_mysql_query_rules(
    char **host, uint16_t *port)
{
  ...
  if (epoch)
  {
    ...
    {
      proxy_warning(
          "Cluster: detected a peer with mysql_query_rules epoch %llu , but "
          "not enough diff_check. We won't sync from epoch %llu: temporarily "
          "skipping sync\n",
          max_epoch, epoch);
      if (hostname)
      {
  ...
  if (hostname)
  {
    ...
    *port= p;
    proxy_info(
        "Cluster: detected peer %s:%d with mysql_query_rules version %llu, "
        "epoch %llu\n",
        hostname, p, version, epoch);
  }
}

void ProxySQL_Cluster_Nodes::get_peer_to_sync_mysql_servers(
    char **host, uint16_t *port, char **peer_checksum)
{
  unsigned long long version= 0;
  unsigned long long epoch= 0;
  unsigned long long max_epoch= 0;
  ...
  if (epoch)
  {
    ...
    {
      proxy_warning(
          "Cluster: detected a peer with mysql_servers epoch %llu , but not "
          "enough diff_check. We won't sync from epoch %llu: temporarily "
          "skipping sync\n",
          max_epoch, epoch);
      if (hostname)
      {
  ...
  if (hostname)
  {
    ...
    *port= p;
    proxy_info(
        "Cluster: detected peer %s:%d with mysql_servers version %llu, epoch "
        "%llu\n",
        hostname, p, version, epoch);
    *peer_checksum= pc;
  }
...
void ProxySQL_Cluster_Nodes::get_peer_to_sync_mysql_users(char **   host,
                                                          uint16_t *port)
{
  unsigned long long version= 0;
  unsigned long long epoch= 0;
  ...
  if (epoch)
  {
    ...
    {
      proxy_warning(
          "Cluster: detected a peer with mysql_users epoch %llu , but not "
          "enough diff_check. We won't sync from epoch %llu: temporarily "
          "skipping sync\n",
          max_epoch, epoch);
      if (hostname)
  ...
  if (hostname)
  {
    ...
    *port= p;
    proxy_info(
        "Cluster: detected peer %s:%d with mysql_users version %llu, epoch "
        "%llu\n",
        hostname, p, version, epoch);
  }
}

void ProxySQL_Cluster_Nodes::get_peer_to_sync_proxysql_servers(char **   host,
                                                               uint16_t *port)
{
  unsigned long long version= 0;
  unsigned long long epoch= 0;
  ...
  if (epoch)
  {
    ...
    {
      proxy_warning(
          "Cluster: detected a peer with proxysql_servers epoch %llu , but "
          "not enough diff_check. We won't sync from epoch %llu: temporarily "
          "skipping sync\n",
          max_epoch, epoch);
      if (hostname)
  ...
  if (hostname)
  {
    ...
    *port= p;
    proxy_info(
        "Cluster: detected peer %s:%d with proxysql_servers version %llu, "
        "epoch %llu\n",
        hostname, p, version, epoch);
  }
}
...
SQLite3_result *ProxySQL_Cluster_Nodes::stats_proxysql_servers_checksums()
{
       ...
       it != umap_proxy_nodes.end();)
  {
    ...
    for (int i= 0; i < 6; i++)
    {
      ...
      switch (i)
      {
        case 0: pta[2]= strdup((char *)"admin_variables"); break;
        case 1: pta[2]= strdup((char *)"mysql_query_rules"); break;
        case 2: pta[2]= strdup((char *)"mysql_servers"); break;
        case 3: pta[2]= strdup((char *)"mysql_users"); break;
        case 4: pta[2]= strdup((char *)"mysql_variables"); break;
        case 5: pta[2]= strdup((char *)"proxysql_servers"); break;
        default: break;
      }
