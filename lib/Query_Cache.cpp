...
#define THR_UPDATE_CNT(__a, __b, __c, __d)         \
  ...
  {                                                \
    ...
    if (__a >= __d)                                \
    {                                              \
      __sync_fetch_and_add(&__b, __a - __a % __d); \
      __a= __a % __d;                              \
    }                                              \
  } while (0)
...
#define THR_DECREASE_CNT(__a, __b, __c, __d)       \
  ...
  {                                                \
    ...
    if (__a >= __d)                                \
    {                                              \
      __sync_fetch_and_sub(&__b, __a - __a % __d); \
      __a= __a % __d;                              \
    }                                              \
  } while (0)
...
KV_BtreeArray::~KV_BtreeArray()
{
  proxy_debug(PROXY_DEBUG_QUERY_CACHE, 3,
              "Size of  KVBtreeArray:%d , ptrArray:%llu\n", cnt(),
              ptrArray->len);
  empty();
  QC_entry_t *qce= NULL;
  while (ptrArray->len)
...
{
  uint64_t r= __sync_fetch_and_add(&Glo_num_entries, 0) *
              (sizeof(QC_entry_t) + sizeof(QC_entry_t *) * 2 +
               sizeof(uint64_t) *
                   2);  // +  __sync_fetch_and_add(&Glo_size_values,0) ;
  return r;
};
...
void KV_BtreeArray::purge_some(unsigned long long QCnow_ms, bool aggressive)
{
  ...
  for (i= 0; i < ptrArray->len; i++)
  {
    ...
    if (aggressive)
    {  // we have been asked to do aggressive purging
      if (access_ms_min == 0)
      {
    ...
    {  // no aggresssive purging , legacy algorithm
  ...
  if (aggressive == false)
  {
    ...
    total_freeable_memory= freeable_memory + ret * (sizeof(QC_entry_t) +
                                                    sizeof(QC_entry_t *) * 2 +
                                                    sizeof(uint64_t) * 2);
  ...
  if (aggressive || cond_freeable_memory)
  {
    ...
    if (aggressive)
    {
      access_ms_lower_mark=
          access_ms_min +
          (access_ms_max - access_ms_min) *
              0.1;  // hardcoded for now. Remove the entries with access time in the 10% range closest to access_ms_min
    }
#ifdef PROXYSQL_QC_PTHREAD_MUTEX
    ...
    for (i= 0; i < ptrArray->len; i++)
    {
      ...
      if (__sync_fetch_and_add(&qce->ref_count, 0) <= 1)
      {  // currently not in use
        ...
        {  // we have been asked to do aggressive purging
          if (drop_entry == false)
          {  // if the entry is already marked to be dropped, no further check
...
int KV_BtreeArray::cnt() { return bt_map.size(); };
...
{
  fprintf(stderr,
          "In memory Standard Query Cache (SQC) rev. %s -- %s -- %s\n",
...
unsigned char *Query_Cache::get(uint64_t user_hash, const unsigned char *kp,
                                const uint32_t kl, uint32_t *lv,
                                unsigned long long curtime_ms)
{
...
bool Query_Cache::set(uint64_t user_hash, const unsigned char *kp,
                      uint32_t kl, unsigned char *vp, uint32_t vl,
                      unsigned long long curtime_ms,
...
void *Query_Cache::purgeHash_thread(void *)
{
  ...
  while (shutdown == 0)
  {
    ...
    if (curr_pct < purge_threshold_pct_min)
      continue;
    for (i= 0; i < SHARED_QUERY_CACHE_HASH_TABLES; i++)
    {
