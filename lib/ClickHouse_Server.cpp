...
inline void ClickHouse_to_MySQL(const Block &block)
{
  ...
  ClickHouse_Session *clickhouse_sess=
      (ClickHouse_Session *)sess->thread->gen_args;
  ...
  if (clickhouse_sess->transfer_started == false)
  {
    ...
    myprot->generate_pkt_column_count(true, NULL, NULL, sid,
...
*/
  ...
  unsigned long *l=
      (unsigned long *)malloc(sizeof(unsigned long *) * columns);
  int rows= block.GetRowCount();
  for (int r= 0; r < rows; r++)
  {
    for (int i= 0; i < columns; i++)
    {
      ...
      switch (cc)
      {
        ...
        case clickhouse::Type::Code::Nullable:
        {
          ...
          {
            clickhouse::Type::Code cnc=
                block[i]->Type()->GetNestedType()->GetCode();
            switch (cnc)
            {
              case clickhouse::Type::Code::Int8:
    ...
    myprot->generate_pkt_row(true, NULL, NULL, sid, columns, l, p);
    sid++;
    for (int i= 0; i < columns; i++)
    {
      free(p[i]);
...
static void StringToHex(unsigned char *string, unsigned char *hexstring,
                        size_t l)
{
  unsigned char ch;
  size_t        i, j;
...
static char *ClickHouse_Server_variables_names[]= {
    (char *)"hostname", (char *)"mysql_ifaces", (char *)"read_only",
    (char *)"port", NULL};
...
class sqlite3server_main_loop_listeners {
 ...
 public:
  ...
  void           wrunlock() { pthread_rwlock_unlock(&rwlock); }
...
void ClickHouse_Server_session_handler(MySQL_Session *sess, void *_pa,
                                       PtrSize_t *pkt)
{
  ...
  if (sess->session_type == PROXYSQL_SESSION_CLICKHOUSE)
  {
    ...
    {
      if (!strncasecmp("SET AUTOCOMMIT", query_no_space, 14) ||
    ...
    {
      if (!strncasecmp("SHOW COLUMNS FROM ", query_no_space, 18))
      {
      ...
      if (!strncasecmp("SHOW SESSION STATUS LIKE ", query_no_space, 25) ||
          ...
          !strncasecmp("SHOW VARIABLES LIKE ", query_no_space, 20))
      {
        ...
        char *q= (char *)malloc(query_length + 256);
        sprintf(q,
                "SELECT variable_name Variable_name, Variable_value Value "
                "FROM global_variables WHERE Variable_name LIKE %s",
                query_no_space + offset);
        //fprintf(stderr,"%s\n",q);
        query= l_strdup(q);
        query_length= strlen(query) + 1;
        free(q);
      ...
      if ((query_no_space_length == strlen("SHOW GLOBAL VARIABLES") &&
          ...
          (query_no_space_length == strlen("SHOW ALL VARIABLES") &&
           !strncasecmp("SHOW ALL VARIABLES", query_no_space,
                        query_no_space_length)) ||
          (query_no_space_length == strlen("SHOW GLOBAL STATUS") &&
           !strncasecmp("SHOW GLOBAL STATUS", query_no_space,
                        query_no_space_length)) ||
          (query_no_space_length == strlen("SHOW VARIABLES") &&
           !strncasecmp("SHOW VARIABLES", query_no_space,
                        query_no_space_length)))
      {
        l_free(query_length, query);
        query= l_strdup(
            "SELECT variable_name AS Variable_name, variable_value AS Value "
            "FROM global_variables ORDER BY variable_name");
        query_length= strlen(query) + 1;
        run_query_sqlite= true;
        goto __run_query_sqlite;
      }
      if ((query_no_space_length == strlen("SHOW ENGINES") &&
           !strncasecmp("SHOW ENGINES", query_no_space,
                        query_no_space_length)))
      {
        l_free(query_length, query);
      ...
      if ((pkt->size == (strlen("show charset") + 5) &&
           strncasecmp((char *)"show charset", (char *)pkt->ptr + 5,
                       pkt->size - 5) == 0))
      {
        l_free(query_length, query);
        query= l_strdup(
      ...
      if ((pkt->size == (strlen("show collation") + 5) &&
           strncasecmp((char *)"show collation", (char *)pkt->ptr + 5,
                       pkt->size - 5) == 0))
      {
        l_free(query_length, query);
      ...
      if ((pkt->size == (strlen("SHOW FULL TABLES FROM `default`") + 5) &&
           strncasecmp((char *)"SHOW FULL TABLES FROM `default`",
                       (char *)pkt->ptr + 5, pkt->size - 5) == 0))
      {
        l_free(query_length, query);
        query= l_strdup(
            "SELECT name, 'BASE TABLE' AS Table_type FROM system.tables "
    ...
    if ((pkt->size ==
             (strlen("SELECT * FROM INFORMATION_SCHEMA.CHARACTER_SETS") +
              5) &&
         strncasecmp(
             (char *)"SELECT * FROM INFORMATION_SCHEMA.CHARACTER_SETS",
             (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
      query= l_strdup(
          "SELECT Charset AS CHARACTER_SET_NAME , Collation AS "
          "DEFAULT_COLLATE_NAME, 'UTF-8 Unicode' AS DESCRIPTION , 3 AS LEN "
          "FROM mysql_collations WHERE `Default`='Yes'");
      query_length= strlen(query) + 1;
      run_query_sqlite= true;
      goto __run_query_sqlite;
    }

    if ((pkt->size == (strlen("SELECT @@character_set_results") + 5) &&
         strncasecmp((char *)"SELECT @@character_set_results",
                     (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen("SELECT @@collation_server") + 5) &&
         strncasecmp((char *)"SELECT @@collation_server",
                     (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen("SELECT @@have_profiling") + 5) &&
         strncasecmp((char *)"SELECT @@have_profiling", (char *)pkt->ptr + 5,
                     pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen("SELECT @@lower_case_table_names") + 5) &&
         strncasecmp((char *)"SELECT @@lower_case_table_names",
                     (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen("SELECT @@version, @@version_comment") + 5) &&
         strncasecmp((char *)"SELECT @@version, @@version_comment",
                     (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
      query= l_strdup(
          "SELECT '5.7.19-ProxySQL-ClickHouse' AS '@@version', "
          "'(ProxySQL-ClickHouse)' AS '@@version_comment'");
      query_length= strlen(query) + 1;
      run_query_sqlite= true;
      goto __run_query_sqlite;
    }
    if ((pkt->size == (strlen("SELECT @@storage_engine;") + 5) &&
         strncasecmp((char *)"SELECT @@storage_engine;", (char *)pkt->ptr + 5,
                     pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    {
      if (!strncasecmp((char *)"SELECT CURRENT_USER()", query_no_space,
                       query_no_space_length))
      {
        l_free(query_length, query);
    ...
    {
      if (!strncasecmp((char *)"SELECT USER()", query_no_space,
                       query_no_space_length))
      {
        l_free(query_length, query);
    ...
    if ((pkt->size ==
             (strlen("SELECT * FROM INFORMATION_SCHEMA.COLLATIONS") + 5) &&
         strncasecmp((char *)"SELECT * FROM INFORMATION_SCHEMA.COLLATIONS",
                     (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
      query= l_strdup(
          "SELECT Collation AS COLLATION_NAME, Charset AS "
          "CHARACTER_SET_NAME, Id AS ID, 'Default' AS IS_DEFAULT, 'Yes' AS "
          "IS_COMPILED, '3' AS SORTLEN FROM mysql_collations");
      query_length= strlen(query) + 1;
      run_query_sqlite= true;
      goto __run_query_sqlite;
    }
    if (!strncasecmp("/*!40101 SET ", query_no_space, 13))
    {
      GloClickHouseServer->send_MySQL_OK(&sess->client_myds->myprot, NULL);
      run_query= false;
      goto __run_query;
    ...
    if (((query_no_space_length > 40) &&
         strncasecmp("SELECT DEFAULT_COLLATION_NAME FROM "
                     "information_schema.SCHEMATA WHERE SC",
                     query_no_space,
                     strlen("SELECT DEFAULT_COLLATION_NAME FROM "
                            "information_schema.SCHEMATA WHERE SC") == 0)))
    {
      l_free(query_length, query);
                 ...
                 "GROUP BY BINARY s.SCHEMA_NAME, s.DEFAULT_COLLATION_NAME"))))
    {
      l_free(query_length, query);
      query= l_strdup(
          "SELECT name AS BIN_NAME, 'utf8_general_ci' AS "
          "DEFAULT_COLLATION_NAME, name AS SCHEMA_NAME FROM "
          "system.databases");
      query_length= strlen(query) + 1;
      goto __run_query;
    }

    if ((pkt->size == (strlen("SELECT `SCHEMA_NAME` FROM "
                              "`INFORMATION_SCHEMA`.`SCHEMATA`") +
                       5) &&
         strncasecmp((char *)"SELECT `SCHEMA_NAME` FROM "
                             "`INFORMATION_SCHEMA`.`SCHEMATA`",
                     (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen("SELECT version()") + 5) &&
         strncasecmp((char *)"SELECT version()", (char *)pkt->ptr + 5,
                     pkt->size - 5) == 0))
    {
      l_free(query_length, query);
      char *q= (char *)malloc(query_length + 256);
      sprintf(q,
              "SELECT Variable_value 'version' FROM global_variables WHERE "
    ...
    if ((pkt->size ==
             (strlen("select name, type FROM mysql.proc where db='default'") +
              5) &&
         strncasecmp(
             (char *)"select name, type FROM mysql.proc where db='default'",
             (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
      char *q= (char *)malloc(query_length + 256);
    ...
    if ((pkt->size == (strlen((char *)"SELECT logfile_group_name FROM "
                                      "information_schema.FILES") +
                       5) &&
         strncasecmp(
             (char
                  *)"SELECT logfile_group_name FROM information_schema.FILES",
             (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen((char *)"SELECT tablespace_name FROM "
                                      "information_schema.FILES") +
                       5) &&
         strncasecmp(
             (char *)"SELECT tablespace_name FROM information_schema.FILES",
             (char *)pkt->ptr + 5, pkt->size - 5) == 0))
    {
      l_free(query_length, query);
    ...
    if ((pkt->size == (strlen("SELECT CONNECTION_ID()") + 5) &&
         strncasecmp((char *)"SELECT CONNECTION_ID()", (char *)pkt->ptr + 5,
                     pkt->size - 5) == 0))
    {
      char buf[16];
      ...
      myprot->generate_pkt_field(
          true, NULL, NULL, sid, (char *)"", (char *)"", (char *)"",
          (char *)"CONNECTION_ID()", (char *)"", 63, 31, MYSQL_TYPE_LONGLONG,
          161, 0, false, 0, NULL);
      sid++;
      myds->DSS= STATE_COLUMN_DEFINITION;
      myprot->generate_pkt_EOF(true, NULL, NULL, sid, 0, setStatus);
    ...
    if ((pkt->size == (strlen("SELECT current_user()") + 5) &&
         strncasecmp((char *)"SELECT current_user()", (char *)pkt->ptr + 5,
                     pkt->size - 5) == 0))
    {
      char buf[32];
      ...
      myprot->generate_pkt_field(
          true, NULL, NULL, sid, (char *)"", (char *)"", (char *)"",
          (char *)"current_user()", (char *)"", 33, 15, MYSQL_TYPE_VAR_STRING,
          1, 0x1f, false, 0, NULL);
      sid++;
      myds->DSS= STATE_COLUMN_DEFINITION;
      myprot->generate_pkt_EOF(true, NULL, NULL, sid, 0, setStatus);
...
*/
  if ((query_no_space_length == strlen("SHOW DATABASES") &&
      ...
      (query_no_space_length == strlen("SHOW DATABASES LIKE '%'") &&
       !strncasecmp("SHOW DATABASES LIKE '%'", query_no_space,
                    query_no_space_length)))
  {
    l_free(query_length, query);
  ...
  if ((query_no_space_length > 24) &&
      (!strncasecmp("SHOW TABLE STATUS FROM `", query_no_space, 24)))
  {
...
__end_show_commands:
  ...
  if ((query_no_space_length > 50) &&
      (!strncasecmp("SELECT TABLE_NAME ", query_no_space, 18)))
  {
    if ((strstr(query_no_space, "information_schema.VIEWS")))
    {
      l_free(query_length, query);
      char *q=
          (char *)"SELECT name AS TABLE_NAME FROM system.tables WHERE 1=0";
      //fprintf(stderr,"%s\n",q);
      query= l_strdup(q);
      query_length= strlen(query) + 1;
      goto __run_query;
...
*/
  ...
  if (sess->session_type == PROXYSQL_SESSION_SQLITE)
  {  // no admin
        ...
        (strncasecmp("ATTACH", query_no_space, 6) == 0))
    {
      proxy_error(
          ...
          "Module: \"%s\"\n",
          query_no_space);
      GloClickHouseServer->send_MySQL_ERR(&sess->client_myds->myprot,
  ...
  if (sess->session_type == PROXYSQL_SESSION_CLICKHOUSE)
  {  // no admin
    if ((strncasecmp("SHOW SESSION VARIABLES", query_no_space, 22) == 0) ||
    ...
    if ((strncasecmp("SHOW MASTER STATUS", query_no_space, 18) == 0) ||
    ...
    if ((strncasecmp("LOCK TABLE", query_no_space, 10) == 0) ||
...
*/
  if (run_query)
  {
    ...
    if (supported_command == false &&
        strncasecmp("SELECT ", query_no_space, 7) == 0)
    {
    ...
    if (supported_command == false &&
        strncasecmp("INSERT ", query_no_space, 7) == 0)
    {
    ...
    if (supported_command == false &&
        strncasecmp("SET ", query_no_space, 4) == 0)
    {
      supported_command= true;
      expected_resultset= false;
    }
    if (supported_command == false &&
        strncasecmp("USE ", query_no_space, 4) == 0)
    {
      supported_command= true;
      expected_resultset= false;
    }
    if (supported_command == false)
    {
      if ((strncasecmp("CREATE ", query_no_space, 7) == 0) ||
    ...
    if (supported_command == false)
    {
      if ((strncasecmp("SHOW ", query_no_space, 5) == 0) ||
...
*/
    if (supported_command == false)
    {
      ...
      MySQL_Data_Stream *myds= myprot->get_myds();
      myds->DSS= STATE_QUERY_SENT_DS;
      myprot->generate_pkt_ERR(true, NULL, NULL, 1, 1148, (char *)"42000",
    ...
    else
    {
        ...
        if (clickhouse_sess->connected == true)
        {
          if (clickhouse_sess->schema_initialized == false)
          {
            if (sess && sess->client_myds)
            {
              ...
              if (ds->myconn && ds->myconn->userinfo &&
                  ds->myconn->userinfo->schemaname)
              {
          ...
          {
            clickhouse_sess->client->Select(query, [](const Block &block) {
            ...
            {
              myprot->generate_pkt_EOF(true, NULL, NULL, clickhouse_sess->sid,
                                       0, 2);
              clickhouse_sess->sid++;
            }
            else
            {
              myprot->generate_pkt_OK(true, NULL, NULL, 1, 0, 0, 2, 0,
                                      (char *)"");
            }
            myds->DSS= STATE_SLEEP;
      ...
      } catch (const std::exception &e)
      {
        ...
        myds->DSS= STATE_SLEEP;
        std::cerr << "Exception in query for ClickHouse: " << e.what()
                  << std::endl;
        sess->set_unhealthy();
...
__run_query_sqlite
    :  // we are introducing this new section to send some query to internal sqlite to simplify the execution of dummy queries

  if (run_query_sqlite)
  {
    ClickHouse_Session *sqlite_sess=
        (ClickHouse_Session *)sess->thread->gen_args;
    sqlite_sess->sessdb->execute_statement(query, &error, &cols,
...
ClickHouse_Session::ClickHouse_Session()
{
  ...
  sessdb->open(
      (char *)"file:mem_sqlitedb_clickhouse?mode=memory&cache=shared",
...
static void *child_mysql(void *arg)
{
  ...
  while (__sync_fetch_and_add(&glovars.shutdown, 0) == 0)
  {
    ...
    fds[0].revents= 0;
    rc= poll(fds, nfds,
             __sync_fetch_and_add(&__ClickHouse_Server_refresh_interval, 0));
    if (rc == -1)
    {
      if (errno == EINTR)
      {
...
static void *sqlite3server_main_loop(void *arg)
{
  ...
  for (i= 0; i < MAX_SQLITE3SERVER_LISTENERS; i++)
  {
    socket_names[i]= NULL;
  }
  pthread_attr_t attr;
...
ClickHouse_Server::ClickHouse_Server()
{
  ...
  SQLite_General_DB->open(
      (char *)"file:mem_sqlitedb_clickhouse?mode=memory&cache=shared",
      SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX);
  SQLite_General_DB->execute((
      char *)"CREATE TABLE global_variables (variable_name VARCHAR NOT NULL "
             "PRIMARY KEY , variable_value VARCHAR NOT NULL)");
  SQLite_General_DB->execute((
      char *)"INSERT INTO global_variables VALUES "
             "('lower_case_table_names','0')");
  SQLite_General_DB->execute(
      (char *)"INSERT INTO global_variables VALUES ('sql_mode','')");
  SQLite_General_DB->execute((
      char *)"INSERT INTO global_variables VALUES "
             "('version','5.5.30-clickhouse')");
  SQLite_General_DB->execute((
      char *)"INSERT INTO global_variables VALUES "
             "('version_comment','(ProxySQL ClickHouse Module)')");
  SQLite_General_DB->execute(
      (char *)"INSERT INTO global_variables VALUES ('wait_timeout','3600')");
  SQLite_General_DB->execute((
      char *)"INSERT INTO global_variables VALUES "
             "('interactive_wait_timeout','3600')");

  SQLite_General_DB->execute((
      char *)"CREATE TABLE mysql_collations (Id INTEGER NOT NULL PRIMARY KEY "
             ", Collation VARCHAR NOT NULL , Charset VARCHAR NOT NULL , "
             "`Default` VARCHAR NOT NULL)");
  dump_mysql_collations();
  SQLite_General_DB->execute((
      char *)"CREATE TABLE show_engines (Engine VARCHAR , Support VARCHAR , "
             "Comment VARCHAR , Transactions VARCHAR , XA VARCHAR , "
             "Savepoints)");
  SQLite_General_DB->execute((
      char *)"INSERT INTO show_engines VALUES "
             "('ClickHouse','DEFAULT','ProxySQL frontend to "
...
void ClickHouse_Server::wrlock() { pthread_rwlock_wrlock(&rwlock); };

void ClickHouse_Server::wrunlock() { pthread_rwlock_unlock(&rwlock); };
...
*/
  main_callback_func=
      (int *)malloc(sizeof(int) * MAX_SQLITE3SERVER_LISTENERS);
  main_poll_fds= (struct pollfd *)malloc(sizeof(struct pollfd) *
                                         MAX_SQLITE3SERVER_LISTENERS);
  main_poll_nfds= 0;
...
*/
  ...
  if (pthread_create(&ClickHouse_Server_thr, NULL, sqlite3server_main_loop,
                     (void *)arg) != 0)
  {
    perror("Thread creation");
    exit(EXIT_FAILURE);
...
char **ClickHouse_Server::get_variables_list()
{
  ...
  {
    ret[i]=
        (i == l - 1 ? NULL : strdup(ClickHouse_Server_variables_names[i]));
  }
  return ret;
}
...
bool ClickHouse_Server::has_variable(const char *name)
{
  ...
  for (unsigned int i= 0; i < no_vars - 1; ++i)
  {
    ...
    if (strlen(name) == var_len &&
        !strncmp(name, ClickHouse_Server_variables_names[i], var_len))
    {
      return true;
    }
  }
...
*/
  if (!strcasecmp(name, "hostname"))
    return s_strdup(variables.hostname);
...
*/
bool ClickHouse_Server::set_variable(char *name, char *value)
...
void ClickHouse_Server::send_MySQL_OK(MySQL_Protocol *myprot, char *msg,
                                      int rows)
{
  assert(myprot);
  MySQL_Data_Stream *myds= myprot->get_myds();
