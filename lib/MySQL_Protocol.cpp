...
#ifdef DEBUG
static void __dump_pkt(const char *func, unsigned char *_ptr,
                       unsigned int len)
{
  if (GloVars.global.gdbg == 0)
    return;
  if (GloVars.global.gdbg_lvl[PROXY_DEBUG_MYSQL_PROTOCOL].verbosity < 8)
    return;
  unsigned int i;
...
void proxy_create_random_string(char *to, uint length,
...
static inline int write_encoded_length(unsigned char *p, uint64_t val,
                                       uint8_t len, char prefix)
{
  if (len == 1)
  {
    *p= (char)val;
...
static inline int write_encoded_length_and_string(unsigned char *p,
                                                  uint64_t val, uint8_t len,
                                                  char prefix, char *string)
{
...
void proxy_compute_sha1_hash_multi(uint8 *digest, const char *buf1, int len1,
...
void proxy_compute_two_stage_sha1_hash(const char *password, size_t pass_len,
                                       uint8 *hash_stage1, uint8 *hash_stage2)
{
  proxy_compute_sha1_hash(hash_stage1, password, pass_len);
  proxy_compute_sha1_hash(hash_stage2, (const char *)hash_stage1,
                          SHA_DIGEST_LENGTH);
}
...
bool proxy_scramble_sha1(char *pass_reply, const char *message,
                         const char *sha1_sha1_pass, char *sha1_pass)
{
  bool  ret= false;
  uint8 hash_stage1[SHA_DIGEST_LENGTH];
  uint8 hash_stage2[SHA_DIGEST_LENGTH];
  ...
  proxy_compute_sha1_hash_multi((uint8 *)to, message, SCRAMBLE_LENGTH,
                                (const char *)hash_stage2, SHA_DIGEST_LENGTH);
  proxy_my_crypt((char *)hash_stage1, (const uchar *)pass_reply, to,
                 SCRAMBLE_LENGTH);
  proxy_compute_sha1_hash(hash_stage3, (const char *)hash_stage1,
                          SHA_DIGEST_LENGTH);
...
enum MySQL_response_type mysql_response(unsigned char *pkt,
                                        unsigned int   length)
{
  unsigned char c= *pkt;
  switch (c)
...
int pkt_ok(unsigned char *pkt, unsigned int length, MySQL_Protocol *mp)
{
  ...
  proxy_debug(
      PROXY_DEBUG_MYSQL_PROTOCOL, 1,
      "OK Packet <affected_rows:%u insert_id:%u status:%u warns:%u msg:%s>\n",
      (uint32_t)affected_rows, (uint32_t)insert_id, (uint16_t)mp->prot_status,
      (uint16_t)warns, msg);
...
MySQL_Prepared_Stmt_info::MySQL_Prepared_Stmt_info(unsigned char *pkt,
                                                   unsigned int   length)
{
  pkt+= 5;
...
void MySQL_Protocol::init(MySQL_Data_Stream **       __myds,
                          MySQL_Connection_userinfo *__userinfo,
                          MySQL_Session *            __sess)
{
  myds= __myds;
  userinfo= __userinfo;
  sess= __sess;
...
int MySQL_Protocol::parse_mysql_pkt(PtrSize_t *        PS_entry,
                                    MySQL_Data_Stream *__myds)
{
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 1,
              "MySQL Packet length=%d, senquence_id=%d, addr=%p\n",
  ...
  switch (*DSS)
  {
    ...
    case STATE_NOT_CONNECTED:
      if (from == MYDS_FRONTEND)
      {  // at this stage we expect a packet from the server, not from client
        return PKT_ERROR;
      }
      break;
    ...
    default:
      ...
      switch (c)
      {
        case OK_Packet:
          ...
          break;
        case EOF_Packet: pkt_end(payload, hdr.pkt_length, this); break;
        default:
          return PKT_ERROR;  // from the server we expect either an OK or an ERR. Everything else is wrong
      }
...
bool MySQL_Protocol::generate_statistics_response(bool send, void **ptr,
                                                  unsigned int *len)
{
  ...
  sprintf(buf1,
          "Uptime: %llu Threads: %d  Questions: %llu  Slow queries: %llu",
          (t1 - GloVars.global.start_time) / 1000 / 1000,
          MyHGM->status.client_connections, GloMTH->get_total_queries(),
  ...
  if (dump_pkt)
  {
    __dump_pkt(__func__, _ptr, size);
  }
#endif
  return true;
}

bool MySQL_Protocol::generate_pkt_EOF(bool send, void **ptr,
                                      unsigned int *len, uint8_t sequence_id,
                                      uint16_t warnings, uint16_t status,
                                      MySQL_ResultSet *myrs)
{
  ...
  memcpy(_ptr + l, &warnings, sizeof(uint16_t));
  l+= sizeof(uint16_t);
  ...
  if (send == true)
  {
    ...
    switch ((*myds)->DSS)
    {
      case STATE_COLUMN_DEFINITION: (*myds)->DSS= STATE_EOF1; break;
      case STATE_ROW: (*myds)->DSS= STATE_EOF2; break;
      default:
        //assert(0);
        break;
    }
...
bool MySQL_Protocol::generate_pkt_ERR(bool send, void **ptr,
                                      unsigned int *len, uint8_t sequence_id,
                                      uint16_t error_code, char *sql_state,
                                      char *sql_message)
{
  if ((*myds)->sess->mirror == true)
  {
  ...
  _ptr[l]= 0xff;
  l++;
  memcpy(_ptr + l, &error_code, sizeof(uint16_t));
  l+= sizeof(uint16_t);
  _ptr[l]= '#';
  l++;
  memcpy(_ptr + l, sql_state, 5);
  l+= 5;
  if (sql_message)
    memcpy(_ptr + l, sql_message, sql_message_len);
  ...
  if (send == true)
  {
    ...
    switch ((*myds)->DSS)
    {
      ...
      case STATE_QUERY_SENT_NET: (*myds)->DSS= STATE_ERR; break;
      case STATE_OK: break;
      case STATE_SLEEP:
...
bool MySQL_Protocol::generate_pkt_OK(bool send, void **ptr, unsigned int *len,
                                     uint8_t      sequence_id,
                                     unsigned int affected_rows,
                                     uint64_t last_insert_id, uint16_t status,
                                     uint16_t warnings, char *msg)
{
  if ((*myds)->sess->mirror == true)
  {
    return true;
  ...
  uint8_t affected_rows_len=
      mysql_encode_length(affected_rows, &affected_rows_prefix);
  char    last_insert_id_prefix;
  uint8_t last_insert_id_len=
      mysql_encode_length(last_insert_id, &last_insert_id_prefix);
  ...
  _ptr[l]= 0x00;
  l++;
  l+= write_encoded_length(_ptr + l, affected_rows, affected_rows_len,
                           affected_rows_prefix);
  ...
  if (send == true)
  {
    ...
    switch ((*myds)->DSS)
    {
      case STATE_CLIENT_HANDSHAKE:
      case STATE_QUERY_SENT_DS:
      case STATE_QUERY_SENT_NET: (*myds)->DSS= STATE_OK; break;
      case STATE_OK: break;
      default: assert(0);
    }
  }
...
bool MySQL_Protocol::generate_pkt_column_count(bool send, void **ptr,
                                               unsigned int *   len,
                                               uint8_t          sequence_id,
                                               uint64_t         count,
                                               MySQL_ResultSet *myrs)
...
bool MySQL_Protocol::generate_pkt_field(
    bool send, void **ptr, unsigned int *len, uint8_t sequence_id,
    char *schema, char *table, char *org_table, char *name, char *org_name,
    uint16_t charset, uint32_t column_length, uint8_t type, uint16_t flags,
    uint8_t decimals, bool field_list, uint64_t defvalue_length,
    char *defvalue, MySQL_ResultSet *myrs)
{
  if ((*myds)->sess->mirror == true)
  {
    return true;
  ...
  uint8_t  org_table_len=
      mysql_encode_length(org_table_strlen, &org_table_prefix);
  ...
  uint8_t  org_name_len=
      mysql_encode_length(org_name_strlen, &org_name_prefix);


  char    defvalue_length_prefix;
  uint8_t defvalue_length_len=
      mysql_encode_length(defvalue_length, &defvalue_length_prefix);

  mysql_hdr myhdr;
  myhdr.pkt_id= sequence_id;
  myhdr.pkt_length= def_len + def_strlen + schema_len + schema_strlen +
                    table_len + table_strlen + org_table_len +
                    org_table_strlen + name_len + name_strlen + org_name_len +
                    org_name_strlen + 1  // filler
  ...
  l+= write_encoded_length_and_string(_ptr + l, def_strlen, def_len,
                                      def_prefix, def);
  l+= write_encoded_length_and_string(_ptr + l, schema_strlen, schema_len,
                                      schema_prefix, schema);
  l+= write_encoded_length_and_string(_ptr + l, table_strlen, table_len,
                                      table_prefix, table);
  l+= write_encoded_length_and_string(
      _ptr + l, org_table_strlen, org_table_len, org_table_prefix, org_table);
  l+= write_encoded_length_and_string(_ptr + l, name_strlen, name_len,
                                      name_prefix, name);
  l+= write_encoded_length_and_string(_ptr + l, org_name_strlen, org_name_len,
                                      org_name_prefix, org_name);
  _ptr[l]= 0x0c;
  l++;
  memcpy(_ptr + l, &charset, sizeof(uint16_t));
  l+= sizeof(uint16_t);
  memcpy(_ptr + l, &column_length, sizeof(uint32_t));
  l+= sizeof(uint32_t);
  _ptr[l]= type;
  l++;
  uint16_t flags_= flags;
  flags_= flags_ & ~NUM_FLAG;
  memcpy(_ptr + l, &flags_, sizeof(uint16_t));
  l+= sizeof(uint16_t);
  _ptr[l]= decimals;
  l++;
  _ptr[l]= 0x00;
  ...
  if (field_list)
  {
    l+= write_encoded_length_and_string(_ptr + l, strlen(defvalue),
                                        defvalue_length_len,
                                        defvalue_length_prefix, defvalue);
  }
  //else _ptr[l]=0x00;
...
bool MySQL_Protocol::generate_STMT_PREPARE_RESPONSE(
    uint8_t sequence_id, MySQL_STMT_Global_info *stmt_info, uint32_t _stmt_id)
{
  ...
  if (stmt_info->num_params)
  {
    ...
    {
      generate_pkt_field(true, NULL, NULL, sid, (char *)"", (char *)"",
                         (char *)"", (char *)"?", (char *)"", 63, 0, 253, 128,
                         0, false, 0,
                         NULL);  // NOTE: charset is 63 = binary !
      sid++;
    }
  ...
  if (stmt_info->num_columns)
  {
    for (i= 0; i < stmt_info->num_columns; i++)
    {
      ...
      generate_pkt_field(true, NULL, NULL, sid, fd->db, fd->table,
                         fd->org_table, fd->name, fd->org_name, fd->charsetnr,
                         fd->length, fd->type, fd->flags, fd->decimals, false,
                         0, NULL);
      sid++;
    }
    generate_pkt_EOF(true, NULL, NULL, sid, 0, setStatus);
...
bool MySQL_Protocol::generate_pkt_row(bool send, void **ptr,
                                      unsigned int *len, uint8_t sequence_id,
                                      int colnums, unsigned long *fieldslen,
                                      char **fieldstxt)
{
  int col= 0;
  int rowlen= 0;
  ...
  {
    rowlen+= (fieldstxt[col]
                  ? fieldslen[col] + mysql_encode_length(fieldslen[col], NULL)
                  : 1);
  }
  mysql_hdr myhdr;
  myhdr.pkt_id= sequence_id;
  ...
  for (col= 0; col < colnums; col++)
  {
    if (fieldstxt[col])
    {
      ...
      l+= write_encoded_length_and_string(_ptr + l, fieldslen[col],
                                          length_len, length_prefix,
                                          fieldstxt[col]);
    }
    else
    {
...
uint8_t MySQL_Protocol::generate_pkt_row3(MySQL_ResultSet *myrs,
                                          unsigned int *   len,
                                          uint8_t sequence_id, int colnums,
                                          unsigned long *fieldslen,
                                          char **        fieldstxt)
{
  if ((*myds)->sess->mirror == true)
  {
  ...
  for (col= 0; col < colnums; col++)
  {
    if (fieldstxt[col])
    {
      char    length_prefix;
      uint8_t length_len= mysql_encode_length(fieldslen[col], &length_prefix);
      l+= write_encoded_length_and_string((unsigned char *)pkt.ptr + l,
                                          fieldslen[col], length_len,
                                          length_prefix, fieldstxt[col]);
    }
    else
    {
  ...
  else
  {
    ...
    while (left >= (0xFFFFFF + sizeof(mysql_hdr)))
    {
      ...
      memcpy((char *)pkt2.ptr + sizeof(mysql_hdr),
             (char *)pkt.ptr + sizeof(mysql_hdr) + copied, 0xFFFFFF);
      mysql_hdr myhdr;
      myhdr.pkt_id= pkt_sid;
      pkt_sid++;
    ...
    pkt2.ptr= l_alloc(pkt2.size);
    memcpy((char *)pkt2.ptr + sizeof(mysql_hdr),
           (char *)pkt.ptr + sizeof(mysql_hdr) + copied,
           left - sizeof(mysql_hdr));
    mysql_hdr myhdr;
    myhdr.pkt_id= pkt_sid;
  ...
  if (len)
  {
    *len= pkt.size + (pkt_sid - sequence_id) * sizeof(mysql_hdr);
  }
  if (pkt.size >= (0xFFFFFF + sizeof(mysql_hdr)))
  {
...
bool MySQL_Protocol::generate_pkt_auth_switch_request(bool send, void **ptr,
                                                      unsigned int *len)
{
  proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 7,
              "Generating auth switch request pkt\n");
  mysql_hdr myhdr;
  myhdr.pkt_id= 1;
  myhdr.pkt_length= 1                                             // fe
                    + (strlen("mysql_native_password") + 1) + 20  // scramble
                    + 1;                                          // 00
  unsigned int   size= myhdr.pkt_length + sizeof(mysql_hdr);
  ...
  _ptr[l]= 0x00;
  l++;
  memcpy(_ptr + l, (*myds)->myconn->scramble_buff + 0, 20);
  l+= 20;
...
bool MySQL_Protocol::generate_pkt_initial_handshake(bool send, void **ptr,
                                                    unsigned int *len,
                                                    uint32_t *    _thread_id)
{
  ...
  myhdr.pkt_length= sizeof(protocol_version) +
                    ...
                    sizeof(server_status) + 3  // unknown stuff
                    + 10                       // filler
                    + 12                       // scramble2
                    + 1                        // 0x00
  ...
  *_thread_id= thread_id;
  ...
  memcpy(_ptr + l, &protocol_version, sizeof(protocol_version));
  l+= sizeof(protocol_version);
  memcpy(_ptr + l, mysql_thread___server_version,
         strlen(mysql_thread___server_version));
  l+= strlen(mysql_thread___server_version) + 1;
  memcpy(_ptr + l, &thread_id, sizeof(uint32_t));
  l+= sizeof(uint32_t);
  //#ifdef MARIADB_BASE_VERSION
  //  proxy_create_random_string(myds->myconn->myconn.scramble_buff+0,8,(struct my_rnd_struct *)&rand_st);
  //#else
  proxy_create_random_string((*myds)->myconn->scramble_buff + 0, 8,
                             (struct rand_struct *)&rand_st);
  //#endif
  ...
  memcpy(_ptr + l, (*myds)->myconn->scramble_buff + 0, 8);
  l+= 8;
  _ptr[l]= 0x00;
  l+= 1;  //0x00
  if (mysql_thread___have_compress)
  {
    mysql_thread___server_capabilities|=
        CLIENT_COMPRESS;  // FIXME: shouldn't be here
  }
  (*myds)->myconn->options.server_capabilities=
      mysql_thread___server_capabilities;
  memcpy(_ptr + l, &mysql_thread___server_capabilities,
         sizeof(mysql_thread___server_capabilities));
  l+= sizeof(mysql_thread___server_capabilities);
  memcpy(_ptr + l, &mysql_thread___default_charset,
         sizeof(mysql_thread___default_charset));
  ...
  //#else
  proxy_create_random_string((*myds)->myconn->scramble_buff + 8, 12,
                             (struct rand_struct *)&rand_st);
  //#endif
  ...
  memcpy(_ptr + l, (*myds)->myconn->scramble_buff + 8, 12);
  l+= 12;
  l+= 1;  //0x00
  memcpy(_ptr + l, "mysql_native_password", strlen("mysql_native_password"));
...
bool MySQL_Protocol::process_pkt_OK(unsigned char *pkt, unsigned int len)
{
  ...
  proxy_debug(
      PROXY_DEBUG_MYSQL_PROTOCOL, 1,
      "OK Packet <affected_rows:%u insert_id:%u status:%u warns:%u msg:%s>\n",
      (uint32_t)affected_rows, (uint32_t)insert_id, (uint16_t)prot_status,
      (uint16_t)warns, msg);
...
bool MySQL_Protocol::process_pkt_COM_QUERY(unsigned char *pkt,
                                           unsigned int   len)
{
  bool ret= false;
...
bool MySQL_Protocol::process_pkt_auth_swich_response(unsigned char *pkt,
                                                     unsigned int   len)
{
  bool  ret= false;
  char *password= NULL;
  ...
  {
#ifdef PROXYSQLCLICKHOUSE
    password= GloClickHouseAuth->lookup(
        (char *)userinfo->username, USERNAME_FRONTEND, &_ret_use_ssl,
        &default_hostgroup, NULL, NULL, &transaction_persistent, NULL, NULL,
        &sha1_pass);
#endif /* PROXYSQLCLICKHOUSE */
  }
  else
  ...
  else
  {
    ...
    {
      ret= proxy_scramble_sha1((char *)pass, (*myds)->myconn->scramble_buff,
                               password + 1, reply);
      if (ret)
      {
        if (sha1_pass == NULL)
        {
          ...
          GloMyAuth->set_SHA1((char *)userinfo->username, USERNAME_FRONTEND,
                              reply);
        }
...
bool MySQL_Protocol::process_pkt_COM_CHANGE_USER(unsigned char *pkt,
                                                 unsigned int   len)
{
  bool           ret= false;
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          ret= proxy_scramble_sha1((char *)pass,
                                   (*myds)->myconn->scramble_buff,
                                   password + 1, reply);
          if (ret)
          {
            if (sha1_pass == NULL)
  ...
  if (ret == true)
  {
    ...
    if (db)
      userinfo->set_schemaname(db, strlen(db));
  }
  else
  {
...
bool MySQL_Protocol::process_pkt_handshake_response(unsigned char *pkt,
                                                    unsigned int   len)
{
  bool              ret= false;
  uint8_t           charset;
  ...
  else
  {
    ...
    pass_len= (capabilities & CLIENT_SECURE_CONNECTION ? *pkt++
                                                       : strlen((char *)pkt));
    ...
    (*myds)->sess->default_schema=
        default_schema;  // just the pointer is passed
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          if (session_type == PROXYSQL_SESSION_MYSQL ||
              session_type == PROXYSQL_SESSION_SQLITE ||
              PROXYSQL_SESSION_ADMIN || PROXYSQL_SESSION_STATS)
          {
            ret= proxy_scramble_sha1((char *)pass,
                                     (*myds)->myconn->scramble_buff,
                                     password + 1, reply);
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 1,
              "Handshake (%s auth) <user:\"%s\" pass:\"%s\" scramble:\"%s\" "
              "db:\"%s\" max_pkt:%u>, capabilities:%u char:%u, use_ssl:%s\n",
              (capabilities & CLIENT_SECURE_CONNECTION ? "new" : "old"), user,
              password, pass, db, max_pkt, capabilities, charset,
              ((*myds)->encrypted ? "yes" : "no"));
  assert(sess);
...
void *MySQL_Protocol::Query_String_to_packet(uint8_t sid, std::string *s,
                                             unsigned int *l)
{
  mysql_hdr hdr;
  hdr.pkt_id= sid;
...
stmt_execute_metadata_t *MySQL_Protocol::get_binds_from_pkt(
    void *ptr, unsigned int size, MySQL_STMT_Global_info *stmt_info,
    stmt_execute_metadata_t **stmt_meta)
{
  ...
  char *p= (char *)ptr + 5;
  if (*stmt_meta)
  {  // this PS was executed at least once, and we already have metadata
    ret= *stmt_meta;
  }
  else
  {  // this is the first time that this PS is executed
  ...
  if (num_params)
  {
    ...
    {  // if STMT_EXECUTE was already executed once
      binds= ret->binds;
    ...
    for (i= 0; i < num_params; i++)
    {
      ...
      switch (buffer_type)
      {
        ...
        {
          binds[i].buffer=
              malloc(sizeof(MYSQL_TIME));  // NOTE: remember to free() this
          uint8_t l;
          memcpy(&l, p, 1);
          p++;
          MYSQL_TIME ts;
...
MySQL_ResultSet::MySQL_ResultSet(MySQL_Protocol *_myprot, MYSQL_RES *_res,
                                 MYSQL *_my, MYSQL_STMT *_stmt)
{
  ...
  //PSarrayOUT = NULL;
  if (myprot)
  {  // if myprot = NULL , this is a mirror
  ...
  myprot->generate_pkt_column_count(false, &pkt.ptr, &pkt.size, sid,
                                    num_fields, this);
  sid++;
  ...
  for (unsigned int i= 0; i < num_fields; i++)
  {
    ...
    myprot->generate_pkt_field(false, &pkt.ptr, &pkt.size, sid, field->db,
                               field->table, field->org_table, field->name,
                               field->org_name, field->charsetnr,
                               field->length, field->type, field->flags,
                               field->decimals, false, 0, NULL, this);
    //PSarrayOUT->add(pkt.ptr,pkt.size);
    resultset_size+= pkt.size;
    sid++;
  }
  ...
  setStatus=
      setStatus & ~SERVER_STATUS_CURSOR_EXISTS;  // Do not send cursor #1128
  ...
  //}
  if (_stmt)
  {  // binary protocol , we also assume we have ALL the resultset
    buffer_to_PSarrayOut();
...
unsigned int MySQL_ResultSet::add_row(MYSQL_ROW row)
{
  ...
  if (myprot)
  {
    sid= myprot->generate_pkt_row3(this, &pkt_length, sid, num_fields,
                                   lengths, row);
  }
  else
  {
    ...
    {
      pkt_length+=
          (row[col] ? lengths[col] + mysql_encode_length(lengths[col], NULL)
                    : 1);
    }
  }
  sid++;
...
void MySQL_ResultSet::add_err(MySQL_Data_Stream *_myds)
{
  ...
  if (myprot)
  {
    ...
    {  // see case #750
      ...
      {
        myprot->generate_pkt_ERR(
            false, &pkt.ptr, &pkt.size, sid, 1907, sqlstate,
            (char
                 *)"Query execution was interrupted, query_timeout exceeded");
      }
      else
      {
        myprot->generate_pkt_ERR(false, &pkt.ptr, &pkt.size, sid, 1317,
                                 sqlstate,
    ...
    {
      myprot->generate_pkt_ERR(false, &pkt.ptr, &pkt.size, sid,
                               mysql_errno(_mysql), sqlstate,
                               mysql_error(_mysql));
    }
    PSarrayOUT.add(pkt.ptr, pkt.size);
    sid++;
