                              ...
                              void *_data, unsigned long _size)
{
  ...
  sld->is_null=
      0;  // because the client is sending data, the field cannot be NULL
...
unsigned int StmtLongDataHandler::reset(uint32_t _stmt_id)
{
  ...
  int               i;
  stmt_long_data_t *sld= NULL;
  for (i= 0; i < (int)long_datas->len; i++)
  {  // we treat it as an int, so we can go to -1
    sld= (stmt_long_data_t *)long_datas->index(i);
                                               ...
                                               MYSQL_STMT *stmt, uint64_t _h)
{
  ...
  {  // see bug #899 . Most of the code is borrowed from
    ...
    if (ql >= 7)
    {
      if (strncasecmp(q, (char *)"SELECT ", 7) == 0)
      {  // is a SELECT
...
void MySQL_STMT_Global_info::update_metadata(MYSQL_STMT *stmt)
{
  ...
  if ((num_params != stmt->param_count) || (num_columns != stmt->field_count))
  {
    need_refresh= true;
  }
  for (i= 0; i < num_columns; i++)
  {
    ...
    {  // don't bother to check if need_refresh == true
      bool         ok= true;
      MYSQL_FIELD *fs= &(stmt->fields[i]);
      MYSQL_FIELD *fd= fields[i];
      if (ok)
      ...
      if (ok)
      {
        ...
        {
          if (fd->org_name && fs->org_name &&
              strcmp(fd->org_name, fs->org_name) == 0)
          {
            ok= true;
          }
        }
      ...
      if (ok)
      {
        ...
        {
          if (fd->org_table && fs->org_table &&
              strcmp(fd->org_table, fs->org_table) == 0)
          {
            ok= true;
          }
        }
      ...
      if (ok)
      {
        ...
        {
          if (fd->catalog && fs->catalog &&
              strcmp(fd->catalog, fs->catalog) == 0)
          {
            ok= true;
          }
        }
...
void MySQL_STMTs_local_v14::backend_insert(uint64_t    global_statement_id,
                                           MYSQL_STMT *stmt)
{
  std::pair<std::map<uint64_t, MYSQL_STMT *>::iterator, bool> ret;
  ret= global_stmt_to_backend_stmt.insert(
      std::make_pair(global_statement_id, stmt));
  global_stmt_to_backend_ids.insert(
      std::make_pair(global_statement_id, stmt->stmt_id));
  backend_stmt_to_global_ids.insert(
      std::make_pair(stmt->stmt_id, global_statement_id));
...
uint64_t MySQL_STMTs_local_v14::compute_hash(unsigned int hostgroup,
                                             char *user, char *schema,
                                             char *       query,
                                             unsigned int query_length)
{
  uint64_t hash;
...
MySQL_STMT_Manager_v14::MySQL_STMT_Manager_v14()
{
  ...
  map_stmt_id_to_info=
      std::map<uint64_t,
               MySQL_STMT_Global_info *>();  // map using statement id
  map_stmt_hash_to_info=
      std::map<uint64_t, MySQL_STMT_Global_info *>();  // map using hashes
...
void MySQL_STMT_Manager_v14::ref_count_client(uint64_t _stmt_id, int _v,
                                              bool lock)
{
  if (lock)
    pthread_rwlock_wrlock(&rwlock_);
  ...
  if (s != map_stmt_id_to_info.end())
  {
    ...
    time_t   ct= time(NULL);
    uint64_t num_client_count_zero=
        __sync_add_and_fetch(&num_stmt_with_ref_client_count_zero, 0);
    uint64_t num_server_count_zero=
        __sync_add_and_fetch(&num_stmt_with_ref_server_count_zero, 0);
    ...
    {  // purge only if there is at least 10% gain
      ...
      int       i= -1;
      uint64_t *torem= (uint64_t *)malloc(max_purge * sizeof(uint64_t));
           ...
           it != map_stmt_id_to_info.end(); ++it)
      {
        ...
        if ((__sync_add_and_fetch(&a->ref_count_client, 0) == 0) &&
            (a->ref_count_server ==
             0))  // this to avoid that IDs are incorrectly reused
        {
...
void MySQL_STMT_Manager_v14::ref_count_server(uint64_t _stmt_id, int _v,
                                              bool lock)
{
  if (lock)
    pthread_rwlock_wrlock(&rwlock_);
...
MySQL_STMTs_local_v14::~MySQL_STMTs_local_v14()
{
  ...
  if (is_client_)
  {
    for (std::map<uint32_t, uint64_t>::iterator it=
             client_stmt_to_global_ids.begin();
  ...
  {
    for (std::map<uint64_t, MYSQL_STMT *>::iterator it=
             global_stmt_to_backend_stmt.begin();
...
MySQL_STMT_Manager_v14::find_prepared_statement_by_hash(uint64_t hash,
                                                        bool     lock)
{
...
MySQL_STMT_Global_info *
MySQL_STMT_Manager_v14::find_prepared_statement_by_stmt_id(uint64_t id,
                                                           bool     lock)
{
  MySQL_STMT_Global_info *ret= NULL;  // assume we do not find it
  if (lock)
...
uint32_t MySQL_STMTs_local_v14::generate_new_client_stmt_id(
    uint64_t global_statement_id)
{
  uint32_t ret= 0;
  /*
...
uint64_t MySQL_STMTs_local_v14::find_global_stmt_id_from_client(
...
bool MySQL_STMTs_local_v14::client_close(uint32_t client_statement_id)
{
  ...
  {  // found
    ...
    std::pair<std::multimap<uint64_t, uint32_t>::iterator,
              std::multimap<uint64_t, uint32_t>::iterator>
        ret;
    ret= global_stmt_to_client_ids.equal_range(global_stmt_id);
    for (std::multimap<uint64_t, uint32_t>::iterator it= ret.first;
         it != ret.second; ++it)
    {
...
void MySQL_STMT_Manager_v14::get_metrics(uint64_t *c_unique,
                                         uint64_t *c_total,
                                         uint64_t *stmt_max_stmt_id,
                                         uint64_t *cached, uint64_t *s_unique,
                                         uint64_t *s_total)
{
  uint64_t c_u= 0;
  uint64_t c_t= 0;
...
#endif
  *stmt_max_stmt_id=
      next_statement_id;  // this is max stmt_id, no matter if in used or not
...
class PS_global_stats {
 public:
  ...
  char *             query;
  PS_global_stats(uint64_t stmt_id, unsigned int h, char *u, char *s,
                  uint64_t d, char *q, unsigned long long ref_c,
                  unsigned long long ref_s)
  {
...
SQLite3_result *MySQL_STMT_Manager_v14::get_prepared_statements_global_infos()
{
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 4,
              "Dumping current prepared statements global info\n");
  ...
  {
    MySQL_STMT_Global_info *a= it->second;
    PS_global_stats *       pgs= new PS_global_stats(
        a->statement_id, a->hostgroup_id, a->schemaname, a->username, a->hash,
        a->query, a->ref_count_client, a->ref_count_server);
